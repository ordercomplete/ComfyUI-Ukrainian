...
import { Ai as useCommandStore, Mi as KeybindingImpl, Ni as KeyComboImpl, ji as useKeybindingStore, n as useSettingStore } from "./settingStore-pm7IqVHI.js";
...
function useKeybindingService() {
	...
	function registerCoreKeybindings() {
		for (const keybinding of CORE_KEYBINDINGS) keybindingStore.addDefaultKeybinding(new KeybindingImpl(keybinding));
	}
	function registerUserKeybindings() {
		const unsetBindings = settingStore.get("Comfy.Keybinding.UnsetBindings");
		for (const keybinding of unsetBindings) {
			if (!commandStore.isRegistered(keybinding.commandId)) continue;
			keybindingStore.unsetKeybinding(new KeybindingImpl(keybinding));
		}
		const newBindings = settingStore.get("Comfy.Keybinding.NewBindings");
		for (const keybinding of newBindings) keybindingStore.addUserKeybinding(new KeybindingImpl(keybinding));
	}
...
//# sourceMappingURL=keybindingService-hcJ8JpQE.js.map