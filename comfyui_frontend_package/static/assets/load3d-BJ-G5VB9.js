...
import { Q as nextTick } from "./vendor-vue-core-oGuyqViA.js";
import { i as api } from "./api-btlSMXR9.js";
import { t as useToastStore } from "./toastStore-D7DQZkvm.js";
import { Da as isLoad3dNode, Ti as useExtensionService, Zo as getNodeByLocatorId, ao as ComponentWidgetImpl, c as Load3dUtils, f as SUPPORTED_EXTENSIONS_ACCEPT, i as app, r as ComfyApp, so as addWidget, u as LOAD3D_NONE_MODEL } from "./settingStore-pm7IqVHI.js";
...
import { t as Load3dViewerContent_default } from "./Load3dViewerContent-X9d0nI3p.js";
import { t as useLoad3dService } from "./load3dService-DMEySLTa.js";
import { a as setLoad3dOutputCache, i as nodeToLoad3dMap, n as isLoad3dSceneDirty, o as useLoad3d, r as markLoad3dSceneDirty, t as getLoad3dOutputCache } from "./useLoad3d-DQeWn-fO.js";
import { n as createExportMenuItems, t as Load3DConfiguration } from "./Load3DConfiguration-BM9JfDCf.js";
import { t as snapshotLoad3dState } from "./load3dSerialize-DBoVw4W-.js";
...
function applyPreview3DAdvancedResult(node, load3d, result, loadFolder, comfyClass) {
	const filePath = result[0];
	if (!filePath) return;
	const normalizedPath = filePath.replaceAll("\\", "/");
	node.properties["Last Time Model File"] = normalizedPath;
	new Load3DConfiguration(load3d, node.properties).configureForSaveMesh(loadFolder, normalizedPath, { silentOnNotFound: true });
	const cameraState = result[1];
	...
	return {
		...
		onNodeOutputsUpdated(nodeOutputs) {
			for (const [locatorId, output] of Object.entries(nodeOutputs)) {
				...
				useLoad3d(node).waitForLoad3d((load3d) => {
					applyPreview3DAdvancedResult(node, load3d, result, loadFolder, comfyClass);
				});
			}
		},
		getNodeMenuItems(node) {
		...
		async nodeCreated(node) {
			...
			onLoad3dReady((load3d) => {
				const lastTimeModelFile = node.properties["Last Time Model File"];
				if (!lastTimeModelFile) return;
				new Load3DConfiguration(load3d, node.properties).configureForSaveMesh(loadFolder, lastTimeModelFile, { silentOnNotFound: true });
				const cameraState = node.properties["Camera Config"]?.state;
			...
			waitForLoad3d((load3d) => {
				...
				node.onExecuted = function(output) {
					...
					applyPreview3DAdvancedResult(node, resolveLoad3d(), result, loadFolder, comfyClass);
				};
			});
		}
	};
...
//# sourceMappingURL=load3d-BJ-G5VB9.js.map