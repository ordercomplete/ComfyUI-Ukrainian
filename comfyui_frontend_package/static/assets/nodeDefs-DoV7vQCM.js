//#region src/locales/zh/nodeDefs.json
var APG = {
	...
	"inputs": {
		...
		"momentum": {
			"name": "动量",
			"tooltip": "控制扩散过程中的引导运行平均值，设置为0时禁用。"
		},
		"norm_threshold": {
			"name": "向量归一化",
			"tooltip": "将引导向量归一化到此值，设置为 0 时禁用归一化。"
		}
	},
...
var ARVideoI2V = {
	...
	"inputs": {
		"batch_size": { "name": "批量大小" },
		"height": { "name": "高度" },
		"length": { "name": "长度" },
		"model": { "name": "模型" },
		"start_image": { "name": "起始图像" },
		"vae": { "name": "vae" },
		"width": { "name": "宽度" }
	},
	"outputs": {
...
var AdjustBrightness = {
	...
	"inputs": {
		"factor": {
			"name": "系数",
			"tooltip": "亮度系数。1.0表示无变化，<1.0表示更暗，>1.0表示更亮。"
		},
		"images": {
			"name": "图像",
			"tooltip": "需要调整的图像"
...
var AdjustContrast = {
	...
	"inputs": {
		"factor": {
			"name": "系数",
			"tooltip": "对比度系数。1.0表示无变化，<1.0表示更低，>1.0表示更高。"
		},
		"images": {
			"name": "图像",
			"tooltip": "需要调整的图像"
...
var AlignYourStepsScheduler = {
	...
	"inputs": {
		"denoise": { "name": "降噪" },
		"model_type": { "name": "模型类型" },
		"steps": { "name": "步数" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var AnimaLLLiteApply = {
	"display_name": "应用 Anima LLLite",
	"inputs": {
		"end_percent": { "name": "结束百分比" },
		"image": { "name": "图像" },
		"mask": { "name": "掩码" },
		"model": { "name": "模型" },
		"model_patch": { "name": "模型补丁" },
		"start_percent": { "name": "起始百分比" },
		"strength": { "name": "强度" }
	}
};
var AudioAdjustVolume = {
	"description": "按指定的分贝（dB）调整音频音量。",
	"display_name": "音频调整音量",
	"inputs": {
		"audio": { "name": "音频" },
		"volume": {
			"name": "音量",
			"tooltip": "音量调整，单位为分贝 (dB)。0 = 无变化，+6 = 加倍，-6 = 减半，等等"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var AudioConcat = {
	"description": "将 音频1 与 音频2 按指定方向连接。",
	"display_name": "音频拼接",
	"inputs": {
		"audio1": { "name": "音频1" },
		"audio2": { "name": "音频2" },
		"direction": {
			"name": "方向",
			"tooltip": "音频2 在 音频1 之前或之后。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var AudioEncoderEncode = {
	"display_name": "音频编码器编码",
	"inputs": {
		"audio": { "name": "音频" },
		"audio_encoder": { "name": "音频编码器" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var AudioEncoderLoader = {
	"display_name": "加载音频编码器",
	"inputs": { "audio_encoder_name": { "name": "音频编码器" } },
	"outputs": { "0": { "tooltip": null } }
};
...
var BatchImagesNode = {
	"display_name": "批量图像",
	"inputs": { "images": { "name": "图像" } },
	"outputs": { "0": { "tooltip": null } }
};
var BatchLatentsNode = {
	"display_name": "批量latent",
	"inputs": { "latents": { "name": "latent" } },
	"outputs": { "0": { "tooltip": null } }
};
var BatchMasksNode = {
	"display_name": "批量mask",
	"inputs": { "masks": { "name": "mask" } },
	"outputs": { "0": { "tooltip": null } }
};
var BeebleSwitchXImageEdit = {
	"description": "使用 Beeble SwitchX 编辑单张图像。可切换场景中的任何元素（背景、光照、服装），同时保留原始主体像素。请提供参考图像和/或文本提示来描述新外观。最大约 2.77MP。",
	"display_name": "Beeble SwitchX 图像编辑",
	"inputs": {
		"alpha_mode": {
			"name": "alpha_mode",
			"tooltip": "控制 SwitchX 如何决定保留或重生成内容。'auto' 自动隔离主要主体。'fill' 重生成整个画面并保留几何结构。'select' 将首帧关键帧传播到整个 clip。'custom' 使用您提供的每帧 alpha mask。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"image": { "name": "图像" },
		"max_resolution": {
			"name": "最大分辨率",
			"tooltip": "输出的最大分辨率。"
		...
		"prompt": {
			"name": "提示词",
			"tooltip": "期望输出的文本描述（最多 2000 字符）。“提示词”或“参考图像”至少需填写一项。"
		},
		"reference_image": {
			"name": "参考图像",
			"tooltip": "结果将采用该参考图像的外观（背景、光照、服装）。“参考图像”或“提示词”至少需填写一项。"
		},
		"seed": {
			"name": "种子",
	...
	"outputs": {
		...
		"1": {
			"name": "alpha",
			"tooltip": "Beeble 使用的 alpha mask。'fill' 模式下为空，该模式没有单独的 mask。"
		}
	}
};
var BeebleSwitchXVideoEdit = {
	"description": "使用 Beeble SwitchX 编辑视频。可切换场景中的任何元素（背景、光照、服装），同时保留原始主体像素和动作。请提供参考图像和/或文本提示来描述新外观。最多 240 帧，每帧最大约 2.77MP。",
	"display_name": "Beeble SwitchX 视频编辑",
	"inputs": {
		"alpha_mode": {
			"name": "alpha_mode",
			"tooltip": "控制 SwitchX 如何决定保留或重生成内容。'auto' 自动隔离主要主体。'fill' 重生成整个画面并保留几何结构。'select' 将首帧关键帧传播到整个 clip。'custom' 使用您提供的每帧 alpha mask。"
...
var BerniniConditioning = {
	"description": "用于Bernini上下文视频/图像条件的条件节点。可用于以下任务：t2v（文本转视频）、v2v（视频转视频）、rv2v（参考引导视频编辑）、r2v（参考转视频）、ads2v（将图像/视频插入视频）。作为上下文token注入的参考图像（r2v、rv2v）会以其原生宽高比独立编码（长边最大为ref_max_size）。",
	...
	"inputs": {
		...
		"ref_max_size": {
			"name": "参考最大尺寸",
			"tooltip": "reference_video和reference_images长边的最大尺寸。调整尺寸时保持宽高比，并对齐到16像素。"
		},
		"reference_images": { "name": "参考图像" },
		"reference_video": {
			"name": "参考视频",
			"tooltip": "要插入到源视频中的视频（ads2v）。"
		},
		"source_video": {
			"name": "源视频",
			"tooltip": "要编辑或重绘风格的视频（v2v，rv2v）。会被调整为指定宽高并裁剪到指定长度。"
		},
		"vae": { "name": "vae" },
		"width": { "name": "宽度" }
	},
...
var BriaTransparentVideoBackground = {
	"description": "使用 Bria 移除视频背景，并返回抠图帧和 alpha mask。可将两者连接到合成节点，或传递给保存 WEBM 节点以生成透明视频。",
	"display_name": "Bria 移除视频背景（透明）",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"seed": {
			"name": "种子",
...
var BriaVideoGreenScreen = {
	"description": "使用 Bria 将视频背景替换为纯色抠像屏幕。",
	"display_name": "Bria 视频绿幕",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"green_shade": {
			"name": "green_shade",
			"tooltip": "应用在前景后方的纯色抠像色：broadcast_green（#00B140）、chroma_green（#00FF00）或 blue_screen（#0000FF）。"
		},
		"seed": {
			"name": "seed",
...
var BriaVideoReplaceBackground = {
	"description": "使用 Bria 将视频的背景替换为提供的图像或视频。输出保持前景的分辨率和帧率；如果背景的宽高比不同，则会被拉伸以适应，因此请匹配宽高比以获得无失真的效果。",
	"display_name": "Bria 视频更换背景",
	"inputs": {
		"background_image": {
			"name": "background_image",
			"tooltip": "用于合成在前景后面的背景图像。请只提供背景图像或背景视频中的一个。"
		},
		"background_video": {
			"name": "background_video",
			"tooltip": "用于合成在前景后面的背景视频。请只提供背景图像或背景视频中的一个。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"seed": {
			"name": "seed",
...
var BuildJsonPromptIdeogram = {
	"description": "为 Ideogram 4 模型构建 JSON 提示。",
	"display_name": "构建 JSON 提示（Ideogram）",
	"inputs": {
		"aesthetics": {
			"name": "美学",
			"tooltip": "必填美学关键词（如：情绪化、电影感、去饱和）。"
		},
		"background": {
			"name": "背景",
			"tooltip": "图像背景或环境的必填描述。"
		},
		"color_palette": {
			"name": "色板",
			"tooltip": "用于引导图像主色调的十六进制颜色代码。最多可填 16 项。"
		},
		"element": {
			"name": "元素",
			"tooltip": "来自节点 Create Bounding Boxes 的提示元素。"
		},
		"high_level_description": {
			"name": "高级描述",
			"tooltip": "对图像的可选描述，一到两句话。强烈推荐填写。"
		},
		"lighting": {
			"name": "光照",
			"tooltip": "必填光照描述（如：金色时刻、轮廓光、戏剧性阴影）。"
		},
		"medium": {
			"name": "媒介",
			"tooltip": "必填媒介类型（如：摄影、插画、3d_render、绘画、平面设计）。当风格为 photo 时，设置为摄影。"
		},
		"style": { "name": "风格" }
	},
	"outputs": { "0": {
...
var ByteDance2FirstLastFrameNode = {
	"description": "使用 Seedance 2.0 从首帧图像和可选的末帧图像生成视频。",
	"display_name": "ByteDance Seedance 2.0 首帧-末帧生成视频",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"first_frame": {
			"name": "首帧图像",
		...
		"first_frame_asset_id": {
			...
			"tooltip": "用于第一帧的 Seedance asset_id。与 first_frame 图像输入互斥。"
		},
		"last_frame": {
			"name": "末帧图像",
			"tooltip": "视频的末帧图像。"
		...
		"last_frame_asset_id": {
			...
			"tooltip": "用于最后一帧的 Seedance asset_id。与 last_frame 图像输入互斥。"
		},
		"model": {
			"name": "模型",
			"tooltip": "Seedance 2.0 提供最高质量；Seedance 2.0 Fast 优化速度。"
		},
		"model_duration": { "name": "时长" },
		"model_generate_audio": { "name": "生成音频" },
		"model_prompt": { "name": "提示词" },
		"model_ratio": { "name": "比例" },
		"model_resolution": { "name": "分辨率" },
		"seed": {
			"name": "种子",
			"tooltip": "种子控制节点是否重新运行；无论种子如何，结果都是非确定性的。"
		},
...
var ByteDance2ReferenceNode = {
	"description": "使用 Seedance 2.0 结合参考图像、视频和音频生成、编辑或扩展视频。支持多模态参考、视频编辑和视频扩展。",
	"display_name": "ByteDance Seedance 2.0 参考生成视频",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"model": {
			"name": "模型",
		...
		"model_auto_downscale": { "name": "自动降采样" },
...
var ByteDance2TextToVideoNode = {
	"description": "基于文本提示词，使用 Seedance 2.0 模型生成视频。",
	"display_name": "ByteDance Seedance 2.0 文本生成视频",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"model": {
			"name": "模型",
...
var ByteDanceCreateImageAsset = {
	"description": "创建一个 Seedance 2.0 个人图像资产。上传输入图像并将其注册到指定资产组。如果 group_id 为空，则会运行真人 H5 认证流程，在添加资产前创建新分组。",
	"display_name": "ByteDance 创建图像资产",
	"inputs": {
		"group_id": {
			"name": "group_id",
			"tooltip": "复用已有的 Seedance 资产组 ID，可跳过同一人的重复人脸验证。留空将在浏览器中进行真人认证并创建新分组。"
		},
		"image": {
			"name": "image",
			"tooltip": "要注册为个人资产的图像。"
...
var ByteDanceCreateVideoAsset = {
	"description": "创建一个 Seedance 2.0 个人视频资产。上传输入视频并将其注册到指定资产组。如果 group_id 为空，则会运行真人 H5 认证流程，在添加资产前创建新分组。",
	"display_name": "ByteDance 创建视频资产",
	"inputs": {
		"group_id": {
			"name": "group_id",
			"tooltip": "复用已有的 Seedance 资产组 ID，可跳过同一人的重复人脸验证。留空将在浏览器中进行真人认证并创建新分组。"
...
var ByteDanceFirstLastFrameNode = {
	...
	"inputs": {
		...
		"camera_fixed": {
			"name": "相机固定",
			"tooltip": "指定是否固定相机。平台会在您的提示词中附加固定相机的指令，但不保证实际效果。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"duration": {
			"name": "时长",
		...
		"watermark": {
			"name": "水印",
			"tooltip": "是否在视频中添加“AI 生成”水印。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var ByteDanceImageNode = {
	"description": "通过基于提示的API使用字节跳动模型生成图像",
	"display_name": "字节跳动图片",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"guidance_scale": {
			"name": "引导尺度",
			"tooltip": "数值越高，图像越紧密地遵循提示"
		},
		"height": {
			"name": "高度",
		...
		"size_preset": {
			"name": "尺寸预设",
			"tooltip": "选择一个推荐尺寸。选择自定义以使用下方的宽度和高度"
		},
		"watermark": {
			"name": "水印",
		...
		"width": {
			"name": "宽度",
			"tooltip": "图像的自定义宽度。仅当 `size_preset` 设置为 `Custom` 时该值才生效"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var ByteDanceImageReferenceNode = {
	"description": "使用提示和参考图像生成视频。",
	"display_name": "字节跳动参考图像转视频",
	"inputs": {
		"aspect_ratio": {
			"name": "宽高比",
			"tooltip": "输出视频的宽高比。"
...
var ByteDanceImageToVideoNode = {
	"description": "通过API基于图像和提示使用字节跳动模型生成视频",
	"display_name": "字节跳动图片转视频",
	"inputs": {
		"aspect_ratio": {
			"name": "宽高比",
			"tooltip": "输出视频的宽高比。"
		...
		"watermark": {
			"name": "水印",
			"tooltip": "是否在视频中添加“AI生成”水印。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var ByteDanceSeedAudio = {
	"description": "使用 ByteDance Seed Audio 1.0，通过单个提示生成语音、音乐、音效和多说话人对话。在提示中描述声音、情感、氛围、背景音乐和音效，并包含要说的台词。可选内置预设声音，从最多 3 个参考 clip（在提示中标记为 @Audio1-3）克隆声音，或从角色图像推导声音。每次最多生成 2 分钟音频。",
	"display_name": "ByteDance Seed Audio 1.0",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"loudness_rate": {
			"name": "loudness_rate",
			"tooltip": "响度。0 = 正常，100 = 2.0x，-50 = 0.5x。"
		},
		"model": {
			"name": "model",
			"tooltip": "seed-audio-1.0-multilingual：支持20种语言（英语、中文、日语、韩语、墨西哥西班牙语、西班牙西班牙语、印尼语、德语、巴西葡萄牙语、法语、泰语、越南语、马来语、菲律宾语、意大利语、俄语、荷兰语、波兰语、土耳其语、瑞典语），并可通过“[5.5s:8.0s] ...”时间戳进行逐句时序控制。seed-audio-1.0：仅支持英语和中文，不支持时序控制。"
		},
		"pitch_rate": {
			"name": "pitch_rate",
			"tooltip": "音高变化（半音，-12 到 12）。"
		},
		"reference_mode": {
			"name": "reference_mode",
			"tooltip": "声音的条件方式：'仅文本'（所有内容都在提示中描述）、'音频参考'（克隆最多 3 个声音，标记为 @Audio1-3）、'图像参考'（从一个角色图像推导声音）、或'预设声音'（选择内置命名声音朗读提示）。"
		},
		"sample_rate": {
			"name": "sample_rate",
			"tooltip": "输出采样率（Hz）。"
		...
		"speech_rate": {
			"name": "speech_rate",
			"tooltip": "语速。0 = 正常，100 = 2.0x，-50 = 0.5x。"
		},
		"text_prompt": {
			"name": "text_prompt",
			"tooltip": "描述声音、情感、语速、氛围、背景音乐和音效，并包含要说的台词（对话请直接写角色名）。在“音频参考”模式下，按顺序引用已连接的 clip，如 @Audio1、@Audio2、@Audio3。最多 3000 字符。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var ByteDanceSeedNode = {
	"description": "使用字节跳动的 Seed 2.0 模型生成文本回复。请提供文本提示，并可选地添加一张或多张图像或视频以实现多模态上下文。",
	"display_name": "字节跳动 Seed",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"model": {
			"name": "model",
			"tooltip": "用于生成回复的 Seed 模型。"
		},
...
var ByteDanceSeedreamNode = {
	...
	"inputs": {
		...
		"fail_on_partial": {
			"name": "部分失败时停止",
			"tooltip": "如果启用，当任何请求的图像缺失或返回错误时，将中止执行。"
		},
		"height": {
			"name": "高度",
			"tooltip": "图像的自定义高度。仅当 `size_preset` 设置为 `Custom` 时该值生效"
		},
		"image": {
			"name": "图像",
			"tooltip": "用于图像到图像生成的输入图像。用于单参考或多参考生成的1-10张图像列表。"
		},
		"max_images": {
			"name": "最大图片数",
			"tooltip": "当 sequential_image_generation='auto' 时生成图像的最大数量。总图像数（输入+生成）不能超过 15。"
		},
		"model": { "name": "模型" },
		"prompt": {
			"name": "提示",
		...
		"sequential_image_generation": {
			"name": "顺序图像生成",
			"tooltip": "分组图像生成模式。'disabled' 生成单张图像。'auto' 由模型决定是否生成多张相关图像（例如故事场景、角色变体）。"
		},
		"size_preset": {
			"name": "尺寸预设",
			"tooltip": "选择一个推荐尺寸。选择“自定义”以使用下面的宽度和高度。"
		},
		"watermark": {
			"name": "水印",
			"tooltip": "是否在图像上添加“AI 生成”水印。"
		},
		"width": {
			"name": "宽度",
			"tooltip": "图像的自定义宽度。仅当 `size_preset` 设置为 `Custom` 时该值生效"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var ByteDanceSeedreamNodeV2 = {
	...
	"inputs": {
		...
		"thinking": {
			"name": "thinking",
			"tooltip": "启用模型的提示优化推理（“thinking”），以更好地遵循提示。可能会显著增加生成时间——在 Seedream 5.0 Pro 上尤为明显。仅在文本转图像时可关闭（提供参考图像时无法关闭）。"
		},
		"watermark": {
			"name": "水印",
			"tooltip": "是否在图像上添加“AI生成”水印。"
...
var ByteDanceTextToVideoNode = {
	...
	"inputs": {
		...
		"camera_fixed": {
			"name": "固定相机",
			"tooltip": "指定是否固定相机。平台会在您的提示后附加固定相机的指令，但不保证实际效果。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"duration": {
			"name": "时长",
...
var CFGNorm = {
	...
	"inputs": {
		...
		"pre_cfg": {
			"name": "pre_cfg",
			"tooltip": "如果为 true，则在采样器的 CFG 合并之前对组合噪声进行重新缩放，不进行限制（可能会放大）。这与 Lens 等模型使用的 norm-scaled CFG 相匹配。默认 false 保持原有的 post-CFG x0 空间仅衰减行为。"
		},
		"strength": { "name": "强度" }
	},
	"outputs": { "0": {
...
var CFGOverride = {
	"description": "在步骤的 [起始百分比, 结束百分比] 区间内将 cfg 覆盖为固定值。若有多个覆盖，采样器最近的覆盖优先生效。",
	"display_name": "CFG 覆盖",
	"inputs": {
		"cfg": { "name": "cfg" },
		"end_percent": { "name": "结束百分比" },
...
var CLIPSetLastLayer = {
	...
	"inputs": {
		"clip": { "name": "clip" },
		"stop_at_clip_layer": { "name": "停止在CLIP层" }
	}
};
var CLIPTextEncode = {
	...
	"outputs": { "0": { "tooltip": "包含嵌入文本的条件，用于引导扩散模型。" } }
};
var CLIPTextEncodeControlnet = {
	"display_name": "CLIP文本编码(ControlNet)",
	"inputs": {
		"clip": { "name": "clip" },
		"conditioning": { "name": "条件" },
		"text": { "name": "文本" }
...
var CLIPTextEncodeKandinsky5 = {
	"display_name": "CLIP文本编码（Kandinsky5）",
	"inputs": {
		"clip": { "name": "clip" },
		"clip_l": { "name": "clip_l" },
...
var CLIPTextEncodeSDXLRefiner = {
	"display_name": "CLIP文本编码（SDXLRefiner）",
	"inputs": {
		"ascore": { "name": "美学分数" },
		"clip": { "name": "clip" },
		"height": { "name": "高度" },
...
var CLIPVisionLoader = {
	...
	"inputs": { "clip_name": { "name": "clip名称" } }
};
var Canny = {
	"display_name": "Canny边缘检测",
	"inputs": {
		"high_threshold": { "name": "高阈值" },
		"image": { "name": "图像" },
		"low_threshold": { "name": "低阈值" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var CaseConverter = {
...
var CenterCropImages = {
	"description": "将图像居中裁剪为指定尺寸。",
	"display_name": "裁剪图像（中心）",
	"inputs": {
		"height": {
			"name": "高度",
			"tooltip": "裁剪框高度"
...
var CheckpointLoader = {
	...
	"inputs": {
		"ckpt_name": { "name": "Checkpoint名称" },
		"config_name": { "name": "配置名称" }
	}
};
var CheckpointLoaderSimple = {
	...
	"outputs": {
		"0": { "tooltip": "用于去除 Latent 噪波的模型。" },
		"1": { "tooltip": "用于编码文本提示的 CLIP 模型。" },
		"2": { "tooltip": "用于将图像编码和解码到 Latent 的 VAE 模型。" }
	}
};
var CheckpointSave = {
...
var ChromaRadianceOptions = {
	"description": "允许为Chroma Radiance模型设置高级选项。",
	"display_name": "ChromaRadiance选项",
	"inputs": {
		"end_sigma": {
			"name": "结束sigma",
			"tooltip": "这些选项生效的最后一个sigma值。"
		},
		"force_sequential_txt_ids": {
			"name": "强制顺序文本ID",
			"tooltip": "强制使用顺序文本标记ID，而不是零。应在2026-05-22至2026-06-01期间以此方式训练但状态字典中不包含 __sequential__ 键的检查点中使用。"
		},
		"model": { "name": "模型" },
		"nerf_tile_size": {
			"name": "NeRF瓦片大小",
			"tooltip": "允许覆盖默认的NeRF瓦片大小。-1表示使用默认值（32）。0表示使用非平铺模式（可能需要大量显存）。"
		},
		"preserve_wrapper": {
			"name": "保留包装器",
			"tooltip": "启用时，如果存在现有模型函数包装器，将委托给该包装器。通常应保持启用状态。"
		},
		"start_sigma": {
			"name": "起始sigma",
			"tooltip": "这些选项生效的第一个sigma值。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var ClaudeNode = {
	"description": "使用Anthropic的Claude模型生成文本回复。提供文本提示，并可选提供一张或多张图像用于多模态上下文。",
	"display_name": "Anthropic Claude",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"images": {
			"name": "images",
			"tooltip": "可选图像，作为模型的上下文使用。最多支持20张图像。"
		},
		"model": {
			"name": "model",
			"tooltip": "用于生成回复的Claude模型。"
		},
		"model_max_tokens": { "name": "最大tokens数" },
...
var ColorTransfer = {
	"description": "使用多种算法将一张图像的颜色匹配到另一张图像。",
	"display_name": "ColorTransfer",
	"inputs": {
		"image_ref": {
			"name": "image_ref",
			"tooltip": "要匹配颜色的参考图像。如果未提供，则跳过处理。"
		},
		"image_target": {
			"name": "image_target",
			"tooltip": "要应用颜色变换的图像。"
		...
		"source_stats": {
			"name": "source_stats",
			"tooltip": "per_frame：每一帧分别与 image_ref 匹配。uniform：将所有源帧的统计数据汇总作为基线，与 image_ref 匹配。target_frame：选择一帧作为变换到 image_ref 的基线，统一应用到所有帧（保留相对差异）。"
		},
...
var ComfyAndNode = {
	"description": "逻辑与操作。如果所有值都为真，则返回 true。使用 Python 的真值规则。",
	"display_name": "与",
	"inputs": { "values": { "name": "值" } },
	"outputs": { "0": { "tooltip": null } }
};
...
var ComfyNotNode = {
	"description": "逻辑非操作。如果值为假，则返回 true。使用 Python 的真值规则。",
	"display_name": "非",
	"inputs": { "value": { "name": "值" } },
	"outputs": { "0": { "tooltip": null } }
};
...
var ComfyOrNode = {
	"description": "逻辑或操作。如果任意值为真，则返回 true。使用 Python 的真值规则。",
	"display_name": "或",
	"inputs": { "values": { "name": "值" } },
	"outputs": { "0": { "tooltip": null } }
};
...
var ConditioningAverage = {
	"display_name": "条件平均",
	"inputs": {
		"conditioning_from": { "name": "条件从" },
		"conditioning_to": { "name": "条件到" },
		"conditioning_to_strength": { "name": "条件到强度" }
	}
};
var ConditioningCombine = {
	"display_name": "条件合并",
	"inputs": {
		"conditioning_1": { "name": "条件_1" },
		"conditioning_2": { "name": "条件_2" }
	}
};
...
var ConditioningStableAudio = {
	...
	"inputs": {
		"negative": { "name": "负面条件" },
		"positive": { "name": "正面条件" },
		"seconds_start": { "name": "开始秒数" },
		"seconds_total": { "name": "总秒数" }
	},
	"outputs": {
		"0": {
			"name": "正面条件",
...
var ConditioningZeroOut = {
	"display_name": "条件零化",
	"inputs": { "conditioning": { "name": "条件" } }
};
var ContextWindowsManual = {
	"description": "手动设置上下文窗口。",
	"display_name": "上下文窗口（手动）",
	"inputs": {
		"causal_window_fix": {
			"name": "causal_window_fix",
			"tooltip": "是否为非0索引的上下文窗口添加因果修正帧。"
		},
		"closed_loop": {
			"name": "闭环",
			"tooltip": "是否闭合上下文窗口循环；仅适用于循环调度。"
		},
		"cond_retain_index_list": {
			"name": "cond_retain_index_list",
			"tooltip": "用于每个窗口在条件张量中保留的latent索引列表，例如设置为“0”将为每个窗口使用初始起始图像。"
		},
		"context_length": {
			"name": "上下文长度",
			"tooltip": "上下文窗口的长度。"
		...
		"freenoise": {
			"name": "Freenoise",
			"tooltip": "是否应用 Freenoise，优化窗口融合。"
		},
		"fuse_method": {
			"name": "融合方法",
			"tooltip": "用于融合上下文窗口的方法。"
		},
		"latent_retain_index_list": {
			"name": "latent_retain_index_list",
			"tooltip": "每个窗口中要在噪声latent本身中保留的latent索引列表。适用于参考内容（如起始图像）直接存在于噪声latent中，而不是单独的条件通道中的工作流（如inplace风格的I2V，例如LTXV、AnimateDiff）。与cond_retain_index_list无关。"
		},
		"model": {
			"name": "模型",
			"tooltip": "在采样期间应用上下文窗口的模型。"
		},
		"split_conds_to_windows": {
			"name": "split_conds_to_windows",
			"tooltip": "是否将多个条件（由ConditionCombine创建）根据区域索引分配到各个窗口。"
		}
	},
	"outputs": { "0": { "tooltip": "在采样过程中应用上下文窗口的模型。" } }
};
var ControlNetApply = {
...
var ControlNetLoader = {
	...
	"inputs": { "control_net_name": { "name": "ControlNet名称" } }
};
...
var CosmosImageToVideoLatent = {
	"display_name": "图像到视频Latent（Cosmos）",
	"inputs": {
		"batch_size": { "name": "批量大小" },
		"end_image": { "name": "结束图像" },
		"height": { "name": "高度" },
		"length": { "name": "长度" },
		"start_image": { "name": "开始图像" },
		"vae": { "name": "vae" },
		"width": { "name": "宽度" }
	},
	"outputs": { "0": { "tooltip": null } }
...
var CosmosPredict2ImageToVideoLatent = {
	"display_name": "图像到视频Latent（CosmosPredict2）",
	"inputs": {
...
var CreateBoundingBoxes = {
	"description": "在画布上绘制边界框。输出 Ideogram 提示元素、像素空间边界框和预览图像。",
	"display_name": "创建边界框",
	"inputs": {
		"background": {
			"name": "背景",
			"tooltip": "在画布和预览中用作背景的可选图像。"
		},
		"bboxes": {
			"name": "bboxes",
			"tooltip": "用于初始化画布的边界框、元素或 JSON 字符串。新的上游值会初始化画布；在画布上的编辑优先生效，并会一直保留，直到上游值再次变化。"
		},
		"editor_state": {
			"name": "编辑器状态",
			"tooltip": "绘制边界框并设置每个框的类型、文本、描述、色板。先添加背景元素，最后添加前景元素。"
		},
		"height": {
			"name": "高度",
			"tooltip": "画布及边界框像素网格的高度。"
		},
		"last_incoming": {
			"name": "last_incoming",
			"tooltip": "由画布管理的内部状态：上次用于初始化画布的上游 bboxes 值。留空将在下次运行时根据 bboxes 输入重新初始化画布。"
		},
		"width": {
			"name": "宽度",
			"tooltip": "画布及边界框像素网格的宽度。"
		}
	},
	"outputs": {
		"0": {
...
var CreateCameraInfo = {
	"description": "构建 camera_info。模式 'orbit' 通过偏航/俯仰/距离围绕目标旋转；'look_at' 将相机放置在世界坐标位置。坐标为观察者的世界空间（右手系，Y轴向上）。",
	"display_name": "创建相机信息",
	"inputs": {
		"camera_type": {
			"name": "相机类型",
			"tooltip": "Render Splat 使用的投影方式：透视（有透视缩短）或正交（平行投影）。"
		},
		"fov": {
			"name": "视场角",
			"tooltip": "垂直视场角，单位为度。"
		...
		"mode": {
			"name": "模式",
			"tooltip": "定义相机方式：轨道角度、显式位置，或位置+四元数。"
		},
		"mode_distance": { "name": "距离" },
		"mode_pitch": { "name": "俯仰" },
		"mode_yaw": { "name": "偏航" },
		"roll": {
			"name": "滚转",
			"tooltip": "相机绕视轴的滚转角度，单位为度。"
		},
		"target_x": {
			"name": "目标X",
			"tooltip": "注视点（轨道中心/瞄准点）。在轨道模式下，移动它可平移整个相机。四元数模式下忽略。默认原点。"
		},
		"target_y": { "name": "目标Y" },
		"target_z": { "name": "目标Z" },
		"zoom": {
			"name": "缩放",
			"tooltip": "数码变焦（焦距倍数）。大于1时无需移动相机即可放大。"
		}
	},
	"outputs": { "0": {
		"name": "camera_info",
...
var CreateHookKeyframe = {
	...
	"inputs": {
		"prev_hook_kf": { "name": "前一个约束关键帧" },
		"start_percent": { "name": "开始百分比" },
		"strength_mult": { "name": "强度倍数" }
	},
	"outputs": { "0": { "name": "约束关键帧" } }
};
var CreateHookKeyframesFromFloats = {
	"display_name": "从浮点数创建约束关键帧",
	"inputs": {
		"end_percent": { "name": "结束百分比" },
		"floats_strength": { "name": "浮点强度" },
		"prev_hook_kf": { "name": "前一个约束关键帧" },
		"print_keyframes": { "name": "打印关键帧" },
		"start_percent": { "name": "开始百分比" }
	},
	"outputs": { "0": { "name": "约束关键帧" } }
};
var CreateHookKeyframesInterpolated = {
	...
	"inputs": {
		"end_percent": { "name": "结束百分比" },
		"interpolation": { "name": "插值" },
		"keyframes_count": { "name": "关键帧数量" },
		"prev_hook_kf": { "name": "前一个约束关键帧" },
		"print_keyframes": { "name": "打印关键帧" },
		"start_percent": { "name": "开始百分比" },
		"strength_end": { "name": "结束强度" },
		"strength_start": { "name": "开始强度" }
	},
	"outputs": { "0": { "name": "约束关键帧" } }
};
var CreateHookLora = {
	...
	"inputs": {
		"lora_name": { "name": "LoRA名称" },
		"prev_hooks": { "name": "前一个约束" },
		"strength_clip": { "name": "CLIP强度" },
		"strength_model": { "name": "模型强度" }
	}
};
...
var CropByBBoxes = {
	...
	"inputs": {
		...
		"keep_aspect": {
			"name": "保持宽高比",
			"tooltip": "选择是否拉伸裁剪区域以适应输出尺寸，或用黑色像素填充以保持宽高比。"
		},
		"output_height": {
			"name": "output_height",
	...
	"outputs": { "0": { "tooltip": "所有裁剪区域堆叠为一个图像批量。" } }
};
var CropMask = {
	"display_name": "裁剪遮罩",
	"inputs": {
...
var DA3GeometryToMesh = {
	"description": "将深度图转换为三角化的3D网格。",
	"display_name": "将DA3几何体转换为网格",
	"inputs": {
		"batch_index": {
			"name": "batch_index",
			"tooltip": "要转换的批次中的哪一张图像。每张图像的顶点数量不同，因此批次无法堆叠。"
		},
		"confidence_threshold": {
			"name": "confidence_threshold",
			"tooltip": "排除每张图像归一化置信度低于该值的像素（0 = 保留全部，1 = 仅保留置信度最高的像素）。当几何体包含置信度图时使用（Small/Base模型）。"
		},
		"da3_geometry": { "name": "da3_geometry" },
		"decimation": {
			"name": "decimation",
			"tooltip": "顶点步长。1 = 全分辨率，2 = 一半，依此类推。"
		},
		"discontinuity_threshold": {
			"name": "discontinuity_threshold",
			"tooltip": "如果三角形的3x3深度跨度超过此比例，则丢弃该三角形。0 = 关闭。"
		},
		"texture": {
			"name": "texture",
			"tooltip": "使用源图像作为基础颜色纹理。"
		},
		"use_sky_mask": {
			"name": "use_sky_mask",
			"tooltip": "从网格中排除天空概率像素（sky >= 0.5）。当几何体包含天空图时使用（Mono/Metric模型）。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var DA3Inference = {
	"description": "在图像上运行Depth Anything 3。在多视图模式下，每张图像被视为同一场景的不同视角。",
	"display_name": "运行Depth Anything 3",
	"inputs": {
		"da3_model": { "name": "da3_model" },
		"image": { "name": "image" },
		"mode": {
			"name": "mode",
			"tooltip": "mono：单视图图像（适用于所有模型变体）。\nmultiview：所有图像一起处理以保证几何一致性和相机位姿（仅适用于Small/Base模型）。"
		},
		"resize_method": {
			"name": "resize_method",
			"tooltip": "upper_bound_resize：缩放至最长边=分辨率（限制内存，默认）。\nlower_bound_resize：缩放至最短边=分辨率（在高宽比极端的图像上保留更多细节，但占用更多内存）。"
		},
		"resolution": {
			"name": "resolution",
			"tooltip": "模型运行的分辨率（最长边，14的倍数）。\n较低 = 更快 / 更少显存。\n较高 = 更多细节。\n输出会被上采样回原始大小。"
		}
	},
	"outputs": { "0": {
		"name": "da3_geometry",
		"tooltip": "非归一化张量的字典。\n始终包含的键：depth, image, mode。\n可选键：sky（Mono/Metric），confidence（Small/Base），extrinsics + intrinsics（多视图时）。"
	} }
};
var DA3Render = {
	"description": "从Depth Anything 3几何数据渲染深度图、置信度图或天空mask。",
	"display_name": "渲染Depth Anything 3",
	"inputs": {
		"da3_geometry": { "name": "da3_geometry" },
		"output": {
			"name": "output",
			"tooltip": "- depth：归一化灰度深度图。\n- depth_colored：通过Turbo色图映射的深度。\n- sky_mask：天空概率，范围[0, 1]（仅Mono/Metric模型）。\n- confidence：归一化深度置信度（仅Small/Base模型）。"
		},
...
var DiffControlNetLoader = {
	...
	"inputs": {
		"control_net_name": { "name": "ControlNet名称" },
		"model": { "name": "模型" }
	}
};
...
var DualCFGGuider = {
	...
	"inputs": {
		"cfg_cond2_negative": { "name": "cfg_条件2_负面" },
		"cfg_conds": { "name": "cfg_条件1" },
		"cond1": { "name": "条件1" },
		"cond2": { "name": "条件2" },
		"model": { "name": "模型" },
		"negative": { "name": "负面条件" },
...
var DualCLIPLoader = {
	...
	"inputs": {
		"clip_name1": { "name": "CLIP名称1" },
		"clip_name2": { "name": "CLIP名称2" },
		"device": { "name": "设备" },
		"type": { "name": "类型" }
	}
};
var DualModelGuider = {
	"display_name": "双模型 CFG 指导器",
	"inputs": {
		"cfg": { "name": "cfg" },
		"model": {
			"name": "模型",
			"tooltip": "用于正向（条件）推理的模型。"
		},
		"model_negative": {
			"name": "负向模型",
			"tooltip": "用于负向（无条件）推理的模型。普通 CFG 可用同一模型。"
		},
		"negative": {
			"name": "负向",
			"tooltip": "在负向模型上运行的负向条件。若不连接则为无文本（仅图像）无条件推理。"
		},
		"positive": { "name": "正向" }
	},
	"outputs": { "0": { "tooltip": null } }
...
var EasyCache = {
	"description": "原生 EasyCache 实现。",
	"display_name": "EasyCache",
	"inputs": {
		"end_percent": {
			"name": "结束位置",
			"tooltip": "结束使用 EasyCache 的相对采样步数。"
		},
		"model": {
			"name": "模型",
		...
		"start_percent": {
			"name": "开始位置",
			"tooltip": "开始使用 EasyCache 的相对采样步数。"
		},
		"verbose": {
			"name": "调试信息",
			"tooltip": "是否记录详细信息。"
	...
	"outputs": { "0": { "tooltip": "带有 EasyCache 的模型。" } }
};
var ElevenLabsAudioIsolation = {
	"description": "去除音频中的背景噪音，仅保留人声或语音。",
	"display_name": "ElevenLabs 语音隔离",
	"inputs": { "audio": {
		"name": "音频",
		"tooltip": "需要去除背景噪音的音频。"
	} },
...
var ElevenLabsInstantVoiceClone = {
	...
	"display_name": "ElevenLabs 即时语音克隆",
	"inputs": {
		"files": {
			"name": "音频文件",
			"tooltip": "用于语音克隆的录音。"
...
var ElevenLabsSpeechToSpeech = {
	"description": "将一种声音的语音转换为另一种声音，同时保留原始内容和情感。",
	"display_name": "ElevenLabs 语音转换",
	"inputs": {
		"audio": {
			"name": "源音频",
			"tooltip": "需要转换的源音频。"
		...
		"model_similarity_boost": { "name": "相似度增强" },
		"model_speed": { "name": "速度" },
		"model_style": { "name": "风格" },
		"model_use_speaker_boost": { "name": "扬声器增强" },
		"output_format": {
			"name": "输出格式",
			"tooltip": "音频输出格式。"
		},
		...
		"stability": {
			"name": "稳定性",
			"tooltip": "语音稳定性。较低数值带来更广的情感范围，较高数值则更一致但可能较为单调。"
		},
		"voice": {
			"name": "目标语音",
			"tooltip": "转换的目标语音。可从语音选择器或即时语音克隆连接。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var ElevenLabsSpeechToText = {
	"description": "将音频转录为文本。支持自动语言检测、说话人分离和音频事件标记。",
	"display_name": "ElevenLabs 语音转文本",
	"inputs": {
		"audio": {
			"name": "音频",
			"tooltip": "要转录的音频。"
		...
		"language_code": {
			"name": "语言代码",
			"tooltip": "ISO-639-1 或 ISO-639-3 语言代码（如 'en'、'es'、'fra'）。留空以自动检测。"
		},
		"model": {
			"name": "模型",
			"tooltip": "用于转录的模型。"
		},
		"model_diarization_threshold": { "name": "分离阈值" },
		"model_diarize": { "name": "说话人分离" },
		"model_tag_audio_events": { "name": "音频事件标记" },
		"model_temperature": { "name": "温度" },
		"model_timestamps_granularity": { "name": "时间戳粒度" },
		"num_speakers": {
			"name": "说话人数",
			"tooltip": "要预测的最大说话人数。设置为 0 以自动检测。"
		},
		"seed": {
			"name": "种子",
			"tooltip": "用于结果复现的种子（不保证确定性）。"
		}
	},
	"outputs": {
		"0": {
...
var ElevenLabsTextToDialogue = {
	"description": "从文本生成多说话人对话。每条对话都有独立的文本和声音。",
	"display_name": "ElevenLabs 文本转对话",
	"inputs": {
		"apply_text_normalization": {
			"name": "应用文本规范化",
			"tooltip": "文本规范化模式。'auto' 由系统决定，'on' 总是应用规范化，'off' 跳过规范化。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"inputs": {
			"name": "对话条数",
		...
		"stability": {
			"name": "稳定性",
			"tooltip": "语音稳定性。值越低情感范围越广，值越高语音更一致但可能更单调。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var ElevenLabsTextToSoundEffects = {
	...
	"inputs": {
		...
		"model_duration": { "name": "时长" },
		"model_loop": { "name": "循环" },
		"model_prompt_influence": { "name": "提示影响" },
		"output_format": {
			"name": "输出格式",
			"tooltip": "音频输出格式。"
		},
...
var ElevenLabsTextToSpeech = {
	...
	"inputs": {
		...
		"language_code": {
			"name": "language_code",
			"tooltip": "ISO-639-1 或 ISO-639-3 语言代码（如 'en'、'es'、'fra'）。留空则自动检测。"
		},
		"model": {
			"name": "model",
			"tooltip": "用于文字转语音的模型。"
		...
		"seed": {
			"name": "seed",
			"tooltip": "用于结果可复现的种子（不保证完全确定性）。"
		},
		"stability": {
			"name": "stability",
			"tooltip": "语音稳定性。较低的数值带来更广泛的情感变化，较高的数值则语音更一致但可能单调。"
		},
		"text": {
			"name": "text",
			"tooltip": "要转换为语音的文本。"
		...
		"voice": {
			"name": "voice",
			"tooltip": "用于语音合成的声音。从 Voice Selector 或 Instant Voice Clone 连接。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var ElevenLabsVoiceSelector = {
	"description": "为文字转语音生成选择预设的 ElevenLabs 声音。",
	"display_name": "ElevenLabs 声音选择器",
	"inputs": { "voice": {
		"name": "voice",
		"tooltip": "从预设的 ElevenLabs 声音中选择一个声音。"
	} },
	"outputs": { "0": {
		"name": "voice",
		"tooltip": null
...
var EmptyAudio = {
	...
	"inputs": {
		"channels": {
			"name": "通道",
			"tooltip": "音频通道数（1 为单声道，2 为立体声）。"
		},
		"duration": {
			"name": "长度",
			"tooltip": "空音频片段的持续时间（秒）"
		},
		"sample_rate": {
			"name": "采样率",
			"tooltip": "空音频片段的采样率。"
...
var EmptyChromaRadianceLatentImage = {
	"display_name": "空Latent图像（ChromaRadiance）",
	"inputs": {
		"batch_size": { "name": "批次大小" },
		"height": { "name": "高度" },
		"width": { "name": "宽度" }
...
var EmptyHunyuanLatentVideo = {
	"display_name": "空Latent视频（Hunyuan）",
	"inputs": {
		"batch_size": { "name": "批量大小" },
		"height": { "name": "高度" },
		"length": { "name": "长度" },
...
var EmptyHunyuanVideo15Latent = {
	"display_name": "空Latent视频（Hunyuan1.5）",
	"inputs": {
		"batch_size": { "name": "批次大小" },
		"height": { "name": "高度" },
		"length": { "name": "时长" },
...
var EmptyLatentHunyuan3Dv2 = {
	"display_name": "空Latent图像（Hunyuan3Dv2）",
	"inputs": {
		"batch_size": {
			"name": "批次大小",
			"tooltip": "批次中的Latent数量。"
...
var EmptyQwenImageLayeredLatentImage = {
	"display_name": "空Latent图像（QwenImageLayerd）",
	"inputs": {
		"batch_size": { "name": "批次大小" },
		"height": { "name": "高度" },
		"layers": { "name": "图层" },
...
var ExponentialScheduler = {
	...
	"inputs": {
		"sigma_max": { "name": "最大Sigma" },
		"sigma_min": { "name": "最小Sigma" },
		"steps": { "name": "步数" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var ExtendIntermediateSigmas = {
	"display_name": "插值扩展Sigmas",
	"inputs": {
		"end_at_sigma": { "name": "结束 sigma" },
		"sigmas": { "name": "sigmas" },
		"spacing": { "name": "间距方式" },
...
var File3DToSplat = {
	"description": "将 Splat File3D 解析为高斯 Splat。与“从 Splat 创建 3D 文件”相反。支持格式：PLY、SPLAT、KSPLAT、SPZ。PLY 包含完整球谐信息，其他格式仅有基础颜色。格式会根据文件内容自动检测。",
	"display_name": "获取 Splat",
	"inputs": { "model_3d": {
		"name": "3D模型",
		"tooltip": "高斯 Splat 3D 文件"
	} },
	"outputs": { "0": {
		"name": "splat",
		"tooltip": null
...
var Flux2MaxImageNode = {
	...
	"inputs": {
		...
		"prompt_upsampling": {
			"name": "提示词上采样",
			"tooltip": "选择启用提示词采样，启用后会自动修改提示词生成更具创意的内容。"
		},
		"seed": {
			"name": "随机种",
			"tooltip": "创建噪波的随机种"
...
var FluxDisableGuidance = {
	"description": "此节点完全禁用Flux和类似Flux模型上的引导嵌入",
	"display_name": "Flux禁用指导",
	"inputs": { "conditioning": { "name": "条件" } },
	"outputs": { "0": { "tooltip": null } }
};
var FluxEraseNode = {
	"description": "从图像中移除被遮罩的对象并重建背景。用遮罩涂抹你想要擦除的部分。",
	"display_name": "Flux 擦除图像",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"dilate_pixels": {
			"name": "膨胀像素",
			"tooltip": "扩展遮罩边界，以确保对象边缘被干净覆盖。"
		},
		"image": { "name": "图像" },
		"mask": {
			"name": "遮罩",
			"tooltip": "白色区域将被移除；黑色区域将被保留。"
		},
		"seed": {
			"name": "seed",
			"tooltip": "用于生成噪声的随机种子。"
...
var FluxKVCache = {
	...
	"outputs": { "0": { "tooltip": "已启用 KV 缓存的模型。" } }
};
var FluxKontextImageScale = {
	"description": "将图像调整为更适合 Flux Kontext 的尺寸。",
	"display_name": "图像缩放为FluxKontext",
	"inputs": { "image": { "name": "图像" } },
	"outputs": { "0": { "tooltip": null } }
};
var FluxKontextMaxImageNode = {
	"description": "使用Flux.1 Kontext [max]通过API基于提示词和宽高比编辑图像。",
	"display_name": "Flux.1 Kontext [max] 图像",
	"inputs": {
		"aspect_ratio": {
			"name": "宽高比",
		...
		"input_image": { "name": "输入图像" },
		"prompt": {
			"name": "提示词",
			"tooltip": "图像生成的提示词 - 指定编辑内容和方式。"
		},
		"prompt_upsampling": {
			"name": "提示词上采样",
			"tooltip": "是否对提示词执行上采样。如果启用，会自动修改提示词以获得更具创意的生成结果，但结果具有不确定性（相同种子不会产生完全相同的结果）。"
		},
		"seed": {
			"name": "种子",
			"tooltip": "用于创建噪声的随机种子。"
...
var FluxKontextMultiReferenceLatentMethod = {
	...
	"inputs": {
		"conditioning": { "name": "条件化" },
		"reference_latents_method": { "name": "参考潜在方法" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var FluxKontextProImageNode = {
	"description": "使用Flux.1 Kontext [pro]通过API基于提示词和宽高比编辑图像。",
	"display_name": "Flux.1 Kontext [pro] 图像",
	"inputs": {
		"aspect_ratio": {
			"name": "宽高比",
			"tooltip": "图像宽高比；必须在1:4到4:1之间。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"guidance": {
			"name": "引导强度",
...
var FluxVTONode = {
	"description": "虚拟试穿：为人物穿上提供的服装。",
	"display_name": "Flux 虚拟试穿",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"garment": {
			"name": "服装",
...
var FrameInterpolationModelLoader = {
	"display_name": "加载帧插值模型",
	"inputs": { "model_name": {
		"name": "模型名称",
		"tooltip": "选择要加载的帧插值模型。模型文件需放置在 'frame_interpolation' 文件夹中。"
	} },
	"outputs": { "0": { "tooltip": null } }
};
var FreSca = {
...
var GLIGENLoader = {
	...
	"inputs": { "gligen_name": { "name": "gligen名称" } }
};
var GLIGENTextBoxApply = {
	...
	"inputs": {
		"clip": { "name": "CLIP" },
		"conditioning_to": { "name": "条件到" },
		"gligen_textbox_model": { "name": "GLIGEN文本框模型" },
		"height": { "name": "高度" },
		"text": { "name": "文本" },
		"width": { "name": "宽度" },
		"x": { "name": "x" },
...
var GLSLShader = {
	"description": "对图像应用 GLSL ES 片段着色器。u_resolution（vec2）始终可用。",
	"display_name": "GLSL 着色器",
	"inputs": {
		"bools": {
			"name": "布尔值",
			"tooltip": "布尔值在着色器代码中以 u_bool0-9（bool）形式提供"
		},
		"curves": {
			"name": "曲线",
			"tooltip": "曲线在着色器代码中以 u_curve0-3（sampler2D，1D LUT）形式提供。可通过 texture(u_curve0, vec2(x, 0.5)).r 采样"
		},
		"floats": {
			"name": "floats",
			"tooltip": "浮点数在着色器代码中以 u_float0-4 形式可用"
		},
		"fragment_shader": {
			"name": "fragment_shader",
			"tooltip": "GLSL 片段着色器源码（兼容 GLSL ES 3.00 / WebGL 2.0）"
		},
		"images": {
			"name": "images",
			"tooltip": "图像在着色器代码中以 u_image0-4（sampler2D）形式可用"
		},
		"ints": {
			"name": "ints",
			"tooltip": "整数在着色器代码中以 u_int0-4 形式可用"
		},
		"size_mode": {
			"name": "size_mode",
			"tooltip": "输出尺寸：'from_input' 使用第一个输入图像尺寸，'custom' 允许手动设置尺寸"
		}
	},
	"outputs": {
		"0": {
			"name": "IMAGE0",
			"tooltip": "在着色器代码中通过 layout(location = 0) out vec4 fragColor0 可用"
		},
		"1": {
			"name": "IMAGE1",
			"tooltip": "在着色器代码中通过 layout(location = 1) out vec4 fragColor1 可用"
		},
		"2": {
			"name": "IMAGE2",
			"tooltip": "在着色器代码中通过 layout(location = 2) out vec4 fragColor2 可用"
		},
		"3": {
			"name": "IMAGE3",
			"tooltip": "在着色器代码中通过 layout(location = 3) out vec4 fragColor3 可用"
		}
	}
};
var GeminiImage2Node = {
	"description": "通过Google API编辑图像。",
	"display_name": "Nano Banana Pro（Google Gemini 图像）",
	"inputs": {
		"aspect_ratio": {
			"name": "宽高比",
			"tooltip": "自动：会自动匹配输入图像的宽高比，或如果没有提供图像，则生成1:1的正方形。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"files": {
			"name": "文件",
			"tooltip": "模型上下文文件。接受来自 Gemini输入文件 节点的输入。"
		},
		"images": {
			"name": "图像",
			"tooltip": "参考图像。如果要使用多个参考图，使用 图像批次 节点（最多14张）。"
		},
		"model": { "name": "模型" },
		"prompt": {
			"name": "提示词",
			"tooltip": "使用文本描述要生成的图像或要应用的编辑的内容。包括模型应该遵循的任何约束、样式或细节。"
		},
		"resolution": {
			"name": "分辨率",
			"tooltip": "目标分辨率。2K/4K 会使用 Gemini 放大生成。"
		},
		"response_modalities": {
			"name": "响应模态",
			"tooltip": "选择 图像 仅生成图像，选择 图像+文本 会生成图像和文本。"
		},
		"seed": {
			"name": "随机种",
			"tooltip": "为特定值时，模型将尽可能提供相同的结果。但不保证确定一致的输出。此外，即使使用相同的种子值，更改模型或参数设置（例如温度）也会导致结果变化。默认使用随机值。"
		},
		"system_prompt": {
			"name": "系统提示词",
			"tooltip": "决定 AI 行为的基本指令。"
		}
	},
	"outputs": {
		"0": { "tooltip": null },
...
var GeminiImageNode = {
	"description": "通过Google API同步编辑图像。",
	"display_name": "Nano Banana （Google Gemini 图像）",
	"inputs": {
		"aspect_ratio": {
			"name": "宽高比",
			"tooltip": "默认匹配输入图像的输出尺寸，否则生成1:1正方形。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"files": {
			"name": "文件",
			"tooltip": "可选文件，用作模型的上下文。接受来自Gemini生成内容输入文件节点的输入。"
		},
		"images": {
			"name": "图像",
			"tooltip": "可选图像，用作模型的上下文。要包含多个图像，可以使用批量图像节点。"
		},
		"model": {
			"name": "模型",
			"tooltip": "用于生成响应的Gemini模型。"
		},
		"prompt": {
			"name": "提示词",
			"tooltip": "用于生成的文本提示词"
		...
		"response_modalities": {
			"name": "回复模态",
			"tooltip": "选择“图像”仅输出图像，选择“图像+文本”输出图像和文本。"
		},
		"seed": {
			"name": "种子",
			"tooltip": "当种子固定为特定值时，模型会尽力为重复请求提供相同的响应。不能保证确定性输出。此外，更改模型或参数设置（如温度）即使使用相同的种子值也可能导致响应变化。默认使用随机种子值。"
		},
		"system_prompt": {
			"name": "系统提示词",
			"tooltip": "决定 AI 行为的基本指令。"
...
var GeminiInputFiles = {
	"description": "加载并准备输入文件，作为 Gemini LLM 节点的输入。文件将在生成响应时由 Gemini 模型读取。文本文件的内容计入令牌限制。🛈 提示：可与其他 Gemini 输入文件节点链式连接。",
	"display_name": "Gemini 输入文件",
	"inputs": {
		"GEMINI_INPUT_FILES": {
			"name": "Gemini 输入文件",
			"tooltip": "与此节点加载的文件批量组合的可选附加文件。允许链式连接输入文件，以便单个消息可包含多个输入文件。"
		},
		"file": {
			"name": "文件",
			"tooltip": "作为模型上下文包含的输入文件。目前仅接受文本 (.txt) 和 PDF (.pdf) 文件。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var GeminiNanoBanana2 = {
	...
	"outputs": {
		...
		"2": {
			"name": "thought_image",
			"tooltip": "模型思考过程中的第一张图像。仅在thinking_level为HIGH且模态为IMAGE+TEXT时可用。"
		}
	}
};
var GeminiNanoBanana2V2 = {
	...
	"inputs": {
		...
		"seed": {
			"name": "seed",
			"tooltip": "当 seed 固定为特定值时，模型会尽力为重复请求提供相同的响应，但不保证输出完全一致。此外，即使使用相同的 seed，修改模型或参数设置（如 temperature）也可能导致响应变化。默认情况下，使用随机 seed 值。"
		},
		"system_prompt": {
			"name": "system_prompt",
			"tooltip": "决定 AI 行为的基础性指令。"
		...
		"temperature": {
			"name": "temperature",
			"tooltip": "控制生成的随机性。数值越低越集中/确定。"
		},
		"top_p": {
			"name": "top_p",
			"tooltip": "Nucleus 采样阈值。数值越低越集中，越高越多样。"
		}
	},
	"outputs": {
		"0": { "tooltip": null },
		...
		"2": {
			"name": "thought_image",
			"tooltip": "模型思考过程中的第一张图像。仅在 thinking_level 设为 HIGH 且 response_modalities 为 IMAGE+TEXT 时可用。"
		}
	}
};
var GeminiNode = {
	...
	"inputs": {
		"audio": {
			"name": "音频",
			"tooltip": "用作模型上下文的可选音频。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"files": {
			"name": "文件",
			"tooltip": "用作模型上下文的可选文件。接受来自 Gemini 生成内容输入文件节点的输入。"
		},
		"images": {
			"name": "图像",
			"tooltip": "用作模型上下文的可选图像。要包含多个图像，可使用批处理图像节点。"
		},
		"model": {
			"name": "模型",
			"tooltip": "用于生成响应的 Gemini 模型。"
		},
		"prompt": {
			"name": "提示",
			"tooltip": "模型的文本输入，用于生成响应。您可以包含详细的指令、问题或模型上下文。"
		},
		"seed": {
			"name": "种子",
			"tooltip": "当种子固定为特定值时，模型会尽力为重复请求提供相同的响应。不保证确定性输出。此外，更改模型或参数设置（如温度）即使使用相同的种子值也可能导致响应变化。默认使用随机种子值。"
		},
		"system_prompt": {
			"name": "系统提示词",
			"tooltip": "决定 AI 行为的基本指令。"
		...
		"video": {
			"name": "视频",
			"tooltip": "用作模型上下文的可选视频。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var GeminiNodeV2 = {
	"description": "使用 Google 的 Gemini 模型生成文本回复。可提供文本提示，并可选提供一张或多张图片、音频片段、视频或文件作为多模态上下文。",
	"display_name": "Google Gemini",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"model": {
			"name": "model",
			"tooltip": "用于生成回复的 Gemini 模型。"
		},
		...
		"prompt": {
			"name": "prompt",
			"tooltip": "模型的文本输入。可包含详细指令、问题或上下文。"
		},
		"seed": {
			"name": "seed",
			"tooltip": "采样用的种子。设为 0 表示随机种子。无法保证输出确定性。"
		},
		"system_prompt": {
			"name": "system_prompt",
			"tooltip": "决定模型行为的基础指令。"
...
var GeminiVideoOmni = {
	"description": "使用 Google 的 Gemini Omni Flash 模型，通过文本提示生成带音频的视频。可选提供参考图像和/或视频以引导或编辑结果。在提示中直接描述期望时长（3-10 秒）和宽高比（16:9 或 9:16）。",
	"display_name": "Google Gemini Omni（视频）",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"model": {
			"name": "model",
			"tooltip": "用于生成视频的 Gemini 视频模型。"
		},
...
var GenerateTracks = {
	...
	"inputs": {
		"bezier": {
			"name": "贝塞尔曲线",
			"tooltip": "贝塞尔曲线路径，使用中点作为控制点。"
		},
		"end_x": {
			"name": "结束点X",
			"tooltip": "结束点的规格化X坐标（0-1）。"
		},
		"end_y": {
			"name": "结束点Y",
			"tooltip": "结束点的规格化Y坐标（0-1）。"
		},
		"height": { "name": "高度" },
		"interpolation": {
			"name": "插值",
			"tooltip": "控制轨迹沿着路径移动的时间/速度。"
		},
		"mid_x": {
			"name": "中间点Y",
			"tooltip": "中间点的规格化Y坐标（0-1）。"
		},
		"mid_y": {
			"name": "中间点Y",
			"tooltip": "中间点的规格化Y坐标（0-1）。"
		...
		"start_x": {
			"name": "起始点X",
			"tooltip": "起始点的规格化X坐标（0-1）。"
		},
		"start_y": {
			"name": "起始点Y",
			"tooltip": "起始点的规格化Y坐标（0-1）。"
		},
		"track_mask": {
			"name": "轨道遮罩",
			"tooltip": "遮罩，用于标记可见帧"
...
var GetICLoRAParameters = {
	"description": "从已加载 LoRA 模型的 safetensors 元数据中提取 IC-LoRA 参数，并将其输出用于 LTXVAddGuide（例如 reference_downscale_factor）。",
	"display_name": "获取 IC-LoRA 参数",
	"inputs": { "iclora_model": {
		"name": "iclora_model",
		"tooltip": "来自 LoRA 加载器的直接输出，用于提取特定 IC-LoRA 的元数据。"
	} },
	"outputs": { "0": {
		"name": "iclora_parameters",
		"tooltip": "从 LoRA 元数据中提取的 IC-LoRA 参数（例如 reference_downscale_factor）。如果 LoRA 需要特殊的引导处理，请连接到 LTXVAddGuide。"
	} }
};
var GetImageSize = {
	"description": "返回图像的宽度和高度，并原样传递图像。",
	"display_name": "获取图像尺寸",
	"inputs": { "image": { "name": "图像" } },
	"outputs": {
		"0": {
...
var GetSplatCount = {
	"description": "返回批处理中所有 splat 的总数。",
	"display_name": "获取 Splat 数量",
...
var GetVideoComponents = {
	"description": "提取视频中的所有元素：帧、音频和帧率。",
	"display_name": "获取视频元素",
	"inputs": { "video": {
		"name": "视频",
		"tooltip": "要提取元素的视频。"
...
var GrokImageEditNodeV2 = {
	...
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"model": { "name": "模型" },
		"model_aspect_ratio": { "name": "宽高比" },
		"model_number_of_images": { "name": "图像数量" },
		"model_resolution": { "name": "分辨率" },
		"prompt": {
			"name": "提示词",
			"tooltip": "用于生成图像的文本提示"
...
var GrokVideoEditNode = {
	"description": "根据文本提示编辑现有视频。",
	"display_name": "Grok 视频编辑",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"model": { "name": "model" },
		"prompt": {
		...
		"video": {
			"name": "video",
			"tooltip": "最大支持时长为 8.7 秒，文件大小为 50MB。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var GrokVideoExtendNode = {
	"description": "根据文本提示无缝扩展现有视频。",
	"display_name": "Grok 视频扩展",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"model": {
			"name": "模型",
		...
		"prompt": {
			"name": "提示词",
			"tooltip": "描述视频接下来内容的文本。"
		},
		"seed": {
			"name": "种子",
			"tooltip": "用于决定节点是否重新运行的种子；无论种子如何，实际结果都是不确定的。"
		},
		"video": {
			"name": "视频",
			"tooltip": "要扩展的视频源。MP4 格式，2-15 秒。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var GrokVideoNode = {
	"description": "根据提示或图像生成视频",
	...
	"inputs": {
		...
		"image": {
			"name": "image",
			"tooltip": "grok-imagine-video 的可选起始图像。grok-imagine-video-1.5 必须提供。"
		},
		"model": {
			"name": "model",
			"tooltip": "grok-imagine-video-1.5 当前始终需要输入图像。"
		},
		"prompt": {
			"name": "prompt",
			"tooltip": "所需视频的文本描述。"
...
var GrokVideoReferenceNode = {
	"description": "根据参考图像的风格和内容生成视频。",
	"display_name": "Grok 参考生成视频",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"model": {
			"name": "模型",
...
var GrowMask = {
	...
	"inputs": {
		"expand": { "name": "扩展" },
		"mask": { "name": "遮罩" },
		"tapered_corners": { "name": "倒角" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var HappyHorseImageToVideoApi = {
	"description": "使用 HappyHorse 模型根据首帧图像生成视频。",
	"display_name": "HappyHorse 图像转视频",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"first_frame": {
			"name": "first_frame",
			"tooltip": "首帧图像。输出视频的宽高比将根据该图像确定。"
		},
		"model": { "name": "model" },
		"model_duration": { "name": "时长" },
		"model_prompt": { "name": "提示词" },
...
var HappyHorseReferenceVideoApi = {
	"description": "使用 HappyHorse 模型根据参考素材生成包含人物或物体的视频。支持单角色表演和多角色互动。",
	"display_name": "HappyHorse 参考转视频",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"model": { "name": "model" },
		"model_duration": { "name": "时长" },
...
var HappyHorseTextToVideoApi = {
	"description": "使用 HappyHorse 模型根据文本提示生成视频。",
	"display_name": "HappyHorse 文本转视频",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"model": { "name": "model" },
		"model_duration": { "name": "时长" },
...
var HappyHorseVideoEditApi = {
	"description": "使用 HappyHorse 模型通过文本指令或参考图像编辑视频。输出时长为 3-15 秒，并与输入视频一致；超过 15 秒的输入将被截断。",
...
var HeyGenAvatarVideoNode = {
	"description": "通过 HeyGen 虚拟人生成讲解者视频。包含 HeyGen 最受欢迎的公开虚拟人；可通过 look ID 覆盖选择任意形象。",
	"display_name": "HeyGen 虚拟人视频",
	"inputs": {
		"aspect_ratio": {
			"name": "宽高比",
			"tooltip": "输出宽高比。'auto' 跟随虚拟人源视频比例。"
		},
		"background_color": {
			"name": "背景颜色",
			"tooltip": "可选纯色背景，使用十六进制代码（如 '#00ff00'）。留空则使用虚拟人自带背景。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"custom_avatar_id": {
			"name": "自定义虚拟人 ID",
			"tooltip": "可选的 HeyGen 虚拟人 look ID。设置后将覆盖上方选择的虚拟人。可使用 HeyGen 3000+ 公开形象（或你的私人虚拟人）。"
		},
		"engine": {
			"name": "引擎",
			"tooltip": "渲染引擎；每个选项仅显示支持该引擎的虚拟人。'auto' 提供所有虚拟人并自动选择最佳引擎（优先 Avatar IV）。Avatar V 画质最高，Avatar III 最经济。"
		},
		"engine_avatar": { "name": "虚拟人" },
		"resolution": {
			"name": "分辨率",
			"tooltip": "输出视频分辨率。"
		},
		"seed": {
			"name": "种子",
			"tooltip": "不会发送给 HeyGen；更改后可强制重新生成。"
		},
		"speech": {
			"name": "语音",
			"tooltip": "通过文本脚本（HeyGen 文本转语音）或自定义音频驱动虚拟人。"
		},
		"speech_custom_voice_id": { "name": "自定义语音 ID" },
		"speech_text": { "name": "文本" },
		"speech_voice": { "name": "语音" },
		"speech_voice_speed": { "name": "语速" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var HeyGenCreateAvatarNode = {
	"description": "通过人物照片或文本提示（生成角色）创建你自己的可复用 HeyGen 虚拟人。将生成的 avatar_id 输入到 HeyGen 虚拟人视频的 custom_avatar_id，并保存该 ID 以便在后续工作流中复用。",
	"display_name": "HeyGen 创建虚拟人",
	"inputs": {
		"source": {
			"name": "来源",
			"tooltip": "通过文本提示生成新角色，或通过连接的人像照片创建虚拟人。"
		},
		"source_prompt": { "name": "提示词" }
	},
	"outputs": {
		"0": {
			"name": "avatar_id",
			"tooltip": "虚拟人 look ID。传递给 HeyGen 虚拟人视频的 custom_avatar_id；保存以便后续复用。"
		},
		"1": {
			"name": "预览",
			"tooltip": null
...
var HeyGenTalkingPhotoNode = {
	"description": "将任意人物图像动画化为口型同步的说话视频（HeyGen Avatar IV）。可通过文本脚本或自定义音频驱动。",
	"display_name": "HeyGen 说话照片",
	"inputs": {
		"aspect_ratio": {
			"name": "宽高比",
			"tooltip": "输出宽高比。'auto' 跟随输入图像。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"expressiveness": {
			"name": "表现力",
			"tooltip": "动画面部和手势的表现力程度。"
		},
		"image": {
			"name": "图像",
			"tooltip": "要动画化的人物图像。若大于2K将自动缩小。"
		},
		"resolution": {
			"name": "分辨率",
			"tooltip": "输出视频分辨率。"
		...
		"seed": {
			"name": "种子",
			"tooltip": "不会发送到 HeyGen；更改后可强制重新生成。"
		},
		"speech": {
			"name": "语音",
			"tooltip": "通过文本脚本（HeyGen 文本转语音）或自定义音频驱动头像。"
		},
		"speech_custom_voice_id": { "name": "自定义语音ID" },
		"speech_text": { "name": "文本" },
		"speech_voice": { "name": "语音" },
		"speech_voice_speed": { "name": "语速" }
	},
...
var HeyGenTextToSpeechNode = {
	"description": "使用 HeyGen 的 Starfish TTS 引擎将文本生成语音音频。包含 HeyGen 最受欢迎的17种语言语音。",
	"display_name": "HeyGen 文本转语音",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"custom_voice_id": {
			"name": "自定义语音ID",
			"tooltip": "可选的 HeyGen 语音ID。设置后将覆盖上方选择的语音。所选语音需支持 Starfish 引擎。"
		},
		"seed": {
			"name": "种子",
			"tooltip": "不会发送到 HeyGen；更改后可强制重新生成。"
		...
		"ssml": {
			"name": "SSML",
			"tooltip": "将文本作为 SSML 标记处理（用于停顿、强调和发音控制）。"
		},
		"text": {
			"name": "文本",
			"tooltip": "要合成的文本（最多5000字符）。生成的语音时长需至少1秒。"
		},
		"voice": {
			"name": "语音",
			"tooltip": "要使用的语音（精选自 HeyGen 最受欢迎的 Starfish 兼容语音）。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var HeyGenVideoTranslateNode = {
	"description": "将带有语音的视频翻译为另一种语言。克隆原说话人的声音，并重新动画嘴型以匹配翻译后的语音。",
	"display_name": "HeyGen 视频翻译",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"mode": {
			"name": "模式",
			"tooltip": "“speed”速度更快；“precision”可生成更高质量的唇形同步，但价格翻倍。"
		},
		"output_language": {
			"name": "目标语言",
			"tooltip": "翻译后视频的目标语言。"
		...
		"seed": {
			"name": "随机种子",
			"tooltip": "不会发送给 HeyGen；更改此项可强制重新运行。"
		},
		"speaker_count": {
			"name": "说话人数",
			"tooltip": "视频中的说话人数。0 = 自动检测。"
		},
		"translate_audio_only": {
			"name": "仅翻译音频",
			"tooltip": "只替换音轨，保留原始嘴型（无唇形同步）。"
		},
		"video": {
			"name": "视频",
			"tooltip": "需要翻译语音的视频。"
...
var Hunyuan3Dv2Conditioning = {
	...
	"inputs": { "clip_vision_output": { "name": "clip视觉输出" } },
	"outputs": {
		"0": {
			"name": "正向",
			"tooltip": null
...
var HunyuanImageToVideo = {
	"display_name": "图像到视频（Hunyuan）",
	"inputs": {
		"batch_size": { "name": "批量大小" },
		"guidance_type": { "name": "指导类型" },
		"height": { "name": "高度" },
		"length": { "name": "长度" },
		"positive": { "name": "正向" },
		"start_image": { "name": "起始图像" },
...
var HunyuanVideo15ImageToVideo = {
	"display_name": "图像到视频（Hunyuan Video 15 ）",
	"inputs": {
		"batch_size": { "name": "批次大小" },
		"clip_vision_output": { "name": "CLIP视觉输出" },
		"height": { "name": "高度" },
		"length": { "name": "帧数" },
		"negative": { "name": "负面条件" },
		"positive": { "name": "正面条件" },
...
var HunyuanVideo15LatentUpscaleWithModel = {
	"display_name": "Hunyuan Video 15 Latent 使用模型放大",
	"inputs": {
		"crop": { "name": "裁剪" },
		"height": { "name": "高度" },
		"model": { "name": "模型" },
...
var HunyuanVideo15SuperResolution = {
	"display_name": "Hunyuan Video 15超分辨率",
	"inputs": {
		"clip_vision_output": { "name": "CLIP视觉输出" },
		"latent": { "name": "latent" },
		"negative": { "name": "负面条件" },
		"noise_augmentation": { "name": "噪波增强" },
		"positive": { "name": "正面条件" },
		"start_image": { "name": "图像" },
		"vae": { "name": "vae" }
	},
...
var HypernetworkLoader = {
	"display_name": "超网络加载器",
	"inputs": {
		"hypernetwork_name": { "name": "HyperNetwork名称" },
		"model": { "name": "模型" },
		"strength": { "name": "强度" }
	},
	"outputs": { "0": { "tooltip": null } }
...
var IdeogramV4 = {
	"description": "使用 Ideogram 4.0 模型根据文本提示生成图像。",
...
var ImageCompositeMasked = {
	...
	"inputs": {
		"destination": { "name": "目标图像" },
		"mask": { "name": "遮罩" },
		"resize_source": { "name": "缩放来源图像" },
		"source": { "name": "来源图像" },
		"x": { "name": "x" },
		"y": { "name": "y" }
	},
...
var ImageCropV2 = {
	"description": "将图像裁剪为指定尺寸。",
	"display_name": "图像裁剪",
	"inputs": {
		"crop_region": { "name": "裁剪区域" },
		"height": {},
		"image": { "name": "图像" },
		"width": {},
		"x": {},
...
var ImageDeduplication = {
	...
	"inputs": {
		...
		"similarity_threshold": {
			"name": "相似度阈值",
			"tooltip": "相似阈值（0-1）。越高表示越相似。超过该值的图像则认定为重复图像。"
		}
	},
	"outputs": { "0": {
		"name": "图像",
...
var ImageOnlyCheckpointLoader = {
	...
	"inputs": { "ckpt_name": { "name": "Checkpoint名称" } }
};
...
var ImageScaleToTotalPixels = {
	...
	"inputs": {
		"image": { "name": "图像" },
		"megapixels": { "name": "像素数量" },
		"resolution_steps": { "name": "分辨率步数" },
		"upscale_method": { "name": "缩放算法" }
	},
	"outputs": { "0": { "tooltip": null } }
};
...
var ImageStitch = {
	...
	"inputs": {
		...
		"image1": { "name": "图像1" },
		"image2": { "name": "图像2" },
		"match_image_size": { "name": "匹配图像尺寸" },
		"spacing_color": { "name": "间距颜色" },
		"spacing_width": { "name": "间距宽度" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var ImageToMask = {
...
var JoinAudioChannels = {
	"description": "将左声道和右声道的单声道音频合成为立体声音频。",
	"display_name": "音频通道合并",
	"inputs": {
...
var KSampler = {
	...
	"inputs": {
		...
		"control_after_generate": { "name": "生成后的控制" },
		"denoise": {
			"name": "降噪",
...
var KSamplerAdvanced = {
	...
	"inputs": {
		...
		"cfg": { "name": "cfg" },
		"control_after_generate": { "name": "生成后的控制" },
		"end_at_step": { "name": "结束步数" },
		"latent_image": { "name": "Latent图像" },
		"model": { "name": "模型" },
		"negative": { "name": "负面条件" },
		"noise_seed": { "name": "随机种" },
		"positive": { "name": "正面条件" },
		"return_with_leftover_noise": { "name": "返回剩余噪波" },
		"sampler_name": { "name": "采样器名称" },
		"scheduler": { "name": "调度器" },
		"start_at_step": { "name": "开始步数" },
		"steps": { "name": "步数" }
	}
};
var KSamplerSelect = {
	"display_name": "K采样器选择",
	"inputs": { "sampler_name": { "name": "采样器名称" } },
	"outputs": { "0": { "tooltip": null } }
};
var Kandinsky5ImageToVideo = {
	"display_name": "图像到视频（Kandinsky5）",
	"inputs": {
		"batch_size": { "name": "批次大小" },
		"height": { "name": "高度  " },
		"length": { "name": "时长" },
	...
	"outputs": {
		...
		"3": {
			"name": "条件Latent",
			"tooltip": "纯净编码的 Latent，用于替代纯噪波 Latent 输出。"
		}
	}
};
var KarrasScheduler = {
...
var KlingDualCharacterVideoEffectNode = {
	"description": "根据 效果 在生成视频时实现不同的特效。第一张图片将被放置在合成画面的左侧，第二张图片在右侧。",
	"display_name": "Kling 双角色视频特效",
	"inputs": {
		"duration": { "name": "时长" },
		"effect_scene": { "name": "效果" },
		"image_left": {
			"name": "左侧图像",
			"tooltip": "左侧图片"
		},
...
var KlingMotionControl = {
	"display_name": "Kling 动作控制",
	"inputs": {
		"character_orientation": {
			"name": "参考方向",
			"tooltip": "控制角色的朝向/方向。\nv视频：动作，表情，相机移动和方向遵循运动参考视频（其他细节通过提示）。\n图像：运动和表情仍然遵循运动参考视频，但角色方向匹配参考图像（通过提示相机/其他细节）"
		},
		"keep_original_sound": { "name": "保留原音频" },
		"mode": { "name": "模式" },
		"model": { "name": "模型" },
		"prompt": { "name": "提示词" },
		"reference_image": { "name": "参考图像" },
		"reference_video": {
			"name": "参考视频",
			"tooltip": "动作或表情的参考视频。\n持续时间取决于参考类型：\n图像：3-10 秒\n视频：3-30 秒"
		}
...
var KlingOmniProEditVideoNode = {
	...
	"inputs": {
		...
		"prompt": {
			"name": "提示词",
			"tooltip": "描述视频内容的文本提示词。可以同时包括正面负面描述。"
		},
		"reference_images": {
			"name": "参考图像",
			"tooltip": "最多 4 个参考图像"
...
var KlingOmniProFirstLastFrameNode = {
	...
	"inputs": {
		...
		"end_frame": {
			"name": "结束帧",
			"tooltip": "可选的视频结束帧。不能和“参考图像”同时使用。"
		},
		"first_frame": { "name": "起始帧" },
		"generate_audio": {
			"name": "生成音频",
...
var KlingOmniProImageNode = {
	"description": "使用 Kling 最新模型生成或编辑图像。",
	"display_name": "Kling Omni 图像 (Pro)",
	"inputs": {
		"aspect_ratio": { "name": "宽高比" },
		"control_after_generate": { "name": "生成后控制" },
		"model_name": { "name": "模型" },
		"prompt": {
			"name": "提示词",
			"tooltip": "描述图像内容的文本提示词。可以同时包括正面负面描述。"
		},
		"reference_images": {
			"name": "参考图像",
			"tooltip": "最多 10 个参考图像"
...
var KlingOmniProImageToVideoNode = {
	"description": "使用 Kling 最新模型基于最多 7 个参考图生成视频。",
	"display_name": "Kling Omni 图像到视频 (Pro)",
	"inputs": {
		"aspect_ratio": { "name": "宽高比" },
		"control_after_generate": { "name": "生成后控制" },
		"duration": { "name": "时长" },
...
var KlingOmniProTextToVideoNode = {
	"description": "使用 Kling 最新模型和提示词生成视频。",
	"display_name": "Kling Omni 文本到视频 (Pro)",
	"inputs": {
		"aspect_ratio": { "name": "宽高比" },
		"control_after_generate": { "name": "生成后控制" },
		"duration": { "name": "时长" },
...
var KlingOmniProVideoToVideoNode = {
	"description": "使用 Kling 最新模型基于最多 4 个参考图编辑视频。",
	"display_name": "Kling Omni 视频到视频 (Pro)",
	"inputs": {
		"aspect_ratio": { "name": "宽高比" },
		"control_after_generate": { "name": "生成后控制" },
		"duration": { "name": "时长" },
...
var KlingSingleImageVideoEffectNode = {
	"description": "根据 效果 在生成视频时实现不同的特效。",
...
var KlingVideoNode = {
	...
	"inputs": {
		...
		"generate_audio": {
			"name": "生成音频",
			"tooltip": "“kling-3.0-turbo”始终生成原生音频，因此音频开关将被忽略。"
		},
		"model": {
			"name": "模型",
			"tooltip": "模型和生成设置。"
		...
		"multi_shot_duration": { "name": "时长" },
		"multi_shot_negative_prompt": { "name": "反向提示词" },
		"multi_shot_prompt": { "name": "提示词" },
		"seed": {
			"name": "seed",
			"tooltip": "Seed 控制节点是否重新运行；无论 seed 如何，结果都是非确定性的。"
		},
...
var Krea2ImageNode = {
	"description": "通过 Krea 2 生成图像 —— 选择 Medium（表现性插画）或 Large（表现性写实）。支持可选情绪板和最多 10 个串联图像风格参考。",
	"display_name": "Krea 2 图像",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"model": {
			"name": "模型",
			"tooltip": "Krea 2 Medium 适合表现性插画；Krea 2 Large 适合表现性写实。"
		},
		"model_aspect_ratio": { "name": "宽高比" },
		"model_creativity": { "name": "创造力" },
		"model_moodboard_id": { "name": "情绪板 ID" },
		"model_moodboard_strength": { "name": "情绪板强度" },
		"model_resolution": { "name": "分辨率" },
		"prompt": {
			"name": "提示词",
			"tooltip": "用于生成图像的文本提示。"
...
var Krea2StyleReferenceNode = {
	"description": "为 Krea 2 生成添加图像风格参考。可串联多个 Krea 2 风格参考节点（最多 10 个），并将最终的 `style_reference` 输出连接到 Krea 2 图像。每张图像会上传到 ComfyAPI 存储并以 URL 形式传递。",
	"display_name": "Krea 2 风格参考",
	"inputs": {
		"image": {
			"name": "参考图像",
			"tooltip": "其风格影响生成结果的参考图像。"
		},
		"strength": {
			"name": "参考强度",
			"tooltip": "参考强度；负值会反转风格影响。"
		},
		"style_reference": {
			"name": "风格参考",
			"tooltip": "可选的风格参考链；此节点会追加一个。"
		}
	},
	"outputs": { "0": {
		"name": "风格参考",
...
var LTXAVTextEncoderLoader = {
	"description": "[Recipes]\n\nltxav: gemma 3 12B",
	"display_name": "LTXV音频文本编码器加载器",
	"inputs": {
		"ckpt_name": { "name": "ckpt名称" },
		"device": { "name": "设备" },
		"text_encoder": { "name": "文本编码器" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var LTXVAddGuide = {
	...
	"inputs": {
		"attention_mask": {
			"name": "attention_mask",
			"tooltip": "可选的像素空间区域 mask。通过自注意力机制并乘以强度，控制每个区域的条件影响。"
		},
		"frame_idx": {
			"name": "帧索引",
		...
		"iclora_parameters": {
			"name": "iclora_parameters",
			"tooltip": "来自 Get IC-LoRA Parameters 节点的可选 IC-LoRA 参数。用于根据某些 IC-LoRA（如 reference_downscale_factor > 1）需求调整引导处理。链式连接时，每个 LTXVAddGuide 仅使用连接到它的参数。"
		},
		"image": {
			"name": "图像",
...
var LTXVAudioVAELoader = {
	"display_name": "LTXV音频VAE加载器",
	"inputs": { "ckpt_name": {
		"name": "ckpt名称",
		"tooltip": "要加载的音频VAE检查点。"
	} },
	"outputs": { "0": {
		"name": "音频VAE",
		"tooltip": null
...
var LTXVContextWindows = {
	"description": "为类似LTXV的模型设置上下文窗口。",
	"display_name": "LTXV上下文窗口",
	"inputs": {
		"closed_loop": {
			"name": "closed_loop",
			"tooltip": "是否闭合上下文窗口循环，仅适用于循环调度。"
		...
		"context_length": {
			"name": "context_length",
			"tooltip": "上下文窗口的实际帧长度。必须为8*n+1。"
		},
		"context_overlap": {
			"name": "context_overlap",
			"tooltip": "上下文窗口在实际帧中的重叠部分。"
		},
		"context_schedule": {
			"name": "context_schedule",
			"tooltip": "上下文窗口的步进调度算法。"
		},
		"context_stride": {
			"name": "context_stride",
			"tooltip": "上下文窗口的步幅，仅适用于均匀调度。"
		...
		"freenoise": {
			"name": "freenoise",
			"tooltip": "是否应用FreeNoise噪声洗牌，提升窗口融合效果。"
		},
		"fuse_method": {
			"name": "fuse_method",
			"tooltip": "用于融合上下文窗口的方法。"
		...
		"model": {
			"name": "model",
			"tooltip": "采样时应用上下文窗口的模型。"
		},
		"retain_first_frame": {
			"name": "retain_first_frame",
			"tooltip": "在每个上下文窗口中保留第一个latent帧（有助于保留初始参考）。"
		},
		"split_conds_to_windows": {
			"name": "split_conds_to_windows",
			"tooltip": "是否将多个条件（由ConditionCombine创建）根据区域索引分配到各个窗口。"
	...
	"outputs": { "0": { "tooltip": "采样时应用了上下文窗口的模型。" } }
};
var LTXVCropGuides = {
...
var LTXVEmptyLatentAudio = {
	"display_name": "LTXV 空音频潜空间",
	"inputs": {
		"audio_vae": {
			"name": "音频VAE",
			"tooltip": "用于获取配置信息的音频VAE模型。"
		},
		"batch_size": {
			"name": "批量大小",
			"tooltip": "批次中潜音频样本的数量。"
...
var LTXVReferenceAudio = {
	"description": "设置参考音频用于 ID-LoRA 说话人身份迁移。将参考音频 clip 编码为条件，并可选地通过身份引导修补模型（每步额外进行一次无参考的前向传播，以增强说话人身份效果）。",
	"display_name": "LTXV 参考音频（ID-LoRA）",
	"inputs": {
		"audio_vae": {
			"name": "audio_vae",
			"tooltip": "用于编码的 LTXV Audio VAE。"
		},
		"end_percent": {
			"name": "end_percent",
			"tooltip": "身份引导激活的 sigma 区间结束百分比。"
		},
		"identity_guidance_scale": {
			"name": "identity_guidance_scale",
			"tooltip": "身份引导强度。每步会额外进行一次无参考的前向传播以增强说话人身份。设置为 0 可禁用（无额外前向传播）。"
		},
		"model": { "name": "model" },
		"negative": { "name": "negative" },
		"positive": { "name": "positive" },
		"reference_audio": {
			...
			"tooltip": "要迁移说话人身份的参考音频 clip。建议时长约 5 秒（训练时长）。过短或过长的音频可能会降低声音身份迁移效果。"
		},
		"start_percent": {
			"name": "start_percent",
			"tooltip": "身份引导激活的 sigma 区间起始百分比。"
		}
	},
	"outputs": {
		"0": { "tooltip": null },
...
var LTXVSeparateAVLatent = {
	"description": "LTXV分离音视频潜空间",
	"display_name": "LTXV分离音视频潜空间",
	"inputs": { "av_latent": { "name": "音视频潜空间" } },
	"outputs": {
		"0": {
			"name": "video_latent",
			"tooltip": null
...
var LatentAdd = {
	...
	"inputs": {
		"samples1": { "name": "Latent1" },
		"samples2": { "name": "Latent2" }
	},
	"outputs": { "0": { "tooltip": null } }
};
...
var LatentComposite = {
	...
	"inputs": {
		"feather": { "name": "羽化" },
		"samples_from": { "name": "Latent从" },
		"samples_to": { "name": "Latent到" },
		"x": { "name": "x" },
		"y": { "name": "y" }
	}
};
var LatentCompositeMasked = {
	...
	"inputs": {
		"destination": { "name": "目标Latent" },
		"mask": { "name": "遮罩" },
		"resize_source": { "name": "调整来源大小" },
		"source": { "name": "来源Latent" },
		"x": { "name": "x" },
		"y": { "name": "y" }
	},
	"outputs": { "0": { "tooltip": null } }
...
var LatentCutToBatch = {
	"display_name": "Latent切割",
	"inputs": {
		"dim": { "name": "维度" },
		"samples": { "name": "Latent" },
		"slice_size": { "name": "碎片大小" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var LatentFlip = {
...
var LatentUpscaleModelLoader = {
	"display_name": "加载Latent放大模型",
	"inputs": { "model_name": { "name": "模型" } },
	"outputs": { "0": { "tooltip": null } }
};
var LazyCache = {
	"description": "EasyCache 的自制版本 - 更'简单'的 EasyCache 实现。总体效果不如 EasyCache，但在某些罕见情况下表现更好，并且与 ComfyUI 中的所有内容具有通用兼容性。",
	"display_name": "惰性缓存",
	"inputs": {
		"end_percent": {
			"name": "结束百分比",
			"tooltip": "结束使用惰性缓存的相对采样步骤。"
		},
		"model": {
			"name": "模型",
			"tooltip": "要添加惰性缓存的模型。"
...
var LoadDA3Model = {
	"display_name": "加载Depth Anything 3",
	"inputs": {
...
var LoadImageDataSetFromFolder = {
	"description": "从指定文件夹加载图像数据集并返回图像列表。支持格式：PNG、JPG、JPEG、WEBP。",
	"display_name": "加载图像数据集",
	"inputs": { "folder": {
		"name": "文件夹",
		"tooltip": "图像数据集的文件夹"
	} },
...
var LoadImageTextDataSetFromFolder = {
	"description": "从指定文件夹加载图像与文本描述对的数据集，并以列表形式返回。支持格式：PNG、JPG、JPEG、WEBP。",
	"display_name": "加载图像和文本数据集",
	"inputs": { "folder": {
		"name": "文件夹",
		"tooltip": "图像数据集的文件夹"
	} },
...
var LoadMediaPipeFaceLandmarker = {
	"display_name": "加载 MediaPipe 人脸标记器",
	"inputs": { "model_name": {
		"name": "model_name",
		"tooltip": "来自 models/mediapipe/ 的 Face Landmarker safetensors。"
	} },
	"outputs": { "0": { "tooltip": null } }
};
var LoadMoGeModel = {
	"display_name": "加载 MoGe 模型",
	"inputs": { "model_name": { "name": "model_name" } },
	"outputs": { "0": { "tooltip": null } }
};
var LoadTrainingDataset = {
	"description": "从磁盘加载已编码的训练数据集（latent + 条件信息），用于训练。",
	"display_name": "加载训练数据集",
	"inputs": { "folder_name": {
		"name": "文件夹名",
		"tooltip": "包含训练数据集的文件夹名（在 输出 文件夹内）。"
	} },
	"outputs": {
		"0": {
			"name": "Latent",
...
var LoadVideo = {
	...
	"inputs": {
		"file": { "name": "文件" },
		"upload": { "name": "选择要上传的文件" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var LoadVideoDataSetFromFolder = {
	"description": "从指定文件夹加载视频数据集并返回视频列表。支持的格式：MP4、AVI、MOV、WEBM、MKV、FLV。",
	"display_name": "加载视频（来自文件夹）",
	"inputs": { "folder": {
		"name": "文件夹",
		"tooltip": "包含视频文件的文件夹。"
	} },
	"outputs": { "0": {
		"name": "视频",
		"tooltip": "惰性视频引用；只有在下游需要时才解码帧。"
	} }
};
var LoadVideoTextDataSetFromFolder = {
	"description": "从指定文件夹加载视频和文本字幕对的数据集，并以列表形式返回。支持的格式：MP4、AVI、MOV、WEBM、MKV、FLV。",
	"display_name": "加载视频-文本（来自文件夹）",
	"inputs": { "folder": {
		"name": "文件夹",
		"tooltip": "包含视频文件和 .txt 字幕的文件夹。"
	} },
	"outputs": {
		"0": {
			"name": "视频",
...
var LoraLoader = {
	...
	"outputs": {
		"0": { "tooltip": "修改后的扩散模型。" },
		"1": { "tooltip": "修改后的CLIP模型。" }
	}
};
var LoraLoaderBypass = {
	"description": "以旁路模式应用LoRA。与常规LoRA不同，此方式不会修改模型权重——而是在前向传播时注入LoRA计算。适用于训练场景。",
	"display_name": "加载LoRA（旁路）（用于调试）",
	"inputs": {
		"clip": {
			"name": "clip",
			"tooltip": "将应用LoRA的CLIP模型。"
		},
		"lora_name": {
			"name": "lora_name",
			"tooltip": "LoRA的名称。"
		...
		"model": {
			"name": "model",
			"tooltip": "将应用LoRA的扩散模型。"
		},
		"strength_clip": {
			"name": "strength_clip",
			"tooltip": "对CLIP模型的修改强度。该值可以为负数。"
		},
		"strength_model": {
			"name": "strength_model",
			"tooltip": "对扩散模型的修改强度。该值可以为负数。"
		}
	},
	"outputs": {
		"0": { "tooltip": "已修改的扩散模型。" },
		"1": { "tooltip": "已修改的CLIP模型。" }
	}
};
var LoraLoaderBypassModelOnly = {
	"description": "以旁路模式应用LoRA。与常规LoRA不同，此方式不会修改模型权重——而是在前向传播时注入LoRA计算。适用于训练场景。",
	"display_name": "加载LoRA（旁路，仅模型）（用于调试）",
	"inputs": {
		"lora_name": { "name": "lora_name" },
		"model": { "name": "model" },
		"strength_model": { "name": "strength_model" }
	},
	"outputs": { "0": { "tooltip": "已修改的扩散模型。" } }
};
...
var LtxvApiImageToVideo = {
	"description": "基于起始图像生成专业品质视频，可自定义时长和分辨率。",
	"display_name": "LTXV 图像到视频",
	"inputs": {
		"duration": { "name": "时长" },
		"fps": { "name": "帧率" },
		"generate_audio": {
			"name": "生成音频",
			"tooltip": "为 true 时，生成的视频将包含与场景匹配的 AI 生成音频。"
		},
		"image": {
			"name": "图像",
			"tooltip": "用于视频的第一帧图像。"
...
var LtxvApiTextToVideo = {
	"description": "可自定义时长和分辨率的专业品质视频。",
	"display_name": "LTXV 文本到视频",
	"inputs": {
		"duration": { "name": "时长" },
		"fps": { "name": "帧率" },
		"generate_audio": {
			"name": "生成音频",
			"tooltip": "生成的视频将包含与场景匹配的 AI 生成音频。"
		},
		"model": { "name": "模型" },
		"prompt": { "name": "提示词" },
		"resolution": { "name": "分辨率" }
...
var LumaImageEditNode2 = {
	"description": "使用 Luma UNI-1 模型，通过文本提示编辑已有图像。",
	"display_name": "Luma UNI-1 图像编辑",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"model": {
			"name": "model",
...
var LumaImageNode2 = {
	"description": "使用 Luma UNI-1 模型，根据文本生成图像。",
	"display_name": "Luma UNI-1 图像生成",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"model": {
			"name": "model",
...
var LumaRay32ExtendVideoNode = {
	"description": "将之前的 Ray 3.2 生成结果向前（在其后继续）或向后（在其前引入）扩展。请连接前一个 Luma Ray 3.2 节点的 generation_id 输出。每次扩展均为 5 秒。",
	"display_name": "Luma Ray 3.2 视频扩展",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"direction": {
			"name": "direction",
			"tooltip": "Forward 表示在前一个 clip 后继续；backward 表示在其前插入。"
		},
		"direction_loop": { "name": "循环" },
		"prompt": {
			"name": "prompt",
			"tooltip": "新内容的文本提示。"
		},
		...
		"seed": {
			"name": "seed",
			"tooltip": "用于决定节点是否重新运行的种子；无论种子如何，结果都是非确定性的。"
		},
		"source_generation_id": {
			"name": "source_generation_id",
			"tooltip": "要扩展的视频的 Ray 3.2 generation_id。请连接另一个 Luma Ray 3.2 节点的 generation_id 输出。"
		}
	},
	"outputs": {
		"0": { "tooltip": null },
...
var LumaRay32ImageToVideoNode = {
	"description": "使用 Luma 的 Ray 3.2 模型从起始帧和/或结束帧生成视频。基于图像锚点的生成时长均为 5 秒。",
	"display_name": "Luma Ray 3.2 图像转视频",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"end_frame": {
			"name": "end_frame",
		...
		"loop": {
			"name": "循环",
			"tooltip": "使视频无缝循环。当设置了 end_frame 时不可用。"
		},
		"prompt": {
			"name": "prompt",
			"tooltip": "视频生成的文本提示。"
...
var LumaRay32KeyframeNode = {
	"description": "将引导图像锚定到 Ray 3.2 输出视频时间线上指定位置。请将此节点连接到 Luma Ray 3.2 关键帧转视频节点的 'keyframes' 输入；可通过下方可选的 'keyframes' 输入将多个关键帧串联。",
	"display_name": "Luma Ray 3.2 关键帧",
	"inputs": {
		"image": {
			"name": "image",
			"tooltip": "要放置在输出视频指定时刻的引导图像。"
		},
		"keyframes": {
			"name": "keyframes",
			"tooltip": "可选的前置关键帧，用于与当前关键帧串联。"
		},
		"position": {
			"name": "position",
			"tooltip": "如何将该图像放置在输出视频的时间线上。"
		},
...
var LumaRay32KeyframesToVideoNode = {
	"description": "使用 Luma Ray 3.2，通过插值一系列锚定在时间轴上的引导图像，生成视频。请使用 Luma Ray 3.2 关键帧节点（至少 2 个）构建序列。",
	"display_name": "Luma Ray 3.2 关键帧转视频",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"duration": { "name": "时长" },
		"keyframes": {
			"name": "关键帧",
			"tooltip": "来自 Luma Ray 3.2 关键帧节点的关键帧序列（至少 2 个）。"
		},
		"prompt": {
			"name": "提示词",
			"tooltip": "用于视频生成的文本提示。"
...
var LumaRay32TextToVideoNode = {
	"description": "使用 Luma 的 Ray 3.2 模型，根据文本提示生成视频。",
	"display_name": "Luma Ray 3.2 文本转视频",
	"inputs": {
		"aspect_ratio": { "name": "宽高比" },
		"control_after_generate": { "name": "生成后控制" },
		"duration": { "name": "时长" },
...
var LumaRay32VideoEditNode = {
	"description": "使用 Luma Ray 3.2 在新提示下重新渲染已有视频（可重绘风格、重新打光、添加或移除元素），同时保持原有运动。源视频最长 18 秒；编辑后的视频保持原长度。",
	"display_name": "Luma Ray 3.2 视频编辑",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"prompt": {
			"name": "提示词",
		...
		"strength": {
			"name": "强度",
			"tooltip": "保留原视频与重新想象的程度。'auto' 由 Ray 3.2 自动选择；adher_* 保留最多，flex_* 平衡，reimagine_* 变化最大。"
		},
		"video": {
			"name": "视频",
			"tooltip": "要编辑的源视频。最长 18 秒。"
		}
	},
	"outputs": {
		"0": { "tooltip": null },
...
var LumaRay32VideoReframeNode = {
	"description": "更改现有视频的宽高比，使用 Luma Ray 3.2 填充新暴露的画布区域。源视频最长 30 秒。按输出秒数计费。",
	"display_name": "Luma Ray 3.2 视频重构",
	"inputs": {
		"aspect_ratio": { "name": "aspect_ratio" },
		"control_after_generate": { "name": "control after generate" },
		"prompt": {
			"name": "prompt",
			"tooltip": "描述如何填充新暴露的画布区域。"
		},
		"resolution": { "name": "resolution" },
		"seed": {
			"name": "seed",
...
var MagnificImageRelightNode = {
	"description": "通过光照调整和可选的参考光照迁移对图像进行重光照。",
	"display_name": "Magnific图像重光照",
	"inputs": {
		"advanced_settings": {
		...
		"prompt": {
			"name": "prompt",
			"tooltip": "光照描述性引导。支持强调符号（1-1.4）。"
		},
		"reference_image": {
			"name": "reference_image",
			"tooltip": "可选的参考图像，用于迁移光照。"
		},
		"style": {
			"name": "style",
			"tooltip": "风格化输出偏好。"
...
var MagnificImageSkinEnhancerNode = {
	"description": "用于人像的美肤增强，支持多种处理模式。",
	"display_name": "Magnific图像美肤增强",
	"inputs": {
		"image": {
			"name": "image",
			"tooltip": "需要增强的人像图像。"
		...
		"mode": {
			"name": "mode",
			"tooltip": "处理模式：creative为艺术增强，faithful为保留原貌，flexible为定向优化。"
		},
		"sharpen": {
			"name": "sharpen",
			"tooltip": "锐化强度等级。"
...
var MagnificImageStyleTransferNode = {
	"description": "将参考图像的风格迁移到你的输入图像上。",
	"display_name": "Magnific 图像风格迁移",
	"inputs": {
		"engine": {
			"name": "engine",
			"tooltip": "处理引擎选择。"
		...
		"fixed_generation": {
			...
			"tooltip": "禁用时，每次生成都会引入一定的随机性，从而产生更多样化的结果。"
		},
		"flavor": {
			"name": "flavor",
			"tooltip": "风格迁移风格类型。"
...
var MagnificImageUpscalerCreativeNode = {
	"description": "基于提示词的增强、风格化，以及 2x/4x/8x/16x 放大。最大输出：25.3 百万像素。",
	...
	"inputs": {
		...
		"image": { "name": "图像" },
		"optimized_for": { "name": "优化目标" },
		"prompt": { "name": "提示词" },
		"resemblance": {
			"name": "相似度",
			"tooltip": "与原始图像的相似程度。"
		},
		"scale_factor": { "name": "放大倍数" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var MagnificImageUpscalerPreciseV2Node = {
	"description": "高保真放大，精细控制锐度、颗粒感和细节。最大输出：10060×10060 像素。",
	...
	"inputs": {
		...
		"flavor": {
			"name": "风格",
			"tooltip": "处理风格：sublime 适用于通用，photo 适用于照片，photo_denoiser 适用于有噪点的照片。"
		},
		"image": { "name": "图像" },
		"scale_factor": { "name": "放大倍数" },
		"sharpen": {
...
var MakeTrainingDataset = {
	"description": "使用VAE对图像编码，使用CLIP对文本编码，生成包含latent和条件信息的训练数据集。",
	"display_name": "制作训练数据集",
	"inputs": {
		"clip": {
			"name": "CLIP",
			"tooltip": "用于编码文本的 CLIP 模型"
		...
		"texts": {
			"name": "文本",
			"tooltip": "标注文本序列。数量可以是n（匹配图像），1（重复使用），或者省略（使用空字符串）。"
		},
		"vae": {
			"name": "VAE",
			"tooltip": "用于编码图像的 VAE 模型"
		}
	},
	"outputs": {
		"0": {
...
var MediaPipeFaceLandmarker = {
	"description": "使用MediaPipe模型检测人脸关键点。",
	"display_name": "MediaPipe 人脸标记器",
	"inputs": {
		"detector_variant": {
			"name": "detector_variant",
			"tooltip": "人脸检测器范围。'short' 适用于近距离人脸（距离相机约 2 米内）；'full' 适用于更远/更小的人脸（最多约 5 米），但速度较慢。'both' 同时运行两个检测器，并保留每帧检测到更多人脸的结果（检测成本约为 2 倍）。"
		},
		"face_detection_model": { "name": "face_detection_model" },
		"image": { "name": "image" },
		"min_confidence": {
			"name": "min_confidence",
			"tooltip": "BlazeFace 分数阈值。降低以捕捉较小/被遮挡的人脸。"
		},
		"missing_frame_fallback": {
			"name": "missing_frame_fallback",
			"tooltip": "批量检测失败时每帧的处理方式。'empty' 表示该帧无人脸。'previous' 复制最近一次成功检测的结果。'interpolate' 在包围的成功帧之间插值地标/边框/混合形状。多脸：通过贪婪边框中心最近邻配对跨帧人脸。"
		},
		"num_faces": {
			"name": "num_faces",
			"tooltip": "每帧返回的最大人脸数。0 = 不限制（返回所有检测到的人脸）。"
		}
	},
	"outputs": {
		"0": {
...
var MediaPipeFaceMask = {
	"description": "根据人脸关键点绘制mask。",
	"display_name": "MediaPipe 人脸蒙版",
	"inputs": {
		"face_landmarks": { "name": "face_landmarks" },
		"regions": {
			"name": "regions",
			"tooltip": "'all' = face_oval+lips+eyes+irises 的并集（由于 face_oval 包含其他区域，最终为 face_oval）。'custom' = 可单独切换每个区域，组合如 lips+eyes。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var MediaPipeFaceMeshVisualize = {
	"description": "在输入图像上绘制人脸关键点网格。",
	"display_name": "MediaPipe 人脸网格可视化",
	"inputs": {
		"color": { "name": "color" },
		"connections": {
			"name": "connections",
			"tooltip": "'all' = oval+eyes+brows+lips+irises+nose。'fill' = 实心 face_oval 多边形（轮廓蒙版）。'custom' = 可单独切换每个特征（包括 'tesselation'，即完整的 2547 边线框）。"
		},
		"face_landmarks": { "name": "face_landmarks" },
		"image": {
			"name": "image",
			"tooltip": "如果未连接，将使用黑色画布。"
		},
		"point_size": {
			"name": "point_size",
			"tooltip": "地标点像素半径。0 表示不绘制点。"
		},
		"thickness": {
			"name": "thickness",
			"tooltip": "边线像素粗细。0 表示不绘制边线。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var MergeImageLists = {
	"description": "将多个图像列表合并为一个。",
	"display_name": "融合图像序列",
	"inputs": { "images": {
		"name": "图像",
		"tooltip": "需要处理的图像序列。"
...
var MergeSplat = {
	"description": "将任意数量的高斯 splat 拼接为一个。对同一 latent 在不同种子下的多次解码进行合并，可以使表面更密集，从而提升网格化时的表面质量。",
	"display_name": "合并 Splats",
...
var MeshyAnimateModelNode = {
	"description": "为已绑定骨骼的角色应用特定动画动作。",
	"display_name": "Meshy：动画模型",
	"inputs": {
		"action_id": {
			"name": "action_id",
			"tooltip": "访问 https://docs.meshy.ai/en/api/animation-library 查看可用值列表。"
		},
...
var MeshyRefineNode = {
	...
	"display_name": "Meshy：精细化草稿模型",
	"inputs": {
		"enable_pbr": {
			"name": "enable_pbr",
			"tooltip": "在基础色之外生成PBR贴图（金属度、粗糙度、法线）。注意：使用雕塑风格时应设置为false，因为雕塑风格会生成其专属的PBR贴图。"
		},
		"meshy_task_id": { "name": "meshy_task_id" },
		"model": { "name": "model" },
		"texture_image": {
			"name": "texture_image",
			"tooltip": "‘texture_image’和‘texture_prompt’只能二选一。"
		},
		"texture_prompt": {
			"name": "texture_prompt",
...
var MeshyRigModelNode = {
	...
	"display_name": "Meshy：骨骼绑定模型",
	"inputs": {
		"height_meters": {
			"name": "height_meters",
			"tooltip": "角色模型的大致高度（米），有助于缩放和骨骼绑定的准确性。"
		},
		"meshy_task_id": { "name": "meshy_task_id" },
		"texture_image": {
			"name": "texture_image",
...
var MeshyTextureNode = {
	...
	"inputs": {
		"enable_original_uv": {
			"name": "启用原始UV",
			"tooltip": "使用模型的原始UV，而不是生成新的UV。启用后，Meshy会保留上传模型中已有的纹理。如果模型没有原始UV，输出质量可能会有所下降。"
		},
		"image_style": {
			"name": "图像风格",
			"tooltip": "用于引导纹理生成的2D图像。不能与“text_style_prompt”同时使用。"
		},
		"meshy_task_id": { "name": "meshy_task_id" },
		"model": { "name": "模型" },
		"pbr": { "name": "PBR" },
...
var MinimaxHailuoVideoNode = {
	...
	"inputs": {
		...
		"first_frame_image": {
			"name": "第一帧图像",
			"tooltip": "可选图像，用作生成视频的第一帧。"
		},
		"prompt_optimizer": {
			"name": "提示优化器",
			"tooltip": "需要时优化提示词以提高生成质量。"
		},
		"prompt_text": {
			"name": "提示文本",
			"tooltip": "指导视频生成的文本提示。"
		...
		"resolution": {
			"name": "分辨率",
			"tooltip": "视频显示的尺寸。1080p为1920x1080，768p为1366x768。"
		},
		"seed": {
			"name": "种子",
			"tooltip": "用于创建噪声的随机种子。"
...
var MoGeInference = {
	"description": "在单张图像上运行MoGe以估算深度和几何信息。",
	"display_name": "MoGe 推理",
	"inputs": {
		"apply_mask": {
			"name": "apply_mask",
			"tooltip": "将被遮罩（天空/无效）像素在点云和深度中设为 inf，以便网格化时剔除。禁用后将保留所有原始预测几何信息；遮罩仍会单独返回。"
		},
		"batch_size": {
			"name": "batch_size",
			"tooltip": "每次推理的图像数量。如果处理长视频或大批量图像时显存不足，请降低此值。"
		},
		"force_projection": { "name": "force_projection" },
		"fov_x_degrees": {
			"name": "fov_x_degrees",
			"tooltip": "源相机的水平视场角。设置用于将深度图反投影到 3D 的焦距。0 = 自动从预测点恢复。"
		},
		"image": { "name": "image" },
		"moge_model": { "name": "moge_model" },
		"resolution_level": {
			"name": "resolution_level",
			"tooltip": "0 = 速度最快，9 = 细节最多。"
		}
	},
	"outputs": { "0": {
		"name": "moge_geometry",
...
var MoGePanoramaInference = {
	"description": "将全景图像分割为12个透视视图，分别运行MoGe推理，并将结果合并为单一深度图。",
	"display_name": "MoGe 全景推理",
	"inputs": {
		"batch_size": {
			"name": "batch_size",
			"tooltip": "每批推理的视角数量（共 12 个分割）。"
		},
		"image": {
			"name": "image",
			"tooltip": "等距柱状全景图（任意宽高比）。"
		},
		"merge_resolution": {
			"name": "merge_resolution",
			"tooltip": "合并后距离图的长边分辨率。"
		},
		"moge_model": { "name": "moge_model" },
		"resolution_level": {
			"name": "resolution_level",
			"tooltip": "每视角细节（0 = 速度最快，9 = 细节最多）。"
		},
		"split_resolution": {
...
var MoGePointMapToMesh = {
	"description": "将MoGe点图转换为3D网格。",
	"display_name": "MoGe 点云映射到网格",
	"inputs": {
		"batch_index": {
			"name": "batch_index",
			"tooltip": "选择批量 MoGe 几何体中的哪一张图像进行网格化。每张图像的顶点数量不同，因此批量无法合并为单一网格。"
		},
		"decimation": {
			"name": "decimation",
			"tooltip": "顶点步长；1 = 全分辨率。"
		...
		"discontinuity_threshold": {
			"name": "discontinuity_threshold",
			"tooltip": "丢弃 3x3 深度跨度超过该阈值的像素。0 = 关闭。"
		},
		"moge_geometry": { "name": "moge_geometry" },
		"texture": {
			"name": "texture",
			"tooltip": "将源图像作为 baseColor 纹理传递。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var MoGeRender = {
	"description": "根据几何数据渲染深度图或法线图。",
	"display_name": "MoGe 渲染",
	"inputs": {
		"moge_geometry": { "name": "moge_geometry" },
		"output": {
			"name": "output",
			"tooltip": "DirectX 与 OpenGL 控制法线贴图绿色通道的约定。DirectX：绿色 = -Y 向下（Unreal）。OpenGL：绿色 = +Y 向上（Blender、Substance、Unity、glTF）。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
...
var ModelMergeAuraflow = {
	...
	"inputs": {
		"cond_seq_linear_": { "name": "条件序列线性." },
		"double_layers_0_": { "name": "双层.0." },
		"double_layers_1_": { "name": "双层.1." },
		"double_layers_2_": { "name": "双层.2." },
		"double_layers_3_": { "name": "双层.3." },
		"final_linear_": { "name": "最终线性." },
		"init_x_linear_": { "name": "初始x线性." },
		"modF_": { "name": "modF." },
		"model1": { "name": "模型1" },
		"model2": { "name": "模型2" },
		"positional_encoding": { "name": "位置编码" },
		"register_tokens": { "name": "注册令牌" },
		"single_layers_0_": { "name": "单层.0." },
		"single_layers_10_": { "name": "单层.10." },
		"single_layers_11_": { "name": "单层.11." },
		"single_layers_12_": { "name": "单层.12." },
		"single_layers_13_": { "name": "单层.13." },
		"single_layers_14_": { "name": "单层.14." },
		"single_layers_15_": { "name": "单层.15." },
		"single_layers_16_": { "name": "单层.16." },
		"single_layers_17_": { "name": "单层.17." },
		"single_layers_18_": { "name": "单层.18." },
		"single_layers_19_": { "name": "单层.19." },
		"single_layers_1_": { "name": "单层.1." },
		"single_layers_20_": { "name": "单层.20." },
		"single_layers_21_": { "name": "单层.21." },
		"single_layers_22_": { "name": "单层.22." },
		"single_layers_23_": { "name": "单层.23." },
		"single_layers_24_": { "name": "单层.24." },
		"single_layers_25_": { "name": "单层.25." },
		"single_layers_26_": { "name": "单层.26." },
		"single_layers_27_": { "name": "单层.27." },
		"single_layers_28_": { "name": "单层.28." },
		"single_layers_29_": { "name": "单层.29." },
		"single_layers_2_": { "name": "单层.2." },
		"single_layers_30_": { "name": "单层.30." },
		"single_layers_31_": { "name": "单层.31." },
		"single_layers_3_": { "name": "单层.3." },
		"single_layers_4_": { "name": "单层.4." },
		"single_layers_5_": { "name": "单层.5." },
		"single_layers_6_": { "name": "单层.6." },
		"single_layers_7_": { "name": "单层.7." },
		"single_layers_8_": { "name": "单层.8." },
		"single_layers_9_": { "name": "单层.9." },
...
var ModelMergeCosmos14B = {
	"display_name": "模型融合（Cosmos14B）",
	"inputs": {
		"affline_norm_": { "name": "仿射规范化." },
		...
		"extra_pos_embedder_": { "name": "额外位置嵌入器." },
		"final_layer_": { "name": "最终层." },
		"model1": { "name": "模型1" },
		"model2": { "name": "模型2" },
...
var ModelMergeCosmosPredict2_14B = {
	"display_name": "模型融合（CosmosPredict2_14B）",
	"inputs": {
		"blocks_0_": { "name": "块0。" },
		"blocks_10_": { "name": "块10。" },
		"blocks_11_": { "name": "块11。" },
		...
		"pos_embedder_": { "name": "位置嵌入器。" },
		"t_embedder_": { "name": "t嵌入器。" },
		"t_embedding_norm_": { "name": "t嵌入归一化。" },
		"x_embedder_": { "name": "x嵌入器。" }
	}
};
var ModelMergeCosmosPredict2_2B = {
	"display_name": "模型融合（CosmosPredict2_2B）",
	"inputs": {
		"blocks_0_": { "name": "块.0." },
		"blocks_10_": { "name": "块.10." },
		"blocks_11_": { "name": "块.11." },
		"blocks_12_": { "name": "块.12." },
		"blocks_13_": { "name": "块.13." },
		"blocks_14_": { "name": "块.14." },
		"blocks_15_": { "name": "块.15." },
		"blocks_16_": { "name": "块.16." },
		"blocks_17_": { "name": "块.17." },
		"blocks_18_": { "name": "块.18." },
		"blocks_19_": { "name": "块.19." },
		"blocks_1_": { "name": "块.1." },
		"blocks_20_": { "name": "块.20." },
		"blocks_21_": { "name": "块.21." },
		"blocks_22_": { "name": "块.22." },
		"blocks_23_": { "name": "块.23." },
		"blocks_24_": { "name": "块.24." },
		"blocks_25_": { "name": "块.25." },
		"blocks_26_": { "name": "块.26." },
		"blocks_27_": { "name": "块.27." },
		"blocks_2_": { "name": "块.2." },
		"blocks_3_": { "name": "块.3." },
		"blocks_4_": { "name": "块.4." },
		"blocks_5_": { "name": "块.5." },
		...
		"model2": { "name": "模型2" },
		"pos_embedder_": { "name": "位置嵌入器。" },
		"t_embedder_": { "name": "t_嵌入器。" },
		"t_embedding_norm_": { "name": "t_嵌入归一化。" },
		"x_embedder_": { "name": "x_嵌入器。" }
	}
};
var ModelMergeFlux1 = {
	"display_name": "模型融合（Flux1）",
	"inputs": {
		"double_blocks_0_": { "name": "双块.0." },
		"double_blocks_10_": { "name": "双块.10." },
		"double_blocks_11_": { "name": "双块.11." },
		"double_blocks_12_": { "name": "双块.12." },
		"double_blocks_13_": { "name": "双块.13." },
		"double_blocks_14_": { "name": "双块.14." },
		"double_blocks_15_": { "name": "双块.15." },
		"double_blocks_16_": { "name": "双块.16." },
		"double_blocks_17_": { "name": "双块.17." },
		"double_blocks_18_": { "name": "双块.18." },
		"double_blocks_1_": { "name": "双块.1." },
		"double_blocks_2_": { "name": "双块.2." },
		"double_blocks_3_": { "name": "双块.3." },
		"double_blocks_4_": { "name": "双块.4." },
		"double_blocks_5_": { "name": "双块.5." },
		"double_blocks_6_": { "name": "双块.6." },
		"double_blocks_7_": { "name": "双块.7." },
		"double_blocks_8_": { "name": "双块.8." },
		"double_blocks_9_": { "name": "双块.9." },
		"final_layer_": { "name": "最终层." },
		"guidance_in": { "name": "输入引导" },
		"img_in_": { "name": "输入图像." },
		"model1": { "name": "模型1" },
		"model2": { "name": "模型2" },
		"single_blocks_0_": { "name": "单块.0." },
		"single_blocks_10_": { "name": "单块.10." },
		"single_blocks_11_": { "name": "单块.11." },
		"single_blocks_12_": { "name": "单块.12." },
		"single_blocks_13_": { "name": "单块.13." },
		"single_blocks_14_": { "name": "单块.14." },
		"single_blocks_15_": { "name": "单块.15." },
		"single_blocks_16_": { "name": "单块.16." },
		"single_blocks_17_": { "name": "单块.17." },
		"single_blocks_18_": { "name": "单块.18." },
		"single_blocks_19_": { "name": "单块.19." },
		"single_blocks_1_": { "name": "单块.1." },
		"single_blocks_20_": { "name": "单块.20." },
		"single_blocks_21_": { "name": "单块.21." },
		"single_blocks_22_": { "name": "单块.22." },
		"single_blocks_23_": { "name": "单块.23." },
		"single_blocks_24_": { "name": "单块.24." },
		"single_blocks_25_": { "name": "单块.25." },
		"single_blocks_26_": { "name": "单块.26." },
		"single_blocks_27_": { "name": "单块.27." },
		"single_blocks_28_": { "name": "单块.28." },
		"single_blocks_29_": { "name": "单块.29." },
		"single_blocks_2_": { "name": "单块.2." },
		"single_blocks_30_": { "name": "单块.30." },
		"single_blocks_31_": { "name": "单块.31." },
		"single_blocks_32_": { "name": "单块.32." },
		"single_blocks_33_": { "name": "单块.33." },
		"single_blocks_34_": { "name": "单块.34." },
		"single_blocks_35_": { "name": "单块.35." },
		"single_blocks_36_": { "name": "单块.36." },
		"single_blocks_37_": { "name": "单块.37." },
		"single_blocks_3_": { "name": "单块.3." },
		"single_blocks_4_": { "name": "单块.4." },
		"single_blocks_5_": { "name": "单块.5." },
		"single_blocks_6_": { "name": "单块.6." },
		"single_blocks_7_": { "name": "单块.7." },
		"single_blocks_8_": { "name": "单块.8." },
		"single_blocks_9_": { "name": "单块.9." },
		"time_in_": { "name": "输入时间." },
		"txt_in_": { "name": "输入文本." },
		"vector_in_": { "name": "输入向量." }
	}
};
var ModelMergeKrea2 = {
...
var ModelMergeQwenImage = {
	...
	"inputs": {
		...
		"proj_out_": { "name": "proj_out." },
		"time_text_embed_": { "name": "时间文本嵌入。" },
		"transformer_blocks_0_": { "name": "变换器块.0." },
		"transformer_blocks_10_": { "name": "transformer_blocks.10." },
		"transformer_blocks_11_": { "name": "transformer_blocks.11." },
		"transformer_blocks_12_": { "name": "transformer_blocks.12." },
		"transformer_blocks_13_": { "name": "transformer_blocks.13." },
		...
		"transformer_blocks_18_": { "name": "transformer_blocks.18." },
		"transformer_blocks_19_": { "name": "transformer_blocks.19." },
		"transformer_blocks_1_": { "name": "变换器块.1." },
		"transformer_blocks_20_": { "name": "transformer_blocks.20." },
		"transformer_blocks_21_": { "name": "transformer_blocks.21." },
		"transformer_blocks_22_": { "name": "transformer_blocks.22." },
		"transformer_blocks_23_": { "name": "transformer_blocks.23." },
		...
		"transformer_blocks_2_": { "name": "变换器块.2." },
		...
		"transformer_blocks_3_": { "name": "变换器块.3." },
		...
		"transformer_blocks_4_": { "name": "变换器块.4." },
		...
		"transformer_blocks_5_": { "name": "变换器块.5." },
		"transformer_blocks_6_": { "name": "变换器块.6." },
		"transformer_blocks_7_": { "name": "变换器块.7." },
		"transformer_blocks_8_": { "name": "变换器块.8." },
		"transformer_blocks_9_": { "name": "变换器块.9." },
		"txt_in_": { "name": "文本输入。" },
		"txt_norm_": { "name": "文本归一化。" }
	}
};
var ModelMergeSD1 = {
	"display_name": "模型融合（SD1）",
	"inputs": {
		"input_blocks_0_": { "name": "输入块.0." },
		"input_blocks_10_": { "name": "输入块.10." },
		"input_blocks_11_": { "name": "输入块.11." },
		"input_blocks_1_": { "name": "输入块.1." },
		"input_blocks_2_": { "name": "输入块.2." },
		"input_blocks_3_": { "name": "输入块.3." },
		"input_blocks_4_": { "name": "输入块.4." },
		"input_blocks_5_": { "name": "输入块.5." },
		"input_blocks_6_": { "name": "输入块.6." },
		"input_blocks_7_": { "name": "输入块.7." },
		"input_blocks_8_": { "name": "输入块.8." },
		"input_blocks_9_": { "name": "输入块.9." },
		"label_emb_": { "name": "标签嵌入." },
		"middle_block_0_": { "name": "中间块.0." },
		"middle_block_1_": { "name": "中间块.1." },
		"middle_block_2_": { "name": "中间块.2." },
		"model1": { "name": "模型1" },
		"model2": { "name": "模型2" },
		"out_": { "name": "出." },
		"output_blocks_0_": { "name": "输出块.0." },
		"output_blocks_10_": { "name": "输出块.10." },
		"output_blocks_11_": { "name": "输出块.11." },
		"output_blocks_1_": { "name": "输出块.1." },
		"output_blocks_2_": { "name": "输出块.2." },
		"output_blocks_3_": { "name": "输出块.3." },
		"output_blocks_4_": { "name": "输出块.4." },
		"output_blocks_5_": { "name": "输出块.5." },
		"output_blocks_6_": { "name": "输出块.6." },
		"output_blocks_7_": { "name": "输出块.7." },
		"output_blocks_8_": { "name": "输出块.8." },
		"output_blocks_9_": { "name": "输出块.9." },
		"time_embed_": { "name": "时间嵌入." }
	}
};
var ModelMergeSD2 = {
...
var ModelMergeSD35_Large = {
	...
	"inputs": {
		"context_embedder_": { "name": "上下文嵌入器." },
		"final_layer_": { "name": "最终层." },
		"joint_blocks_0_": { "name": "联合块.0." },
		"joint_blocks_10_": { "name": "联合块.10." },
		"joint_blocks_11_": { "name": "联合块.11." },
		"joint_blocks_12_": { "name": "联合块.12." },
		"joint_blocks_13_": { "name": "联合块.13." },
		"joint_blocks_14_": { "name": "联合块.14." },
		"joint_blocks_15_": { "name": "联合块.15." },
		"joint_blocks_16_": { "name": "联合块.16." },
		"joint_blocks_17_": { "name": "联合块.17." },
		"joint_blocks_18_": { "name": "联合块.18." },
		"joint_blocks_19_": { "name": "联合块.19." },
		"joint_blocks_1_": { "name": "联合块.1." },
		"joint_blocks_20_": { "name": "联合块.20." },
		"joint_blocks_21_": { "name": "联合块.21." },
		"joint_blocks_22_": { "name": "联合块.22." },
		"joint_blocks_23_": { "name": "联合块.23." },
		"joint_blocks_24_": { "name": "联合块.24." },
		"joint_blocks_25_": { "name": "联合块.25." },
		"joint_blocks_26_": { "name": "联合块.26." },
		"joint_blocks_27_": { "name": "联合块.27." },
		"joint_blocks_28_": { "name": "联合块.28." },
		"joint_blocks_29_": { "name": "联合块.29." },
		"joint_blocks_2_": { "name": "联合块.2." },
		"joint_blocks_30_": { "name": "联合块.30." },
		"joint_blocks_31_": { "name": "联合块.31." },
		"joint_blocks_32_": { "name": "联合块.32." },
		"joint_blocks_33_": { "name": "联合块.33." },
		"joint_blocks_34_": { "name": "联合块.34." },
		"joint_blocks_35_": { "name": "联合块.35." },
		"joint_blocks_36_": { "name": "联合块.36." },
		"joint_blocks_37_": { "name": "联合块.37." },
		"joint_blocks_3_": { "name": "联合块.3." },
		"joint_blocks_4_": { "name": "联合块.4." },
		"joint_blocks_5_": { "name": "联合块.5." },
		"joint_blocks_6_": { "name": "联合块.6." },
		"joint_blocks_7_": { "name": "联合块.7." },
		"joint_blocks_8_": { "name": "联合块.8." },
		"joint_blocks_9_": { "name": "联合块.9." },
		"model1": { "name": "模型1" },
		"model2": { "name": "模型2" },
		"pos_embed_": { "name": "位置嵌入." },
		"t_embedder_": { "name": "t嵌入器." },
		"x_embedder_": { "name": "x嵌入器." },
		"y_embedder_": { "name": "y嵌入器." }
	}
};
...
var ModelMergeSDXL = {
	...
	"inputs": {
		"input_blocks_0": { "name": "输入块.0" },
		"input_blocks_1": { "name": "输入块.1" },
		"input_blocks_2": { "name": "输入块.2" },
		"input_blocks_3": { "name": "输入块.3" },
		"input_blocks_4": { "name": "输入块.4" },
		"input_blocks_5": { "name": "输入块.5" },
		"input_blocks_6": { "name": "输入块.6" },
		"input_blocks_7": { "name": "输入块.7" },
		"input_blocks_8": { "name": "输入块.8" },
		"label_emb_": { "name": "标签嵌入." },
		"middle_block_0": { "name": "中间块.0" },
		"middle_block_1": { "name": "中间块.1" },
		"middle_block_2": { "name": "中间块.2" },
		"model1": { "name": "模型1" },
		"model2": { "name": "模型2" },
		"out_": { "name": "输出." },
		"output_blocks_0": { "name": "输出块.0" },
		"output_blocks_1": { "name": "输出块.1" },
		"output_blocks_2": { "name": "输出块.2" },
		"output_blocks_3": { "name": "输出块.3" },
		"output_blocks_4": { "name": "输出块.4" },
		"output_blocks_5": { "name": "输出块.5" },
		"output_blocks_6": { "name": "输出块.6" },
		"output_blocks_7": { "name": "输出块.7" },
		"output_blocks_8": { "name": "输出块.8" },
		"time_embed_": { "name": "时间嵌入." }
	}
};
...
var ModelSamplingContinuousEDM = {
	...
	"inputs": {
		"model": { "name": "模型" },
		"sampling": { "name": "采样" },
		"sigma_max": { "name": "最大西格玛" },
		"sigma_min": { "name": "最小西格玛" }
	}
};
...
var MultiGPU_WorkUnits = {
	"description": "准备模型以通过拆分工作单元加速采样。\n\n应放置在修改模型对象本身的节点之后，例如编译或注意力切换节点。\n\n除这些例外外，此节点可按任意顺序放置。",
	"display_name": "多GPU CFG 拆分",
	"inputs": {
...
var NAGuidance = {
	...
	"outputs": { "0": { "tooltip": "已启用NAG的修补模型。" } }
};
var NormalizeImages = {
	"description": "使用均值和标准差对图像进行归一化处理。",
	"display_name": "规格化图像",
	"inputs": {
		"images": {
			"name": "图像",
...
var NormalizeVideoLatentStart = {
	"description": "将视频初始帧的 Latent 规格化，使其符合后续参考帧的平均值与标准差。有助于减少起始帧与视频其余部分之间的差异。",
	"display_name": "规格化视频Latent",
	"inputs": {
		"latent": { "name": "Latent" },
		"reference_frame_count": {
			"name": "参考数量",
			"tooltip": "从起始帧开始，用于参考的 Latent 数量"
		},
		"start_frame_count": {
			"name": "规格化数量",
			"tooltip": "从起始帧开始，需要规格化的 Latent 数量"
		}
	},
	"outputs": { "0": {
		"name": "latent",
...
var OpenAIChatConfig = {
	...
	"inputs": {
		"instructions": {
			"name": "指令",
			"tooltip": "指导模型如何生成响应的指令"
		},
		"max_output_tokens": {
			"name": "Token输出上限",
			"tooltip": "生成响应时可生成token数量的上限，包括可见输出token"
		},
		"truncation": {
			"name": "截断",
			"tooltip": "用于模型响应的截断策略。auto：如果此响应和先前响应的上下文超过模型的上下文窗口大小，模型将通过丢弃对话中间部分的输入项来截断响应以适应上下文窗口。disabled：如果模型响应将超过模型的上下文窗口大小，请求将失败并返回400错误。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var OpenAIChatNode = {
	"description": "从OpenAI模型生成文本响应。",
	"display_name": "OpenAI ChatGPT",
	"inputs": {
		"advanced_options": {
			"name": "高级设置",
			"tooltip": "模型的可选配置。接受来自OpenAI聊天高级选项节点的输入。"
		},
		"files": {
			"name": "文件",
			"tooltip": "可选文件，用作模型的上下文。接受来自OpenAI聊天输入文件节点的输入。"
		},
		"images": {
			"name": "图像",
			"tooltip": "可选图像，用作模型的上下文。要包含多张图像，可使用批处理图像节点。"
		},
		"model": {
			"name": "模型",
			"tooltip": "用于生成响应的模型"
		...
		"persist_context": {
			"name": "保持上下文",
			"tooltip": "此参数已弃用，无任何效果。"
		},
		"prompt": {
			"name": "提示词",
			"tooltip": "模型的文本输入，用于生成响应。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
...
var OpenAIGPTImage1 = {
	...
	"inputs": {
		...
		"custom_height": {
			"name": "custom_height",
			"tooltip": "仅当 `size` 为“自定义”时使用。（仅限 GPT Image 2）必须为 16 的倍数。"
		},
		"custom_width": {
			"name": "custom_width",
			"tooltip": "仅当 `size` 为“自定义”时使用。（仅限 GPT Image 2）必须为 16 的倍数。"
...
var OpenAIGPTImageNodeV2 = {
	...
	"inputs": {
		...
		"model_custom_height": { "name": "自定义高度" },
		"model_custom_width": { "name": "自定义宽度" },
		"model_quality": { "name": "质量" },
		"model_size": { "name": "尺寸" },
		"n": {
			"name": "数量",
			"tooltip": "要生成的图像数量"
...
var OpenAIInputFiles = {
	...
	"inputs": {
		"OPENAI_INPUT_FILES": {
			"name": "OpenAI输入文件",
			"tooltip": "可选的附加文件，与此节点加载的文件一起批处理。允许链式连接输入文件，以便单个消息可包含多个输入文件。"
		},
		"file": {
			"name": "文件",
			"tooltip": "作为模型上下文的输入文件。目前仅接受文本(.txt)和PDF(.pdf)文件。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var OpenAIVideoSora2 = {
	"description": "OpenAI视频和音频生成。",
	...
	"inputs": {
		...
		"prompt": {
			"name": "提示词",
			"tooltip": "引导文本；如果存在输入图像，可为空。"
		},
		"seed": {
			"name": "随机种",
			"tooltip": "确定节点是否应重新运行的种子；无论种子如何，实际结果都是非确定性的。"
		},
		"size": { "name": "尺寸" }
	},
	"outputs": { "0": { "tooltip": null } }
...
var OpenRouterLLMNode = {
	"description": "通过OpenRouter生成文本回复。路由到xAI、DeepSeek、Qwen、Mistral、Z.AI（GLM）、Moonshot（Kimi）、Perplexity Sonar等精选热门模型。",
	"display_name": "OpenRouter LLM",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"model": {
			"name": "model",
			"tooltip": "用于生成回复的OpenRouter模型。"
		},
		"model_reasoning_effort": { "name": "reasoning_effort" },
		"prompt": {
			"name": "prompt",
		...
		"seed": {
			"name": "seed",
			"tooltip": "采样种子。设为0表示不使用。大多数模型仅将其作为参考。"
		},
		"system_prompt": {
			"name": "system_prompt",
			"tooltip": "决定模型行为的基础指令。"
...
var OpticalFlowLoader = {
	...
	"inputs": { "model_name": {
		"name": "model_name",
		"tooltip": "要加载的光流模型。文件必须放在 'optical_flow' 文件夹中。目前仅支持 torchvision 的 raft_large.pth。"
	} },
	"outputs": { "0": { "tooltip": null } }
};
var OptimalStepsScheduler = {
	"display_name": "OptimalSteps调度器",
	"inputs": {
		"denoise": { "name": "去噪" },
		"model_type": { "name": "模型" },
		"steps": { "name": "步数" }
...
var PairConditioningCombine = {
	"display_name": "条件对合并",
	"inputs": {
		"negative_A": { "name": "负面条件_A" },
		"negative_B": { "name": "负面条件_B" },
		"positive_A": { "name": "正面条件_A" },
		"positive_B": { "name": "正面条件_B" }
	},
	"outputs": {
		"0": { "name": "正面条件" },
		"1": { "name": "负面条件" }
...
var PairConditioningSetDefaultCombine = {
	...
	"inputs": {
		"hooks": { "name": "约束" },
		"negative": { "name": "负面条件" },
		"negative_DEFAULT": { "name": "默认负面条件" },
		"positive": { "name": "正面条件" },
		"positive_DEFAULT": { "name": "默认正面条件" }
	},
	"outputs": {
		"0": { "name": "正面条件" },
		"1": { "name": "负面条件" }
...
var PairConditioningSetProperties = {
	...
	"inputs": {
		"hooks": { "name": "约束" },
		"mask": { "name": "遮罩" },
		"negative_NEW": { "name": "新负面条件" },
		"positive_NEW": { "name": "新正面条件" },
		"set_cond_area": { "name": "设置条件区域" },
		"strength": { "name": "强度" },
		"timesteps": { "name": "间隔" }
	},
...
var PatchModelAddDownscale = {
	...
	"inputs": {
		"block_number": { "name": "层编号" },
		"downscale_after_skip": { "name": "跳过后收缩" },
		"downscale_factor": { "name": "收缩系数" },
		"downscale_method": { "name": "收缩算法" },
		"end_percent": { "name": "结束百分比" },
		"model": { "name": "模型" },
		"start_percent": { "name": "开始百分比" },
		"upscale_method": { "name": "放大方法" }
...
var PhotoMakerLoader = {
	...
	"inputs": { "photomaker_model_name": { "name": "photomaker模型名称" } },
	"outputs": { "0": { "tooltip": null } }
};
var PiDConditioning = {
	"description": "为 PiD 解码/放大附加 latent 和 degrade_sigma 标量到 CONDITIONING",
	"display_name": "PiD 条件处理",
	"inputs": {
		"degrade_sigma": {
			"name": "degrade_sigma",
			"tooltip": "0 = 干净 latent。增加以去噪受损 latent 输出。"
		},
		"latent": {
			"name": "latent",
			"tooltip": "latent（来自 VAEEncode 或 KSampler）。"
		},
		"latent_format": {
			"name": "latent 格式",
			"tooltip": "Flux1 和 Flux2 latent 可根据通道维自动检测，sd3 需手动选择。"
		},
		"positive": { "name": "正向" }
	},
	"outputs": { "0": { "tooltip": null } }
...
var PixverseImageToVideoNode = {
	...
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"duration_seconds": { "name": "时长（秒）" },
		"image": { "name": "图像" },
		"motion_mode": { "name": "运动模式" },
		"negative_prompt": {
			"name": "反向提示词",
...
var PolyexponentialScheduler = {
	...
	"inputs": {
		"rho": { "name": "rho" },
		"sigma_max": { "name": "sigma最大值" },
		"sigma_min": { "name": "sigma最小值" },
		"steps": { "name": "步数" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var PorterDuffImageComposite = {
	...
	"inputs": {
		"destination": { "name": "目标图像" },
		"destination_alpha": { "name": "目标图像alpha" },
		"mode": { "name": "模式" },
		"source": { "name": "来源图像" },
...
var Preview3D = {
	"description": "预览 3D 模型文件，无需将其保存到 ComfyUI 输出目录。",
	"display_name": "预览3D",
	"inputs": {
		"bg_image": { "name": "背景图像" },
		"camera_info": { "name": "相机信息" },
		"image": { "name": "图像" },
		"model_file": {
			"name": "模型文件",
...
var Preview3DAdvanced = {
	"description": "预览 3D 模型文件，无需将其保存到 ComfyUI 输出目录。",
	"display_name": "3D 预览（高级）",
	"inputs": {
		"camera_info": { "name": "相机信息" },
		"height": { "name": "高度" },
		"model_3d": {
			"name": "model_3d",
			"tooltip": "来自上游 3D 节点的 3D 模型文件。"
		},
		"model_3d_info": { "name": "3D 模型信息" },
		"viewport_state": { "name": "viewport_state" },
		"width": { "name": "宽度" }
	},
	"outputs": {
...
var PreviewAudio = {
	"description": "预览音频，无需将其保存到 ComfyUI 输出目录。",
	"display_name": "预览音频",
	"inputs": {
		"audio": { "name": "音频" },
		"audioUI": { "name": "音频UI" }
...
var PreviewGaussianSplat = {
	"description": "预览高斯点云 3D 文件，无需将其保存到 ComfyUI 输出目录。",
	"display_name": "预览 Splat",
	"inputs": {
		"camera_info": { "name": "camera_info" },
		"height": { "name": "height" },
		"model_3d": {
			"name": "model_3d",
			"tooltip": "一个高斯 Splat 3D 文件。"
		},
...
var PreviewPointCloud = {
	"description": "预览点云 3D 文件，无需将其保存到 ComfyUI 输出目录。",
	"display_name": "预览点云",
	"inputs": {
		"camera_info": { "name": "camera_info" },
		"height": { "name": "height" },
...
var QuiverImageToSVGNode = {
	"description": "使用Quiver AI将光栅图像矢量化为SVG。",
	"display_name": "Quiver 图像转SVG",
	"inputs": {
		"auto_crop": {
			"name": "auto_crop",
			"tooltip": "自动裁剪到主要主体。"
...
var QuiverTextToSVGNode = {
	"description": "使用Quiver AI根据文本提示生成SVG。",
	"display_name": "Quiver 文本转SVG",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"instructions": {
			"name": "instructions",
		...
		"prompt": {
			"name": "prompt",
			"tooltip": "对期望SVG输出的文本描述。"
		},
		"reference_images": {
			"name": "reference_images",
			"tooltip": "最多可提供4张参考图像以辅助生成。"
		},
		"seed": {
			"name": "seed",
			"tooltip": "用于决定节点是否重新运行的种子；无论种子如何，实际结果都是非确定性的。"
...
var RTDETR_detect = {
	"display_name": "RT-DETR 检测",
	"inputs": {
		"class_name": {
			"name": "类别名称",
			"tooltip": "按类别筛选检测结果。设置为 'all' 可禁用筛选。"
		},
		"image": { "name": "图像" },
		"max_detections": {
			"name": "最大检测数",
			"tooltip": "每张图像返回的最大检测数量，按置信度从高到低排序。"
		},
		"model": { "name": "模型" },
		"threshold": { "name": "阈值" }
	},
...
var RandomCropImages = {
	"description": "随机裁剪图像为指定尺寸。",
	"display_name": "裁剪图像（随机）",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"height": {
			"name": "高度",
...
var RebatchLatents = {
	...
	"inputs": {
		"batch_size": { "name": "批量大小" },
		"latents": { "name": "Latent因素" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var RecordAudio = {
	"display_name": "录制音频",
	"inputs": { "audio": { "name": "音频" } },
	"outputs": { "0": { "tooltip": null } }
};
var RecraftColorRGB = {
...
var ReferenceLatent = {
	"description": "此节点为编辑模型设置引导潜在空间。如果模型支持，您可以链式连接多个以设置多个参考图像。",
...
var RegexExtract = {
	...
	"inputs": {
		"case_insensitive": { "name": "忽略大小写" },
		"dotall": { "name": "点号匹配所有" },
		"group_index": { "name": "分组索引" },
		"mode": { "name": "模式" },
		"multiline": { "name": "多行模式" },
		"regex_pattern": { "name": "正则表达式模式" },
		"string": { "name": "字符串" }
	},
	"outputs": { "0": { "tooltip": null } }
};
...
var RegexReplace = {
	"description": "使用正则表达式模式查找和替换文本。",
	"display_name": "正则表达式替换",
	"inputs": {
		"case_insensitive": { "name": "忽略大小写" },
		"count": {
			"name": "计数",
			"tooltip": "最大替换次数。设置为0可替换所有匹配项（默认）。设置为1仅替换第一个匹配项，2替换前两个匹配项，依此类推。"
		},
		"dotall": {
			"name": "点号匹配所有",
			"tooltip": "启用时，点号（.）字符将匹配包括换行符在内的任何字符。禁用时，点号不会匹配换行符。"
		},
		"multiline": { "name": "多行模式" },
		"regex_pattern": { "name": "正则表达式模式" },
		"replace": { "name": "替换内容" },
...
var RemoveBackground = {
	"description": "使用背景移除模型生成前景mask，从图像中移除背景。",
	"display_name": "移除背景",
	"inputs": {
		"bg_removal_model": {
			"name": "背景移除模型",
...
var RenderSplat = {
	"description": "使用各向异性 EWA 光栅化器将高斯 splat 渲染为图像（定向椭圆 splat，抗锯齿，按深度从前到后排序）。相机来自 camera_info 输入（加载 / 预览 3D，或创建相机信息节点）；留空则自动框选 splat。将帧数设置为大于 1，可生成转台批量图像以供视频节点使用。",
	"display_name": "渲染 Splat",
	"inputs": {
		"background": { "name": "背景" },
		"bg_image": {
			"name": "背景图像",
			"tooltip": "可选的背景底图，合成在 splat 背后（覆盖纯色背景）。会调整为渲染尺寸；批量渲染时每帧一张，单张图像则全部使用。仅适用于 color/clay。"
		},
		"camera_info": {
			"name": "camera_info",
			"tooltip": "渲染所用相机 - 来自 Load3D / Preview3D 相机或创建相机信息节点。留空则自动从默认 3/4 视角框选 splat。"
		},
		"frames": {
			"name": "帧数",
			"tooltip": "-1、0、1 = 单张静态图像；>1 = 转台，摄像机围绕 360 度旋转（适用于任何 camera_info）。负值则反向旋转。"
		},
		"headlight_shading": {
			"name": "前照灯光照",
			"tooltip": "来自摄像机位置的光源（前照灯）进行漫反射光照，利用 splat 表面法线：转离视角的表面会变暗以突出形状/曲率。0 = 纯色反照率，1 = 最强光照。"
		},
		"height": { "name": "高度" },
		"opacity_threshold": {
			"name": "不透明度阈值",
			"tooltip": "剔除不透明度低于此值的高斯（去除微弱漂浮点）。"
		},
		"render_style": {
			"name": "渲染风格",
			"tooltip": "图像输出内容：color（颜色）、clay（中性反照率着色）、depth（近=亮）、normal（OpenGL 法线贴图）。"
		},
		"sharpen": {
			"name": "锐化",
			"tooltip": "锐化重叠的 splat：1.0 = 物理正确混合；更高值会让每个像素更偏向其主导（最近）的 splat，获得更清晰的纹理，但不会缩小 splat 或产生空隙。大于 1 时为非物理效果。"
		},
		"splat": { "name": "splat" },
		"splat_scale": {
			"name": "splat 缩放",
			"tooltip": "每个 splat 投影面积的倍数（数值越小点越锐利，数值越大表面越柔和/饱满）。"
		},
		"width": { "name": "宽度" }
	},
	"outputs": {
...
var ReplaceVideoLatentFrames = {
	"display_name": "替换视频Latent",
	"inputs": {
		"destination": {
			"name": "目标Latent",
			"tooltip": "需要被替换的 Latent"
		},
		"index": {
			"name": "序号",
			"tooltip": "选择需要被替换的 Latent 的序号，负值表示从后往前数。"
		},
		"source": {
			"name": "来源Latent",
			"tooltip": "用于替换的 Latent，如果没有则不会替换。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
...
var ResizeAndPadImage = {
	"display_name": "调整尺寸并填充图像",
	"inputs": {
		"image": { "name": "图像" },
		"interpolation": { "name": "插值方法" },
		"padding_color": { "name": "填充颜色" },
		"target_height": { "name": "目标高度" },
		"target_width": { "name": "目标宽度" }
	},
	"outputs": { "0": { "tooltip": null } }
...
var ResizeImageMaskNode = {
	...
	"inputs": {
		...
		"resize_type_crop": { "name": "裁剪" },
		"resize_type_height": { "name": "高度" },
		"resize_type_width": { "name": "宽度" },
		"scale_method": {
			"name": "缩放方法",
...
var ResizeImagesByLongerEdge = {
	"description": "调整图像尺寸，使较长边匹配指定长度，同时保持宽高比。",
	"display_name": "缩放图像（长边）",
	"inputs": {
		"images": {
			"name": "图像",
			"tooltip": "需要处理的图像序列。"
...
var ResizeImagesByShorterEdge = {
	"description": "调整图像尺寸，使较短边与指定尺寸相同，同时保持宽高比。",
...
var ResolutionBucket = {
	"description": "将latent和conditioning分组到不同的桶中",
	"display_name": "分辨率 Bucket",
	"inputs": {
		"conditioning": {
			"name": "条件",
			"tooltip": "条件序列（必须和 Latent 序列数量相同）"
		},
		"latents": {
			"name": "Latent",
			"tooltip": "需要 bucket 的 Latent 字典序列"
		}
	},
	"outputs": {
		"0": {
			"name": "Latent",
			"tooltip": "Latent 字典，按分辨率分批。"
		},
		"1": {
			"name": "条件",
			"tooltip": "条件字典，按分辨率分批。"
...
var Rodin3D_Detail = {
	...
	"inputs": {
		"Images": { "name": "图像" },
		"Material_Type": { "name": "材质类型" },
		"Polygon_count": { "name": "多边形数量" },
		"Seed": { "name": "种子" }
	},
	"outputs": {
		"0": {
...
var Rodin3D_Gen25_Image = {
	"description": "通过 Rodin Gen-2.5 从 1-5 张参考图像生成 3D 模型。选择模式（快速 / 常规 / 极高）以平衡质量与成本。",
	"display_name": "Rodin 3D Gen-2.5 - 图像转3D",
	"inputs": {
		"TAPose": {
			"name": "T/A 姿势",
			"tooltip": "用于类人模型的 T/A 姿势。"
		},
		"addon_highpack": {
			"name": "HighPack 插件",
			"tooltip": "HighPack 插件：4K 贴图和 Quad 模式下约 16 倍面数。"
		},
		"bbox_height": {
			"name": "包围盒高度",
			"tooltip": "包围盒高度（Z 轴）。"
		...
		"bbox_width": {
			"name": "包围盒宽度",
			"tooltip": "包围盒宽度（Y 轴）。与其他设置为 0 可跳过包围盒。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"geometry_file_format": { "name": "几何文件格式" },
		"hd_texture": {
			"name": "高清贴图",
			"tooltip": "高质量贴图增强。"
		},
		"height_cm": {
			"name": "模型高度（厘米）",
			"tooltip": "模型大致高度（厘米，0 表示跳过）。"
		},
		"images": {
			"name": "图像",
			"tooltip": "1-5 张图像。多视角时第一张图像用于材质。"
		},
		"material": { "name": "材质" },
		"mode": {
			"name": "模式",
			"tooltip": "生成模式。常规 = 平衡。快速 = 1K-20K 面，适合快速原型。极高 = 20K-200 万面，可选微细节。"
		},
		"mode_creative": { "name": "创意" },
		"mode_polygon_count": { "name": "面数" },
		"mode_tier": { "name": "档位" },
		"seed": { "name": "种子" },
		"texture_delight": {
			"name": "贴图去光照",
		...
		"texture_mode": {
			"name": "贴图模式",
			"tooltip": "贴图质量预设。'默认'使用服务器为所选档位设置的默认值。"
		},
		"use_original_alpha": {
			"name": "保留原始透明度",
			"tooltip": "保留图像透明通道。"
...
var Rodin3D_Gen25_Text = {
	"description": "通过 Rodin Gen-2.5 根据文本提示生成3D模型。选择模式（快速 / 常规 / 极高）以平衡质量与成本。",
	"display_name": "Rodin 3D Gen-2.5 - 文本转3D",
	"inputs": {
		"TAPose": {
			"name": "T/A 姿势",
			"tooltip": "用于类人模型的T/A姿势。"
		},
		"addon_highpack": {
			"name": "HighPack 插件",
			"tooltip": "HighPack 插件：4K贴图和Quad模式下约16倍面数。"
		},
		"bbox_height": {
			"name": "包围盒高度",
			"tooltip": "包围盒高度（Z轴）。"
		...
		"bbox_width": {
			"name": "包围盒宽度",
			"tooltip": "包围盒宽度（Y轴）。与其他设置为0可跳过包围盒。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"geometry_file_format": { "name": "几何文件格式" },
		"hd_texture": {
		...
		"height_cm": {
			"name": "模型高度（厘米）",
			"tooltip": "模型近似高度（厘米，0为跳过）。"
		},
		"material": { "name": "材质" },
		"mode": {
			"name": "模式",
			"tooltip": "生成模式。常规 = 平衡。快速 = 1K-20K面数，适合快速原型。极高 = 20K-200万面数，可选微细节。"
		},
		"mode_creative": { "name": "创意" },
		"mode_polygon_count": { "name": "面数" },
		"mode_tier": { "name": "档位" },
		...
		"texture_mode": {
			"name": "贴图模式",
			"tooltip": "贴图质量预设。“默认”将使用所选档位的服务器默认值。"
		}
	},
	"outputs": { "0": {
		"name": "模型文件",
...
var RunwayAleph2KeyframeNode = {
	"description": "将引导图像锚定到输入（源）视频的某一时刻，使 Aleph2 在该时刻引导你的素材编辑。将此节点连接到 Runway Aleph2 视频转视频节点的 'keyframes' 输入；可通过下方可选的 'keyframes' 输入串联多个（最多 5 个）。",
	"display_name": "Runway Aleph2 关键帧",
	"inputs": {
		"image": {
			"name": "image",
			"tooltip": "要在输入视频选定时刻应用的引导图像。"
		},
		"keyframes": {
			"name": "keyframes",
			"tooltip": "可选的前置关键帧，与此关键帧串联。"
		},
		"timing": {
			"name": "timing",
			"tooltip": "如何将该图像放置在输入视频的时间线上。"
		},
...
var RunwayAleph2PromptImageNode = {
	"description": "将引导图像锚定到输出（结果）视频的某一时刻，以引导编辑后视频在该时刻的外观。将此节点连接到 Runway Aleph2 视频转视频节点的 'prompt_images' 输入；可通过下方可选的 'prompt_images' 输入串联多个（最多 5 个）。",
	"display_name": "Runway Aleph2 提示图像",
	"inputs": {
		"image": {
			"name": "image",
			"tooltip": "要在输出视频选定时刻放置的引导图像。"
		},
		"position": {
			"name": "position",
			"tooltip": "如何将该图像放置在输出视频的时间线上。"
...
var RunwayAleph2VideoToVideoNode = {
	"description": "使用 Runway 的 Aleph2 模型通过文本提示编辑视频。Aleph2 可在保持原始运动和时序的同时，转换您的素材（重新风格化、重新打光、添加或移除元素、更改视角）；输出分辨率与输入视频一致，输入视频需为 2-30 秒，帧率不高于 30 fps。可选地通过关键帧（锚定到输入视频）或提示图像（锚定到输出视频）引导编辑——二者只能选其一，不可同时使用。",
	"display_name": "Runway Aleph2 视频转视频",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"keyframes": {
			"name": "关键帧",
			"tooltip": "锚定到输入视频的引导图像，来自 Aleph2 Keyframe 节点（最多 5 个）。只能使用关键帧或提示图像，不能同时使用。"
		},
		"prompt": {
			"name": "提示词",
			"tooltip": "描述输出中应出现的内容（1-1000 字符）。"
		},
		"prompt_images": {
			"name": "提示图像",
			"tooltip": "锚定到输出视频的引导图像，来自 Aleph2 Prompt Image 节点（最多 5 个）。只能使用关键帧或提示图像，不能同时使用。"
		},
		"public_figure_threshold": {
			"name": "公众人物阈值",
			"tooltip": "对可识别公众人物的内容进行审核。"
		},
		"seed": {
			"name": "随机种子",
			"tooltip": "生成用的随机种子"
		...
		"video": {
			"name": "视频",
			"tooltip": "要编辑的输入视频。需为 2-30 秒，帧率不高于 30 fps。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var RunwayFirstLastFrameNode = {
	...
	"display_name": "Runway首尾帧转视频",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"duration": { "name": "时长" },
		"end_frame": {
			"name": "结束帧",
			"tooltip": "用于视频的结束帧。仅支持gen3a_turbo。"
		},
		"prompt": {
			"name": "提示词",
			"tooltip": "生成的文本提示词"
...
var RunwayImageToVideoNodeGen3a = {
	"description": "使用Gen3a Turbo模型从单个起始帧生成视频。开始前，请查看这些最佳实践以确保您的输入选择能为生成成功奠定基础：https://help.runwayml.com/hc/en-us/articles/33927968552339-Creating-with-Act-One-on-Gen-3-Alpha-and-Turbo。",
	"display_name": "Runway图像转视频（Gen3a Turbo）",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"duration": { "name": "时长" },
		"prompt": {
...
var RunwayImageToVideoNodeGen4 = {
	"description": "使用Gen4 Turbo模型从单个起始帧生成视频。开始前，请查看这些最佳实践以确保您的输入选择能为生成成功奠定基础：https://help.runwayml.com/hc/en-us/articles/37327109429011-Creating-with-Gen-4-Video。",
	"display_name": "Runway图像转视频（Gen4 Turbo）",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"duration": { "name": "时长" },
		"prompt": {
...
var RunwayTextToImageNode = {
	"description": "使用 Runway 的 Gen 4 模型从文本提示生成图像。您还可以包含参考图像来引导生成过程。",
	"display_name": "Runway 文生图",
	"inputs": {
		"prompt": {
			"name": "提示词",
			"tooltip": "用于生成的文本提示"
		...
		"reference_image": {
			"name": "参考图像",
			"tooltip": "用于引导生成的可选参考图像"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var SAM3_Detect = {
	"display_name": "SAM3 检测",
	"inputs": {
		"bboxes": {
			"name": "边界框",
			"tooltip": "要分割的边界框"
		...
		"conditioning": {
			"name": "条件",
			"tooltip": "来自 CLIPTextEncode 的文本条件"
		},
		"image": { "name": "图像" },
		"individual_masks": {
			"name": "单独 mask",
			"tooltip": "输出每个对象的 mask，而不是合并"
		},
		"model": { "name": "model" },
		"negative_coords": {
			"name": "负向坐标",
			"tooltip": "负向点提示，JSON 格式 [{\"x\": int, \"y\": int}, ...]（像素坐标）"
		},
		"positive_coords": {
			"name": "正向坐标",
			"tooltip": "正向点提示，JSON 格式 [{\"x\": int, \"y\": int}, ...]（像素坐标）"
		},
		"refine_iterations": {
			"name": "细化迭代次数",
			"tooltip": "SAM 解码器细化次数（0=使用原始检测器 mask）"
		},
		"threshold": { "name": "阈值" }
	},
	"outputs": {
...
var SAM3_TrackPreview = {
	"display_name": "SAM3 跟踪预览",
	"inputs": {
		"fps": { "name": "帧率" },
		"images": { "name": "图像" },
		"opacity": { "name": "不透明度" },
		"track_data": { "name": "跟踪数据" }
	}
};
var SAM3_TrackToMask = {
	"display_name": "SAM3 跟踪转 mask",
	"inputs": {
		"object_indices": {
			"name": "对象索引",
			"tooltip": "要包含的对象索引（用逗号分隔，例如 '0,2,3'）。为空表示所有对象。"
		},
		"track_data": { "name": "跟踪数据" }
	},
	"outputs": { "0": {
...
var SAM3_VideoTrack = {
	"display_name": "SAM3 视频跟踪",
	"inputs": {
		"conditioning": {
			"name": "条件",
			"tooltip": "跟踪过程中检测新对象的文本条件"
		},
		"detect_interval": {
			"name": "检测间隔",
			"tooltip": "每 N 帧运行一次检测（1=每帧）。数值越大越省算力。"
		},
		"detection_threshold": {
			"name": "检测阈值",
			"tooltip": "文本提示检测的分数阈值"
		...
		"initial_mask": {
			"name": "初始 mask",
			"tooltip": "第一帧要跟踪的 mask（每个对象一个）"
		},
		"max_objects": {
			"name": "最大对象数",
			"tooltip": "最大跟踪对象数（0=无限制）。初始 mask 计入此限制。"
		},
		"model": { "name": "model" }
	},
	"outputs": { "0": {
...
var SCAIL2ColoredMask = {
	"display_name": "创建 SCAIL-2 彩色掩码",
	"inputs": {
		"driving_track_data": {
			"name": "驱动轨迹数据",
			"tooltip": "驱动姿态视频的SAM3轨迹。将渲染到pose_video_mask输出。"
		},
		"object_indices": {
			"name": "对象索引",
			"tooltip": "要包含的人物索引，逗号分隔（如“0,2,3”）。应用于参考和姿态视频掩码。留空=全部。"
		},
		"ref_track_data": {
			"name": "参考轨迹数据",
			"tooltip": "参考图像的SAM3轨迹。"
		...
		"replacement_mode": {
			"name": "替换模式",
			"tooltip": "False=mask_video为黑色背景（动画模式）。True=白色背景（替换模式）。在WanSCAILToVideo上设置匹配的replacement_mode。reference_image_mask始终为黑色背景。"
		},
		"sort_by": {
			"name": "排序方式",
			"tooltip": "为跟踪对象分配调色板颜色的顺序（应用于参考和姿态视频，每个身份保持相同颜色）。left_to_right=最左侧对象（按首帧质心）获得第一种颜色；area=最大对象（按首帧掩码面积）获得第一种颜色；none=保持SAM3的顺序。"
		}
	},
	"outputs": {
		"0": {
...
var SUPIRApply = {
	...
	"inputs": {
		...
		"restore_cfg": {
			"name": "restore_cfg",
			"tooltip": "将去噪输出拉向输入 latent。数值越高，对输入的保真度越强。0 表示禁用。"
		},
		"restore_cfg_s_tmin": {
			"name": "restore_cfg_s_tmin",
			"tooltip": "低于该 sigma 阈值时，restore_cfg 被禁用。"
		},
		"strength_end": {
			"name": "strength_end",
			"tooltip": "控制采样结束时（低 sigma）的强度。从开始值线性插值。"
		},
		"strength_start": {
			"name": "strength_start",
			"tooltip": "控制采样开始时（高 sigma）的强度。"
		},
		"vae": { "name": "vae" }
	},
	"outputs": { "0": { "tooltip": null } }
...
var SV3D_Conditioning = {
	...
	"inputs": {
		...
		"elevation": { "name": "俯仰角" },
		"height": { "name": "高度" },
		"init_image": { "name": "初始图像" },
		"vae": { "name": "vae" },
		"video_frames": { "name": "帧数" },
		"width": { "name": "宽度" }
	},
	"outputs": {
		"0": {
...
var SVD_img2vid_Conditioning = {
	...
	"inputs": {
		"augmentation_level": { "name": "增强" },
		"clip_vision": { "name": "clip视觉" },
		"fps": { "name": "帧率" },
		"height": { "name": "高度" },
		"init_image": { "name": "初始图像" },
		"motion_bucket_id": { "name": "动态bucketID" },
		"vae": { "name": "vae" },
		"video_frames": { "name": "帧数" },
		"width": { "name": "宽度" }
	},
...
var SamplerARVideo = {
	"display_name": "采样器 AR 视频",
	"inputs": { "num_frame_per_block": {
		"name": "num_frame_per_block",
		"tooltip": "每个自回归块的帧数。1 = 按帧，3 = 按块。必须与 checkpoint 的训练模式一致。"
	} },
	"outputs": { "0": { "tooltip": null } }
};
var SamplerCustom = {
...
var SamplerDPMPP_2M_SDE = {
	...
	"inputs": {
		"eta": { "name": "eta" },
		"noise_device": { "name": "设备" },
		"s_noise": { "name": "s_noise" },
		"solver_type": { "name": "求解器类型" }
	},
	"outputs": { "0": { "tooltip": null } }
};
...
var SamplerER_SDE = {
	"display_name": "ER_SDE采样器",
	"inputs": {
		"eta": {
			"name": "eta",
			"tooltip": "反向时间 SDE 的随机强度。\n当 eta=0 时，简化为确定性 ODE。此设置不适用于 ER-SDE 求解器类型。"
		},
		"max_stage": { "name": "最大阶段" },
		"s_noise": { "name": "s_noise" },
		"solver_type": { "name": "求解器" }
	},
	"outputs": { "0": { "tooltip": null } }
};
...
var SamplerLCMUpscale = {
	...
	"inputs": {
		"scale_ratio": { "name": "缩放比例" },
		"scale_steps": { "name": "缩放步数" },
		"upscale_method": { "name": "缩放方法" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var SamplerLMS = {
	"display_name": "LMS采样器",
	"inputs": { "order": { "name": "顺序" } },
	"outputs": { "0": { "tooltip": null } }
};
var SamplerSASolver = {
	"display_name": "SASolver采样器",
	"inputs": {
		"corrector_order": { "name": "校正器阶数" },
		"eta": { "name": "eta" },
		"model": { "name": "模型" },
		"predictor_order": { "name": "预测器阶数" },
		"s_noise": { "name": "s_noise" },
		"sde_end_percent": { "name": "SDE 结束百分比" },
		"sde_start_percent": { "name": "SDE 起始百分比" },
		"simple_order_2": { "name": "简单二阶" },
		"use_pece": { "name": "使用 PECE" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var SamplerSEEDS2 = {
	"description": "该采样器节点可以表示多个采样器：\n\nseeds 2\n-默认设置\n\nexp_heun_2_x0\n- 求解器=phi 2, r=1.0, eta=0.0\n\nexp_heun_2_x0_sde\n- 求解器=phi 2, r=1.0, eta=1.0, s noise=1.0",
	"display_name": "SEEDS2采样器",
	"inputs": {
		"eta": {
			"name": "eta",
...
var SamplingPercentToSigma = {
	"display_name": "采样比到Sigma",
	"inputs": {
		"model": { "name": "模型" },
		"return_actual_sigma": {
			"name": "返回实际 sigma 值",
			"tooltip": "返回实际的 sigma 值而不是用于区间检查的值。\n这仅影响 0.0 和 1.0 处的结果。"
		},
		"sampling_percent": { "name": "采样百分比" }
	},
	"outputs": { "0": {
		"name": "sigma 值",
		"tooltip": null
...
var SaveAudioAdvanced = {
	...
	"inputs": {
		...
		"filename_prefix": {
			...
			"tooltip": "要保存文件的前缀。可包含格式化标记，如 %date:yyyy-MM-dd%。"
		},
		"format": {
			"name": "format",
			"tooltip": "保存音频的文件格式。"
...
var SaveImageAdvanced = {
	"description": "将输入图像保存到您的 ComfyUI 输出目录。",
	"display_name": "保存图像（高级）",
	"inputs": {
		"filename_prefix": {
			"name": "文件名前缀",
			"tooltip": "要保存文件的前缀。可包含格式化标记，如 %date:yyyy-MM-dd% 或 %Empty Latent Image.width%。"
		},
		"format": {
			"name": "格式",
			"tooltip": "保存图像的文件格式。"
		},
		"format_bit_depth": { "name": "位深度" },
		"format_input_color_space": { "name": "输入色彩空间" },
		"images": {
			"name": "图像",
			"tooltip": "要保存的图像。"
		}
...
var SaveImageDataSetToFolder = {
	"description": "将图像数据集保存到指定文件夹。支持的格式：PNG。",
	"display_name": "保存图像数据集",
	"inputs": {
		"filename_prefix": {
			"name": "文件名前缀",
			"tooltip": "保存的图像的文件名前缀。"
		...
		"folder_name": {
			"name": "文件夹名",
			"tooltip": "保存训练数据集的文件夹名（在 输出 文件夹内）。"
		},
		"images": {
			"name": "图像",
			"tooltip": "需要保存的图像序列。"
		...
		"mode": {
			"name": "模式",
			"tooltip": "选择覆盖已有文件，或通过递增文件名来避免覆盖。"
		}
	}
};
var SaveImageTextDataSetToFolder = {
	"description": "将图像和文本描述对的数据集保存到指定文件夹。图像以PNG格式保存，描述以与文件名前缀相同的TXT文件保存。",
	"display_name": "保存图像文本数据集",
	"inputs": {
		"filename_prefix": {
			"name": "文件名前缀",
			"tooltip": "保存的图像的文件名前缀。"
...
var SaveLoRA = {
	"display_name": "保存 LoRA",
	"inputs": {
		"lora": {
			"name": "LoRA",
			"tooltip": "需要保存的 LoRA 模型。不要使用分层 LoRA。"
		},
		"prefix": {
			"name": "前缀",
			"tooltip": "保存的 LoRA 文件的文件名前缀。"
		},
		"steps": {
			"name": "步数",
			"tooltip": "可选：LoRA 的训练步数，用于保存文件命名。"
		}
	}
};
var SavePointCloud = {
...
var SaveSVGNode = {
	...
	"inputs": {
		"filename_prefix": {
			"name": "文件名前缀",
			"tooltip": "保存文件的前缀。可包含格式化信息，如 %date:yyyy-MM-dd% 或 %Empty Latent Image.width% 以包含节点中的值。"
		},
		"svg": { "name": "svg" }
	},
	"outputs": { "0": {
...
var SaveTrainingDataset = {
	"description": "将编码后的训练数据集（latent + conditioning）保存到磁盘，以便训练时高效加载。",
	"display_name": "保存训练数据集",
	"inputs": {
		"conditioning": {
			"name": "条件",
			"tooltip": "制作训练数据集 节点输出的条件字典"
		},
		"folder_name": {
			"name": "文件夹名",
			"tooltip": "包含训练数据集的文件夹名（在 输出 文件夹内）。"
		...
		"latents": {
			"name": "Latent",
			"tooltip": "制作训练数据集 节点输出的 Latent 字典"
		},
		"shard_size": {
			"name": "shard_size",
...
var SaveWEBM = {
	...
	"inputs": {
		...
		"images": {
			"name": "图像",
			"tooltip": "RGBA 图像将以 alpha 通道作为透明度保存（仅 vp9 编解码器支持）。"
		}
	},
	"outputs": { "0": {
		"name": "images",
...
var SeedVR2Conditioning = {
	"description": "从 VAE latent 构建 SeedVR2 正/负条件。",
	"display_name": "应用 SeedVR2 条件",
	"inputs": {
		"model": {
			"name": "model",
			"tooltip": "SeedVR2 模型。"
...
var SeedVR2PostProcessing = {
	"description": "将生成的图像与原始调整尺寸后的图像对齐，并应用颜色校正。",
	"display_name": "后处理 SeedVR2 输出",
	"inputs": {
		"color_correction_method": {
			"name": "color_correction_method",
			"tooltip": "将生成图像颜色与原始图像匹配的方法。lab：在 CIELAB 空间中转移颜色，保留细节（最忠实）。wavelet：转移低频颜色，保留高分辨率细节。adain：匹配每通道均值/标准差（最快，整体色调）。none：跳过颜色转移（仅几何对齐）。"
		},
		"images": {
			"name": "images",
			"tooltip": "要处理的生成图像。"
...
var SeedVR2Preprocess = {
	"description": "为 SeedVR2 模型填充调整尺寸后的图像。Alpha 通道会被移除。节点“后处理 SeedVR2 输出”会从原始调整尺寸图像重新应用 Alpha 通道。",
	"display_name": "预处理 SeedVR2 输入",
	...
	"outputs": { "0": {
		"name": "images",
		"tooltip": "用于 VAE 编码的填充图像。"
	} }
};
var SeedVR2TemporalChunk = {
	"description": "将 SeedVR2 视频 latent 分割为重叠的时间块，每次采样一个块以适应 VRAM，将 latent 输出分别连接到“应用 SeedVR2 条件”和采样器 latent 输入，最后通过“合并 SeedVR2 latent”重新组合。",
	"display_name": "分割 SeedVR2 latent",
	"inputs": {
		"chunking_mode": {
			"name": "chunking_mode",
			"tooltip": "manual = 精确使用 frames_per_chunk；auto = 预测可适应空闲 VRAM 的最大块。"
		},
		"latent": {
			"name": "latent",
			"tooltip": "要分割的 VAE 编码 SeedVR2 latent。"
		},
		"temporal_overlap": {
			"name": "temporal_overlap",
			"tooltip": "相邻块之间共享的 latent 帧，在合并时进行交叉淡入；0 = 无重叠。"
		}
	},
	"outputs": {
		"0": {
		...
		"1": {
			"name": "temporal_overlap",
			"tooltip": "相邻块之间实际的 latent 帧重叠量，用于“合并 SeedVR2 latent”。"
		}
	}
};
var SeedVR2TemporalMerge = {
	"description": "将采样后的 SeedVR2 latent 时间块重新组合为一个 latent，使用 Hann 窗对每个重叠部分进行交叉淡入，窗口大小由“分割 SeedVR2 latent”传入的 temporal_overlap 决定。",
	"display_name": "合并 SeedVR2 latent",
	"inputs": {
		"latents": {
			"name": "latents",
			"tooltip": "按顺序排列的采样时间块。"
		...
		"temporal_overlap": {
			"name": "temporal_overlap",
			"tooltip": "“分割 SeedVR2 latent”的 temporal_overlap 输出。0 = 直接拼接。"
		}
	},
	"outputs": { "0": {
		"name": "latent",
		"tooltip": "重新组合后的完整 latent。"
	} }
};
var SelectCLIPDevice = {
	"description": "将 CLIP 文本编码器放置在指定设备上（default / cpu / gpu:N）。\n\n- “default” 恢复加载器分配的设备。\n- “cpu” 将加载和卸载设备都固定为 CPU。\n- “gpu:N” 将加载设备固定为第 N 个可用 GPU。\n\n当所选设备在当前机器上不存在时\n（例如在 2-GPU 机器上构建的工作流在 1-GPU 机器上打开），\n节点会直接传递 CLIP 并记录消息，而不会报错。",
	"display_name": "选择 CLIP 设备",
	"inputs": {
		"clip": { "name": "clip" },
		"device": { "name": "device" }
	},
...
var SelectModelDevice = {
	"description": "将扩散模型放置在指定设备上（default / cpu / gpu:N）。\n\n- “default” 恢复加载器分配的设备（即使在之前调用过选择模型设备后）。\n- “cpu” 将加载和卸载设备都固定为 CPU。\n- “gpu:N” 将加载设备固定为第 N 个可用 GPU；卸载设备恢复为加载器的原始选择。\n\n当请求的设备与输入模型当前所在设备不同时，会通过加载器的 reload factory（cached_patcher_init）生成一个新的模型实例，使新的 patcher 在新设备上拥有独立权重。不支持多 GPU 的加载器（无 factory）会导致节点直接传递且发出警告。\n\n如果工作流已应用多GPU CFG 拆分，且所选 GPU 与现有多 GPU 克隆冲突，则该克隆会被移除，避免两个 patcher 绑定到同一设备。\n\n当所选设备在当前机器上不存在时\n（例如在 2-GPU 机器上构建的工作流在 1-GPU 机器上打开），\n节点会直接传递模型并记录消息，而不会报错。\n\n注意：不建议在已被其他节点消费过的模型之后放置选择模型设备节点（例如 KSampler 已在原设备上运行过该模型）——如果所选设备与原设备一致（快速路径），则会观察到之前消费者对原模型的状态更改。请在任何模型消费者之前放置选择模型设备节点。",
	"display_name": "选择模型设备",
	"inputs": {
		"device": { "name": "device" },
		"model": { "name": "model" }
...
var SelectVAEDevice = {
	"description": "将VAE放置在指定设备上（default / gpu:N）。\n\n- “default” 恢复加载器分配的设备。\n- “gpu:N” 将加载设备固定到第N个可用GPU；卸载设备设置为标准VAE卸载设备。\n\nVAE在UI中故意不暴露CPU选项；如果工作流仍然提供了“cpu”（例如从其他机器打开），请求会被记录日志并丢弃，VAE保持不变。\n\n当所选设备在当前机器上不存在时（例如在2-GPU机器上构建的工作流在1-GPU机器上打开），节点会直接传递VAE并记录日志，而不会报错。",
	"display_name": "选择VAE设备",
	"inputs": {
		"device": { "name": "设备" },
		"vae": { "name": "vae" }
	},
...
var SetClipHooks = {
	...
	"inputs": {
		"apply_to_conds": { "name": "应用于条件" },
		"clip": { "name": "clip" },
		"hooks": { "name": "约束" },
		"schedule_clip": { "name": "安排clip" }
	}
};
var SetFirstSigma = {
...
var ShuffleDataset = {
	"description": "随机打乱列表中图像的顺序。",
	"display_name": "打乱图像数据集",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"images": {
			"name": "图像",
...
var ShuffleImageTextDataset = {
	"description": "随机打乱列表中图像-文本对的顺序。",
	"display_name": "打乱图像文本数据集",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"images": {
			"name": "图像",
...
var ShuffleVideoTextDataset = {
	"description": "随机打乱视频-文本对的顺序。",
	"display_name": "打乱视频-文本对",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"seed": {
			"name": "随机种子",
...
var SkipLayerGuidanceDiT = {
	...
	"inputs": {
		"double_layers": { "name": "双层" },
		"end_percent": { "name": "结束百分比" },
		"model": { "name": "模型" },
		"rescaling_scale": { "name": "重新缩放比例" },
		"scale": { "name": "缩放" },
		"single_layers": { "name": "单层" },
		"start_percent": { "name": "开始百分比" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var SkipLayerGuidanceDiTSimple = {
	"description": "跳过层引导（DiT）节点的简化版本，仅修改无条件传递。",
	"display_name": "跳过层引导（DiT简化）",
	"inputs": {
		"double_layers": { "name": "双层" },
		"end_percent": { "name": "结束百分比" },
		"model": { "name": "模型" },
...
var SoniloTextToMusic = {
	"description": "使用 Sonilo 的AI模型根据文本提示生成音乐。将时长设置为0可让模型根据提示自动推断时长。",
	"display_name": "Sonilo 文本生成音乐",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"duration": {
			"name": "duration",
			"tooltip": "目标时长（秒）。设置为0可让模型根据提示自动推断时长。最大时长：6分钟。"
		},
		"prompt": {
			"name": "prompt",
			"tooltip": "描述要生成音乐的文本提示。"
		},
		"seed": {
			"name": "seed",
			"tooltip": "用于结果复现的种子。目前Sonilo服务会忽略此项，但为保持流程一致性而保留。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var SoniloVideoToMusic = {
	"description": "使用 Sonilo 的AI模型根据视频内容生成音乐。分析视频并创作匹配的音乐。",
	"display_name": "Sonilo 视频生成音乐",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"prompt": {
			"name": "prompt",
			"tooltip": "可选文本提示，用于引导音乐生成。留空可获得最佳质量——模型将完全分析视频内容。"
		},
		"seed": {
			"name": "seed",
			"tooltip": "用于结果复现的种子。目前Sonilo服务会忽略此项，但为保持流程一致性而保留。"
		...
		"video": {
			"name": "video",
			"tooltip": "输入要生成音乐的视频。最大时长：6分钟。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var SplatToFile3D = {
	"description": "将高斯 splat 序列化为 File3D 对象，供保存 / 预览 3D 节点使用。仅支持每批次一个项目。",
	"display_name": "创建 3D 文件（由 Splat）",
	"inputs": {
		"format": {
			"name": "格式",
			"tooltip": "ply：标准 3D 高斯 Splat，带完整球谐。ksplat：mkkellogg SplatBuffer（0 级，未压缩），仅基色。spz：Niantic gzip 压缩（约小 10 倍），仅基色。"
		},
		"splat": { "name": "splat" }
	},
	"outputs": { "0": {
...
var SplatToMesh = {
	"description": "从高斯 splat 中提取带颜色的网格。",
	"display_name": "从 Splat 提取网格",
	"inputs": {
		"color_sharpen": {
			"name": "颜色锐化",
			"tooltip": "增强顶点纹理清晰度：1.0=物理正确混合；更高值会让每个体素颜色更偏向主导高斯而非邻域平均（减少纹理模糊）。仅影响颜色，不改变几何。"
		},
		"kernel": {
			"name": "核宽",
			"tooltip": "最大 splat 半宽（体素数）。每个高斯会在自身 3-σ 范围内栅格化，窗口大小受此限制——小的 surfel 计算量低，大的不会被截断。如果稀疏 splat 出现空洞可适当提高。"
		},
		"level": {
			"name": "等值面级别",
			"tooltip": "等值面级别。默认由 Otsu 自动选择；此参数可调整（1.0=自动，数值越低表面越厚/连通性更强，数值越高表面越薄/更紧致）。"
		},
		"min_component": {
			"name": "最小组件",
			"tooltip": "移除小于该顶点数的连通组件（0=保留全部）。可去除悬浮小块和双层壳体的内层。"
		},
		"min_opacity": {
			"name": "最小不透明度",
			"tooltip": "网格化前忽略低于该值的不透明高斯。"
		},
		"resolution": {
			"name": "分辨率",
			"tooltip": "沿最长轴的密度网格分辨率。数值越高，表面越精细，但显存/时间消耗越大（随分辨率的立方增长）。"
		},
		"smooth": {
			"name": "平滑",
			"tooltip": "Taubin 网格平滑迭代次数。可在不缩小体积的情况下平滑表面（与模糊密度不同）。0 表示原始表面。"
		},
		"splat": { "name": "splat" }
	},
	"outputs": { "0": {
...
var SplitImageToTileList = {
	"description": "将图像拆分为带有指定重叠的批量图块列表。",
	"display_name": "将图像拆分为图块列表",
	"inputs": {
		"image": { "name": "image" },
		"overlap": { "name": "overlap" },
...
var StableZero123_Conditioning_Batched = {
	...
	"inputs": {
		"azimuth": { "name": "方位角" },
		"azimuth_batch_increment": { "name": "方位角增量" },
		"batch_size": { "name": "批量大小" },
		"clip_vision": { "name": "clip视觉" },
		"elevation": { "name": "俯仰角" },
		"elevation_batch_increment": { "name": "俯仰角增量" },
		"height": { "name": "高度" },
		"init_image": { "name": "初始图像" },
		"vae": { "name": "vae" },
		"width": { "name": "宽度" }
...
var StringFormat = {
	"description": "与 Python 的字符串格式化方法相同。支持所有 Python 的格式化选项和功能。",
	"display_name": "格式化文本",
	"inputs": {
...
var StyleModelApply = {
	...
	"inputs": {
		...
		"conditioning": { "name": "条件" },
		"strength": { "name": "强度" },
		"strength_type": { "name": "强度类型" },
		"style_model": { "name": "风格模型" }
	}
};
var StyleModelLoader = {
	"display_name": "加载风格模型",
	"inputs": { "style_model_name": { "name": "风格模型名称" } }
};
var SyncLipSyncNode = {
	...
	"inputs": {
		...
		"model_speaker_frame": { "name": "说话者帧" },
		"model_speaker_selection": { "name": "说话者选择" },
		"model_speaker_x": { "name": "说话者 X" },
		"model_speaker_y": { "name": "说话者 Y" },
		"model_sync_mode": { "name": "同步模式" },
		"seed": {
			"name": "种子",
			"tooltip": "种子用于控制节点是否重新运行；无论种子如何，结果都是非确定性的。"
		},
...
var T5TokenizerOptions = {
	...
	"inputs": {
		"clip": { "name": "clip" },
		"min_length": { "name": "最小长度" },
		"min_padding": { "name": "最小填充" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var TCFG = {
	"description": "TCFG – 切向阻尼CFG (2503.18137)\n\n优化无条件（负向）以与有条件（正向）对齐，从而提高质量。",
	"display_name": "切向阻尼CFG",
	"inputs": { "model": { "name": "模型" } },
	"outputs": { "0": {
		"name": "模型",
		"tooltip": null
...
var TemporalScoreRescaling = {
	...
	"display_name": "TSR - 时序分数重缩放",
	"inputs": {
		"model": { "name": "模型" },
		"tsr_k": {
			"name": "TSR_K",
			"tooltip": "控制重缩放强度。\n较低的 k 值会产生更详细的结果；较高的 k 值在图像生成中会产生更平滑的结果。设置 k = 1 将禁用重缩放。"
		},
		"tsr_sigma": {
			"name": "tsr_sigma",
			"tooltip": "控制重缩放何时生效。\n数值越大生效越早。"
...
var Tencent3DTextureEditNode = {
	"description": "输入3D模型后，对3D模型进行纹理重绘。",
	"display_name": "Hunyuan3D：3D纹理编辑",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"model_3d": {
			"name": "model_3d",
			"tooltip": "FBX格式的3D模型。模型面数应少于100000。"
		},
		"prompt": {
			"name": "prompt",
			"tooltip": "描述纹理编辑。支持最多1024个UTF-8字符。"
		},
		"seed": {
			"name": "seed",
			"tooltip": "种子控制节点是否重新运行；无论种子如何，结果都是非确定性的。"
...
var TencentImageToModelNode = {
	...
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"face_count": { "name": "面数" },
		"generate_type": { "name": "生成类型" },
		"generate_type_pbr": { "name": "PBR" },
		"image": { "name": "图像" },
		"image_back": { "name": "背面图像" },
		"image_left": { "name": "左侧图像" },
		"image_right": { "name": "右侧图像" },
		"model": {
			"name": "模型",
...
var TencentModelTo3DUVNode = {
	"description": "对3D模型进行UV展开，生成UV贴图。输入模型面数需小于30000。",
	"display_name": "Hunyuan3D：模型转UV",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"model_3d": {
			"name": "3D模型",
			"tooltip": "输入3D模型（GLB、OBJ或FBX）"
		},
		"seed": {
			"name": "种子",
			"tooltip": "种子控制节点是否重新运行；无论种子如何，结果都是非确定性的。"
...
var TencentSmartTopologyNode = {
	"description": "对3D模型进行智能重拓扑。支持GLB/OBJ格式；最大200MB；推荐用于高面数模型。",
	"display_name": "Hunyuan3D：智能拓扑",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"face_level": {
			"name": "面数级别",
...
var TextEncodeBooguEdit = {
	...
	"inputs": {
		...
		"images": {
			"name": "images",
			"tooltip": "用于编辑的参考图像。Boogu 每个样本专注于一个参考图像；也允许添加更多。"
		},
		"negative_prompt": { "name": "negative_prompt" },
		"prompt": { "name": "prompt" },
		"vae": { "name": "vae" }
...
var TextEncodeMageFlowEdit = {
	"description": "为 Mage-Flow-Edit 编码编辑指令，可包含一张或多张参考图像。参考 latent 会被调整为输出分辨率（宽/高，或为0时使用第一张图像的尺寸）。采样时请使用 latent 输出，以确保尺寸始终一致。",
	...
	"inputs": {
		...
		"height": {
			"name": "height",
			"tooltip": "输出高度。0 = 使用第一张参考图像的尺寸。"
		},
		"images": {
			"name": "images",
			"tooltip": "要编辑的参考图像。所有参考图像在编码前会被调整为输出分辨率。"
		},
		"negative_prompt": { "name": "negative_prompt" },
		"prompt": { "name": "prompt" },
		"vae": { "name": "vae" },
...
var TextEncodeQwenImageEdit = {
	"display_name": "文本编码（QwenImageEdit）",
	"inputs": {
		"clip": { "name": "clip" },
		"image": { "name": "图像" },
		"prompt": { "name": "提示词" },
...
var TextEncodeQwenImageEditPlus = {
	"display_name": "文本编码（QwenImageEditPlus）",
	"inputs": {
		"clip": { "name": "clip" },
		"image1": { "name": "图像1" },
		"image2": { "name": "图像2" },
...
var TextGenerate = {
	...
	"inputs": {
		...
		"clip": { "name": "clip" },
		"image": { "name": "图像" },
		"max_length": { "name": "最大长度" },
		"prompt": { "name": "提示词" },
		"sampling_mode": { "name": "采样模式" },
		"sampling_mode_min_p": { "name": "最小概率" },
		"sampling_mode_presence_penalty": { "name": "presence_penalty" },
		"sampling_mode_repetition_penalty": { "name": "重复惩罚" },
		"sampling_mode_seed": { "name": "种子" },
		"sampling_mode_temperature": { "name": "温度" },
		"sampling_mode_top_k": { "name": "Top K" },
		"sampling_mode_top_p": { "name": "Top P" },
		"thinking": {
			"name": "思考模式",
			"tooltip": "如果模型支持，则以思考模式运行。"
		},
		"use_default_template": {
			"name": "use_default_template",
			"tooltip": "如果模型自带系统提示/模板，则使用内置模板。"
		},
		"video": {
			"name": "视频",
			"tooltip": "视频帧作为图像批处理。假定为24帧/秒；内部降采样为1帧/秒。"
		}
	},
	"outputs": { "0": {
		"name": "生成文本",
...
var TopazImageEnhance = {
	"description": "专业级放大和图像增强。",
	"display_name": "Topaz 图像增强",
	"inputs": {
		"color_preservation": {
			"name": "固定色彩",
			"tooltip": "保证色彩一致性。"
		...
		"crop_to_fill": {
			"name": "裁剪",
			"tooltip": "默认情况下，当输出宽高比不同时，图像会带有边框。启用后会裁剪图像填充输出尺寸。"
		},
		"face_enhancement": {
			"name": "面部增强",
			"tooltip": "优化面部（如果存在）。"
		...
		"subject_detection": { "name": "物体检测" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var TopazVideoEnhance = {
	"description": "通过强大的放大和修复技术，为视频注入新的生命。",
	"display_name": "Topaz 视频增强",
	"inputs": {
		...
		"interpolation_duplicate": {
			"name": "补帧重复帧",
			"tooltip": "分析原视频，移除重复的帧。"
		},
		"interpolation_duplicate_threshold": {
			"name": "补帧重复帧阈值",
			"tooltip": "判断为重复帧的阈值。"
		},
		"interpolation_enabled": { "name": "补帧" },
		"interpolation_frame_rate": {
			"name": "补帧帧率",
			"tooltip": "输出帧率"
		},
		"interpolation_model": { "name": "补帧模型" },
		"interpolation_slowmo": {
			"name": "补帧慢动作",
			"tooltip": "使用慢动作补帧。例如，设为 2 会让视频变慢到 0.5 倍，持续时间增加到 2 倍。"
		},
		"upscaler_creativity": {
			"name": "放大多样性",
			"tooltip": "多样等级（仅适用 Starlight (Astra) Creative 模型）."
		},
		"upscaler_enabled": { "name": "放大" },
		"upscaler_model": { "name": "放大模型" },
		"upscaler_resolution": { "name": "放大分辨率" },
		"video": { "name": "视频" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var TopazVideoEnhanceV2 = {
	"description": "通过强大的放大和修复技术为视频注入新活力。",
	"display_name": "Topaz 视频增强",
	"inputs": {
		"dynamic_compression_level": {
			"name": "动态压缩等级",
		...
		"upscaler_model": { "name": "放大模型" },
		"upscaler_model_creativity": { "name": "创造力" },
		"upscaler_model_prompt": { "name": "提示词" },
		"upscaler_model_realism": { "name": "真实感" },
		"upscaler_model_sharp": { "name": "锐度" },
		"upscaler_model_upscaler_resolution": { "name": "放大分辨率" },
		"video": { "name": "视频" }
	},
	"outputs": { "0": { "tooltip": null } }
};
...
var TrainLoraNode = {
	...
	"inputs": {
		...
		"bucket_mode": {
			"name": "bucket模式",
			"tooltip": "启用分辨率 bucket 模式。开启后会先使用 分辨率Bucket 节点处理 Latent。"
		},
		"bypass_mode": {
			"name": "bypass_mode",
		...
		"existing_lora": {
			"name": "现有LoRA",
			"tooltip": "要附加到的现有LoRA。设置为None以创建新LoRA。"
		},
		...
		"quantized_backward": {
			...
			"tooltip": "当 training_dtype 设为 'none' 并在量化模型上训练时，启用后将使用量化矩阵乘法进行反向传播。"
		},
		"rank": {
			"name": "秩",
			"tooltip": "LoRA 层的秩。"
		...
		"seed": {
			"name": "种子",
			"tooltip": "用于训练的种子（用于 LoRA 权重初始化和噪声采样的生成器）"
		},
		"steps": {
			"name": "步数",
			"tooltip": "训练 LoRA 的步数。"
...
var TransformSplat = {
	"description": "对高斯 splat 进行平移、旋转和缩放。非均匀缩放也会改变每个 splat 的形状，处理速度较慢。",
	"display_name": "变换 Splat",
	"inputs": {
		"rotate_x": { "name": "X 轴旋转" },
		"rotate_y": { "name": "Y 轴旋转" },
		"rotate_z": { "name": "Z 轴旋转" },
		...
		"scale_z": { "name": "Z 轴缩放" },
		"splat": { "name": "splat" },
		"translate_x": { "name": "X 轴平移" },
		"translate_y": { "name": "Y 轴平移" },
		"translate_z": { "name": "Z 轴平移" }
	},
	"outputs": { "0": {
		"name": "splat",
		"tooltip": null
...
var TrimAudioDuration = {
	...
	"inputs": {
		...
		"start_index": {
			"name": "起始索引",
			"tooltip": "开始时间（秒），可为负数表示从末尾开始计数（支持小数秒）。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var TrimVideoLatent = {
	"display_name": "修剪视频Latent",
	"inputs": {
		"samples": { "name": "Latent" },
		"trim_amount": { "name": "修剪数量" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var TripleCLIPLoader = {
	...
	"inputs": {
		"clip_name1": { "name": "CLIP名称1" },
		"clip_name2": { "name": "CLIP名称2" },
		"clip_name3": { "name": "CLIP名称3" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var TripoConversionNode = {
	"display_name": "Tripo：转换模型",
	"inputs": {
		"animate_in_place": { "name": "原地动画" },
		"bake": { "name": "烘焙" },
		"export_orientation": { "name": "导出朝向" },
		"export_vertex_colors": { "name": "导出顶点色" },
		"face_limit": { "name": "面数限制" },
		"fbx_preset": { "name": "fbx预设" },
		"flatten_bottom": { "name": "平滑底部" },
		"flatten_bottom_threshold": { "name": "平滑底部阈值" },
		"force_symmetry": { "name": "强制对称" },
		"format": { "name": "格式" },
		"original_model_task_id": { "name": "原始模型任务ID" },
		"pack_uv": { "name": "打包uv" },
		"part_names": { "name": "部件名称" },
		"pivot_to_center_bottom": { "name": "对齐底部中心" },
		"quad": { "name": "四边形" },
		"scale_factor": { "name": "缩放系数" },
		"texture_format": { "name": "纹理格式" },
		"texture_size": { "name": "纹理大小" },
		"with_animation": { "name": "加入动画" }
	}
};
var TripoImageToModelNode = {
	"display_name": "Tripo：图像转模型",
	"inputs": {
		"face_limit": { "name": "面数限制" },
		"geometry_quality": { "name": "几何质量" },
		"image": { "name": "图像" },
		"model_seed": { "name": "模型种子" },
		"model_version": {
			"name": "模型版本",
			"tooltip": "用于生成的模型版本"
		},
		...
		"style": { "name": "风格" },
		"texture": { "name": "纹理" },
		"texture_alignment": { "name": "纹理对齐" },
		"texture_quality": { "name": "纹理质量" },
		"texture_seed": { "name": "纹理种子" }
	},
	"outputs": {
		"0": {
			"name": "模型文件",
...
var TripoP1ImageToModelNode = {
	"description": "Tripo P1 图像转 3D。针对低面数、游戏级网格进行了优化。",
	"display_name": "Tripo P1：图像转模型",
	"inputs": {
		"auto_size": {
			"name": "自动缩放",
			"tooltip": "将输出缩放至接近真实世界米制。"
		},
		"compress_geometry": {
			"name": "压缩几何体",
			"tooltip": "应用基于几何体的压缩。编辑前需解压。"
		},
		"enable_image_autofix": {
			"name": "启用图像自动修复",
			"tooltip": "对输入图像进行预处理，以提升生成质量。"
		},
		"export_uv": {
			"name": "导出 UV",
			"tooltip": "生成时进行 UV 展开。关闭可加快仅几何体生成速度。"
		},
		"face_limit": {
			"name": "面数限制",
			"tooltip": "目标面数，48-20000。-1 让 Tripo 自适应选择。"
		},
		"image": { "name": "图像" },
		"model_seed": { "name": "模型种子" },
		"output_mode": {
			"name": "输出模式",
			"tooltip": "“仅几何体”返回无贴图网格。“带贴图”添加颜色/PBR 贴图。"
		}
	},
	"outputs": {
		"0": {
...
var TripoP1MultiviewToModelNode = {
	"description": "Tripo P1 多视图转 3D，支持 2-4 张参考图像，顺序为 [正面、左侧、背面、右侧]。正面为必填，其余三项可选。",
	"display_name": "Tripo P1：多视图转模型",
	"inputs": {
		"auto_size": {
			"name": "自动缩放",
			"tooltip": "将输出缩放至接近真实世界米制。"
		...
		"image_left": {
			"name": "左侧图像",
			"tooltip": "左侧视角（90°），即主体左侧。"
		},
		"image_right": {
			"name": "右侧图像",
			"tooltip": "右侧视角（270°），即主体右侧。"
		},
		"model_seed": { "name": "模型种子" },
		"output_mode": {
			"name": "输出模式",
...
var TripoP1TextToModelNode = {
	"description": "Tripo P1 文本转 3D。针对低多边形、游戏级网格和稳定拓扑结构进行了优化。",
	"display_name": "Tripo P1：文本转模型",
	"inputs": {
		"auto_size": {
			"name": "自动缩放",
			"tooltip": "将输出缩放至接近真实世界米制。"
		...
		"output_mode": {
			"name": "输出模式",
			"tooltip": "“仅几何体”返回无贴图网格。“带贴图”则添加颜色/PBR 贴图。"
		},
		"prompt": {
			"name": "提示词",
			"tooltip": "最多 1024 个字符。"
...
var TripoRigNode = {
	...
	"inputs": { "original_model_task_id": { "name": "原始模型任务ID" } },
	"outputs": {
		"0": {
			"name": "模型文件",
			"tooltip": null
...
var TripoSplatConditioning = {
	"description": "使用 DINOv3 和 Flux2 VAE 对图像进行编码，生成 TripoSplat 正/负条件，并为 KSampler 创建固定尺寸的噪声目标（latent + 相机）。",
	"display_name": "TripoSplat 条件编码",
	"inputs": {
		"clip_vision": {
			"name": "clip_vision",
			"tooltip": "DINOv3 ViT-H/16+ 图像编码器"
		},
		"image": { "name": "图像" },
		"vae": {
			"name": "vae",
	...
	"outputs": {
		...
		"2": {
			"name": "latent",
			"tooltip": "固定尺寸的噪声目标（latent + 相机）。"
		}
	}
};
var TripoSplatPreprocessImage = {
	"description": "将每张图像居中裁剪为黑色背景的正方形画布，并添加填充。",
	"display_name": "TripoSplat 预处理图像",
	"inputs": {
		"erode_radius": {
			"name": "erode_radius",
			"tooltip": "在裁剪前按此像素半径腐蚀 alpha 蒙版（避免边缘溢出）。"
		},
		"image": { "name": "image" },
		"mask": { "name": "mask" },
		"size": {
			"name": "size",
			"tooltip": "正方形图像尺寸。模型在 1024 上训练；其他尺寸可用但分布不同。"
		}
	},
	"outputs": { "0": {
		"name": "image",
...
var TripoSplatSamplingPreview = {
	"description": "为标准 Ksampler 节点打补丁，在每一步显示实时解码的高斯溅射预览。",
	"display_name": "TripoSplat 采样预览",
	"inputs": {
		"model": { "name": "model" },
		"num_gaussians": {
			"name": "num_gaussians",
			"tooltip": "用于预览的高斯数量（四舍五入为 32 的倍数）。"
		},
		"octree_level": {
			"name": "octree_level",
			"tooltip": "预览解码的八叉树深度（越低越省资源/越粗糙）。"
		},
		"pitch": {
			"name": "pitch",
			"tooltip": "预览相机的俯仰角（度）。"
		...
		"point_size": {
			"name": "point_size",
			"tooltip": "最大溅射半径（像素）。每个高斯根据其缩放设置大小，并在此处限制；数值越低越细致，越高越粗大。"
		},
		"vae": {
			"name": "vae",
			"tooltip": "TripoSplat VAE 解码器"
		},
		"yaw": {
			"name": "yaw",
			"tooltip": "预览相机的偏航角（度）。"
...
var UNETLoader = {
	...
	"inputs": {
		"unet_name": { "name": "UNet名称" },
		"weight_dtype": { "name": "数据类型" }
	}
};
...
var UNetTemporalAttentionMultiply = {
	...
	"inputs": {
		"cross_structural": { "name": "交叉结构" },
		"cross_temporal": { "name": "交叉时间" },
		"model": { "name": "模型" },
		"self_structural": { "name": "自我结构" },
		"self_temporal": { "name": "自我时间" }
	},
	"outputs": { "0": { "tooltip": null } }
};
var USOStyleReference = {
	"display_name": "USO风格参考",
	"inputs": {
		"clip_vision_output": { "name": "CLIP视觉输出" },
		"model": { "name": "模型" },
		"model_patch": { "name": "模型补丁" }
...
var VAEDecodeTripoSplat = {
	"description": "将采样的 TripoSplat latent 解码为 3D 高斯溅射。可通过调整高斯数量改变密度。",
	"display_name": "TripoSplat 解码",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"num_gaussians": {
			"name": "num_gaussians",
			"tooltip": "生成的高斯数量（四舍五入为 32 的倍数）。262144 匹配八叉树的点密度；更高会对同一点过采样（更密集，但无新细节），并消耗更多显存/时间。"
		},
		"samples": { "name": "samples" },
		"seed": {
			"name": "seed",
			"tooltip": "为八叉树点采样器（全局随机数生成器）设置种子，实现确定性解码。"
		},
		"vae": {
			"name": "vae",
			"tooltip": "TripoSplat VAE 解码器"
...
var VOIDInpaintConditioning = {
	...
	"inputs": {
		...
		"length": {
			"name": "length",
			"tooltip": "要处理的像素帧数。对于 CogVideoX-Fun-V1.5（patch_size_t=2），latent_t 必须为偶数——产生奇数 latent_t 的长度会向下取整（如 49 → 45）。"
		},
		"negative": { "name": "negative" },
		"positive": { "name": "positive" },
		"quadmask": {
...
var VOIDWarpedNoise = {
	...
	"inputs": {
		...
		"length": {
			"name": "length",
			"tooltip": "像素帧数。为满足 patch_size_t=2 的要求，向下取整使 latent_t 为偶数，例如 49 → 45。"
		},
		"optical_flow": {
			"name": "optical_flow",
			"tooltip": "来自 OpticalFlowLoader 的光流模型（RAFT-large）。"
		},
		"video": {
			"name": "video",
			"tooltip": "第一步输出的视频帧 [T, H, W, 3]"
		},
		"width": { "name": "width" }
	},
	"outputs": { "0": {
...
var Veo3FirstLastFrameNode = {
	"description": "使用首尾帧和提示词生成视频",
	"display_name": "Google Veo 3 首尾帧视频",
	"inputs": {
		"aspect_ratio": {
			"name": "宽高比",
			"tooltip": "输出视频的宽高比"
...
var Veo3VideoGenerationNode = {
	"description": "使用 Google Veo 3 API 从文本提示生成视频",
	"display_name": "Google Veo 3 视频生成",
	"inputs": {
		"aspect_ratio": {
			"name": "宽高比",
			"tooltip": "输出视频的宽高比"
		...
		"duration_seconds": {
			"name": "时长",
			"tooltip": "输出视频的时长（秒）（Veo 3 仅支持 8 秒）"
		},
		"enhance_prompt": {
			"name": "优化提示词",
			"tooltip": "是否使用 AI 辅助增强提示"
		},
		"generate_audio": {
			"name": "生成音频",
			"tooltip": "为视频生成音频。所有 Veo 3 模型均支持此功能。"
		},
		"image": {
			"name": "图像",
			"tooltip": "用于指导视频生成的可选参考图像"
		},
		"model": {
			"name": "模型",
			"tooltip": "用于视频生成的 Veo 3 模型"
		},
		"negative_prompt": {
			"name": "负面提示词",
			"tooltip": "负面文本提示，指导视频中应避免的内容"
		},
		"person_generation": {
			"name": "生成人类",
		...
		"resolution": {
			"name": "分辨率",
			"tooltip": "输出视频分辨率。veo-3.1-lite 和 veo-3.0 模型不支持 4K。"
		},
		"seed": {
			"name": "随机种",
			"tooltip": "视频生成的种子值（0 表示随机）"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
...
var VideoFrameSample = {
	"description": "使用多种策略从视频中采样固定数量的帧。",
	"display_name": "采样视频帧",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"num_frames": {
		...
		"seed": {
			"name": "随机种子",
			"tooltip": "随机种子（仅在'random'策略下使用）。"
		},
		"strategy": {
			"name": "采样策略",
			"tooltip": "uniform：均匀间隔，head：前N帧，tail：后N帧，random：随机排序。"
		},
		"video": {
			"name": "视频",
			"tooltip": "输入视频。"
...
var VideoRandomTemporalCrop = {
	"description": "从视频中随机裁剪连续的一段帧。",
	"display_name": "裁剪视频（时间随机）",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"length": {
			"name": "保留帧数",
...
var VideoTemporalCrop = {
	"description": "从视频中裁剪连续的一段帧。",
	"display_name": "裁剪视频（时间）",
	"inputs": {
		"length": {
			"name": "保留帧数",
			"tooltip": "要保留的帧数。"
...
var Vidu2ImageToVideoNode = {
	"description": "根据一张图像和可选提示生成视频。",
	"display_name": "Vidu2 图像转视频生成",
	"inputs": {
		"control_after_generate": { "name": "control after generate" },
		"duration": { "name": "duration" },
		"image": {
			"name": "image",
			"tooltip": "用于生成视频起始帧的图像。"
		},
		"model": { "name": "model" },
		"movement_amplitude": {
...
var Vidu2ReferenceVideoNode = {
	"description": "根据多张参考图像和提示生成视频。",
	"display_name": "Vidu2 参考图像转视频生成",
	"inputs": {
		"aspect_ratio": { "name": "aspect_ratio" },
		"audio": {
			"name": "audio",
			"tooltip": "启用后，视频将根据提示包含生成的语音和背景音乐。"
		},
		"control_after_generate": { "name": "control after generate" },
		"duration": { "name": "duration" },
		"model": { "name": "model" },
		...
		"prompt": {
			"name": "prompt",
			"tooltip": "启用后，视频将根据提示生成语音和背景音乐。"
		},
		"resolution": { "name": "resolution" },
		"seed": { "name": "seed" },
		"subjects": {
			"name": "subjects",
			"tooltip": "每个主体最多提供3张参考图像（所有主体共7张）。可通过 @subject{subject_id} 在提示中引用。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var Vidu2StartEndToVideoNode = {
	...
	"display_name": "Vidu2 起始/结束帧到视频生成",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"duration": { "name": "时长" },
		"end_frame": { "name": "结束帧" },
...
var Vidu2TextToVideoNode = {
	...
	"display_name": "Vidu2 文本到视频生成",
	"inputs": {
		"aspect_ratio": { "name": "宽高比" },
		"background_music": {
			"name": "背景音乐",
			"tooltip": "是否为生成的视频添加背景音乐。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"duration": { "name": "时长" },
		"model": { "name": "模型" },
		"prompt": {
			"name": "提示词",
			"tooltip": "用于视频生成的文本描述，最大长度为2000个字符。"
		},
		"resolution": { "name": "分辨率" },
		"seed": { "name": "种子" }
	},
...
var ViduReferenceVideoNode = {
	...
	"inputs": {
		...
		"images": {
			"name": "图像",
			"tooltip": "用作参考以生成具有一致主体的图像（最多 7 张图像）"
		},
		"model": {
			"name": "模型",
			"tooltip": "模型名称"
...
var Wan22FunControlToVideo = {
	"display_name": "Wan22FunControl视频",
	"inputs": {
		"batch_size": { "name": "批次大小" },
		"control_video": { "name": "控制视频" },
		"height": { "name": "高度" },
		"length": { "name": "长度" },
		"negative": { "name": "负面提示词" },
		"positive": { "name": "正面提示词" },
...
var Wan2ImageToVideoApi = {
	...
	"inputs": {
		"audio": {
			"name": "audio",
			"tooltip": "用于驱动视频生成的音频（如对口型、节奏匹配动作）。时长：2秒-30秒。如未提供，模型会自动生成匹配的背景音乐或音效。"
		},
		"control_after_generate": { "name": "control after generate" },
		"first_frame": {
			"name": "first_frame",
...
var Wan2TextToVideoApi = {
	...
	"inputs": {
		"audio": {
			"name": "audio",
			"tooltip": "用于驱动视频生成的音频（如对口型、节奏匹配动作）。时长：3秒-30秒。如未提供，模型会自动生成匹配的背景音乐或音效。"
		},
		"control_after_generate": { "name": "control after generate" },
		"model": { "name": "model" },
		"model_duration": { "name": "duration" },
...
var Wan2VideoContinuationApi = {
	...
	"inputs": {
		...
		"first_clip": {
			"name": "first_clip",
			"tooltip": "要续写的视频。时长：2秒-10秒。输出宽高比将根据该视频确定。"
		},
		"last_frame": {
			"name": "last_frame",
...
var WanAnimateToVideo = {
	...
	"inputs": {
		"background_video": { "name": "背景视频" },
		"batch_size": { "name": "批次大小" },
		"character_mask": { "name": "角色遮罩" },
		"clip_vision_output": { "name": "CLIP视觉输出" },
		"continue_motion": { "name": "继续运动" },
		"continue_motion_max_frames": { "name": "继续运动最大帧数" },
		"face_video": { "name": "面部视频" },
		"height": { "name": "高度" },
		"length": { "name": "长度" },
		"negative": { "name": "负向提示" },
		"pose_video": { "name": "姿态视频" },
		"positive": { "name": "正向提示" },
		"reference_image": { "name": "参考图像" },
		"vae": { "name": "VAE" },
		"video_frame_offset": {
			"name": "视频帧偏移",
			"tooltip": "在所有输入视频中跳过的帧数。用于通过分块生成更长的视频。连接到上一个节点的video_frame_offset输出以扩展视频。"
		},
		"width": { "name": "宽度" }
	},
	"outputs": {
...
var WanCameraEmbedding = {
	"display_name": "Wan相机嵌入",
	"inputs": {
		"camera_pose": { "name": "相机姿态" },
		"cx": { "name": "cx" },
		"cy": { "name": "cy" },
		"fx": { "name": "fx" },
		"fy": { "name": "fy" },
...
var WanCameraImageToVideo = {
	"display_name": "万相机图像转视频",
	"inputs": {
		"batch_size": { "name": "批次大小" },
		"camera_conditions": { "name": "相机条件" },
		"clip_vision_output": { "name": "CLIP视觉输出" },
		"height": { "name": "高度" },
		"length": { "name": "长度" },
		"negative": { "name": "负面提示词" },
...
var WanContextWindowsManual = {
	"description": "手动设置类WAN模型的上下文窗口（维度=2）。",
	"display_name": "WAN上下文窗口（手动）",
	"inputs": {
		"closed_loop": {
			"name": "闭环",
			"tooltip": "是否关闭上下文窗口循环；仅适用于循环调度。"
		},
		"context_length": {
			"name": "上下文长度",
			"tooltip": "上下文窗口的长度。"
		...
		"model": {
			"name": "模型",
			"tooltip": "在采样过程中应用上下文窗口的模型。"
		},
		"retain_first_frame": {
			"name": "retain_first_frame",
			"tooltip": "在每个上下文窗口中保留第一个 I2V 帧（有助于保留初始参考）。"
		},
		"split_conds_to_windows": {
			"name": "split_conds_to_windows",
			"tooltip": "是否根据区域索引将多个条件（由 ConditionCombine 创建）拆分到每个窗口。"
		}
	},
	"outputs": { "0": { "tooltip": "在采样过程中应用了上下文窗口的模型。" } }
};
...
var WanDancerVideo = {
	...
	"inputs": {
		"audio_encoder_output": { "name": "音频编码器输出" },
		"clip_vision_output": {
			"name": "clip视觉输出",
...
var WanFirstLastFrameToVideo = {
	...
	"inputs": {
		"batch_size": { "name": "批量大小" },
		"clip_vision_end_image": { "name": "clip 视觉结束图像" },
		"clip_vision_start_image": { "name": "clip 视觉起始图像" },
		"end_image": { "name": "结束图像" },
		"height": { "name": "高度" },
		"length": { "name": "长度" },
		"negative": { "name": "负向" },
...
var WanImageToImageApi = {
	"description": "根据一张或两张输入图像和文本提示生成图像。输出图像目前固定为160万像素；其宽高比与输入图像匹配。",
	"display_name": "Wan 图像转图像",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"image": {
			"name": "图像",
			"tooltip": "单图编辑或多图融合，最多2张图像。"
		},
		"model": {
			"name": "模型",
			"tooltip": "要使用的模型。"
		...
		"negative_prompt": {
			"name": "负面提示词",
			"tooltip": "用于指导避免内容的负面文本提示。"
		},
		"prompt": {
			"name": "提示词",
			"tooltip": "用于描述元素和视觉特征的提示词，支持英文/中文。"
		},
		"seed": {
			"name": "种子",
			"tooltip": "生成使用的种子值。"
		...
		"watermark": {
			"name": "水印",
			"tooltip": "是否在结果中添加\"AI生成\"水印。"
		}
	},
	"outputs": { "0": { "tooltip": null } }
};
var WanImageToVideo = {
	"display_name": "图像到视频（Wan）",
	"inputs": {
		"batch_size": { "name": "批量大小" },
		"clip_vision_output": { "name": "clip视觉输出" },
		"height": { "name": "高度" },
...
var WanImageToVideoApi = {
	"description": "基于首帧图像和文本提示生成视频。",
	"display_name": "Wan 图生视频",
	"inputs": {
		"audio": {
			"name": "音频",
			"tooltip": "音频必须包含清晰、响亮的人声，无杂音和背景音乐。"
		},
		"control_after_generate": { "name": "生成后控制" },
		"duration": {
			"name": "时长",
		...
		"shot_type": {
			"name": "镜头类型",
			"tooltip": "选择生成的视频的镜头类型，即视频是单个连续镜头还是多个剪辑镜头。需要开启 优化提示词。"
		},
		"watermark": {
			"name": "水印",
			"tooltip": "是否在结果中添加\"AI生成\"水印。"
...
var WanInfiniteTalkToVideo = {
	...
	"inputs": {
		"audio_encoder_output_1": { "name": "音频编码器输出1" },
		"audio_scale": { "name": "音频缩放" },
		"clip_vision_output": { "name": "clip视觉输出" },
		"height": { "name": "高度" },
		"length": { "name": "长度" },
		"mode": { "name": "模式" },
...
var WanMoveTracksFromCoords = {
	"display_name": "WanMove坐标到轨道",
	"inputs": {
		"track_coords": { "name": "轨道坐标" },
		"track_mask": { "name": "轨道遮罩" }
	},
	"outputs": {
		"0": { "tooltip": null },
		"1": {
...
var WanMoveVisualizeTracks = {
	"display_name": "WanMove预览轨道",
	"inputs": {
		"circle_size": { "name": "圆环尺寸" },
		"images": { "name": "图像" },
		"line_resolution": { "name": "线条分辨率" },
...
var WanReferenceVideoApi = {
	"description": "使用输入视频中的角色和声音，结合提示词，生成保持角色一致性的新视频。",
	"display_name": "Wan 参考视频生成",
	"inputs": {
		"control_after_generate": { "name": "生成后控制" },
		"duration": { "name": "时长" },
		"model": { "name": "模型" },
		"negative_prompt": {
			"name": "反向提示词",
			"tooltip": "描述需要避免内容的反向提示词。"
		},
		"prompt": {
			"name": "提示词",
			"tooltip": "描述元素和视觉特征的提示词。支持英文和中文。使用如 `character1` 和 `character2` 这样的标识符来指代参考角色。"
		},
		"reference_videos": { "name": "参考视频" },
		"seed": { "name": "种子" },
		"shot_type": {
			"name": "镜头类型",
			"tooltip": "指定生成视频的镜头类型，即视频是单一连续镜头还是包含多个剪辑的镜头。"
		},
		"size": { "name": "尺寸" },
		"watermark": {
			"name": "水印",
...
var WanSCAILToVideo = {
	...
	"inputs": {
		...
		"clip_vision_output": {
			"name": "clip视觉输出",
			"tooltip": "用于条件控制的 CLIP 视觉特征。模型训练时采用拉伸缩放至宽高比。"
		},
		"height": { "name": "高度" },
		"length": { "name": "长度" },
		"negative": { "name": "负向" },
		...
		"pose_video": {
			"name": "姿态视频",
			"tooltip": "用于姿态调控的视频。将被缩小到主视频分辨率的一半。"
		},
		"pose_video_mask": {
			"name": "pose_video_mask",
			"tooltip": "仅限 SCAIL-2。每个身份的彩色 SAM3 mask 视频，分辨率与 pose_video 相同。"
		},
		"positive": { "name": "正向" },
		"previous_frame_count": {
			"name": "previous_frame_count",
			"tooltip": "用于锚定的上一帧尾部帧数。SCAIL-2 在 5（81 帧片段，76 帧步长）下训练。"
		},
		"previous_frames": {
			"name": "previous_frames",
			"tooltip": "仅限 SCAIL-2。上一个片段的完整解码输出。仅使用最后的 previous_frame_count 作为扩展锚点。"
		},
		"reference_image": {
			"name": "参考图像",
			"tooltip": "参考图像，如有多个参考请合成到一张图像上。"
		},
		"reference_image_mask": {
			"name": "reference_image_mask",
			"tooltip": "仅限 SCAIL-2。与 reference_image 分辨率相同的彩色参考 mask。"
		},
		"replacement_mode": {
			"name": "replacement_mode",
			"tooltip": "仅限 SCAIL-2。False = 动画模式（pose_video_mask 应为黑色背景）。True = 替换模式（pose_video_mask 应为白色背景）。"
		},
		"vae": { "name": "vae" },
		"video_frame_offset": {
			"name": "video_frame_offset",
			"tooltip": "本片段开始的累计输出帧数。请从上一个片段的 video_frame_offset 输出连接。"
		},
		"width": { "name": "宽度" }
	},
	"outputs": {
		...
		"2": {
			"name": "latent",
			"tooltip": "生成尺寸的空latent。"
		},
		"3": {
			"name": "video_frame_offset",
			"tooltip": "调整后的偏移量和长度。连接到下一个片段。"
		}
	}
};
var WanSoundImageToVideo = {
	"display_name": "图像到视频（WanSound）",
	"inputs": {
		"audio_encoder_output": { "name": "音频编码器输出" },
		"batch_size": { "name": "批次大小" },
		"control_video": { "name": "控制视频" },
...
var WanUni3CControlnetApply = {
	"display_name": "应用 Wan Uni3C ControlNet",
	"inputs": {
		"end_percent": { "name": "结束百分比" },
		"model": { "name": "模型" },
		"model_patch": { "name": "模型补丁" },
		"render_video": {
			"name": "渲染视频",
			"tooltip": "由相机轨迹渲染的引导视频，最常见的是输入图像的点云渲染变形。"
		},
		"start_percent": { "name": "起始百分比" },
		"strength": { "name": "强度" },
		"vae": { "name": "vae" }
...
var nodeDefs_default = {
	...
	EmptySD3LatentImage,
	"Epsilon Scaling": {
		"display_name": "Epsilon缩放",
		"inputs": {
			"model": { "name": "模型" },
			"scaling_factor": { "name": "系数" }
		},
		"outputs": { "0": { "tooltip": null } }
	},
...
//# sourceMappingURL=nodeDefs-DoV7vQCM.js.map