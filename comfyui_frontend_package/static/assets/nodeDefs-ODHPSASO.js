//#region src/locales/en/nodeDefs.json
var AddNoise = {
...
var AnimaLLLiteApply = {
	"display_name": "Apply Anima LLLite",
	"inputs": {
		"model": { "name": "model" },
...
var BeebleSwitchXImageEdit = {
	...
	"description": "Edit a single image with Beeble SwitchX. Switches anything in the scene (background, lighting, costume) while preserving the original subject's pixels. Provide a reference image and/or text prompt to describe the new look. Max ~2.77MP.",
	"inputs": {
		"image": { "name": "image" },
		"prompt": {
			"name": "prompt",
			"tooltip": "Text description of the desired output (max 2000 chars). At least one of 'prompt' or 'reference_image' is required."
		},
		"alpha_mode": {
			"name": "alpha_mode",
			"tooltip": "Controls how SwitchX decides what to keep vs. regenerate. 'auto' isolates the main subject automatically. 'fill' regenerates the entire frame while preserving geometry. 'select' propagates a first-frame keyframe across the clip. 'custom' uses a per-frame alpha matte you provide."
		},
		"max_resolution": {
			"name": "max_resolution",
		...
		"reference_image": {
			...
			"tooltip": "Reference image whose look (background, lighting, costume) the result should adopt. At least one of 'reference_image' or 'prompt' is required."
		},
...
var BeebleSwitchXVideoEdit = {
	...
	"description": "Edit a video with Beeble SwitchX. Switches anything in the scene (background, lighting, costume) while preserving the original subject's pixels and motion. Provide a reference image and/or text prompt to describe the new look. Max 240 frames, max ~2.77MP per frame.",
	"inputs": {
		"video": { "name": "video" },
		"prompt": {
			"name": "prompt",
...
var BriaTransparentVideoBackground = {
	...
	"description": "Remove the background from a video using Bria and return the cut-out frames plus an alpha mask. Connect both to a compositing node, or feed them to Save WEBM to write a transparent video.",
	"inputs": {
		"video": { "name": "video" },
		"seed": {
			"name": "seed",
...
var ByteDance2FirstLastFrameNode = {
	...
	"inputs": {
		...
		"first_frame_asset_id": {
			...
			"tooltip": "Seedance asset_id to use as the first frame. Mutually exclusive with the first_frame image input."
		},
		"last_frame_asset_id": {
			"name": "last_frame_asset_id",
			"tooltip": "Seedance asset_id to use as the last frame. Mutually exclusive with the last_frame image input."
		},
		"control_after_generate": { "name": "control after generate" },
...
var ByteDanceCreateImageAsset = {
	...
	"description": "Create a Seedance 2.0 personal image asset. Uploads the input image and registers it in the given asset group. If group_id is empty, runs a real-person H5 authentication flow to create a new group before adding the asset.",
	"inputs": {
		"image": {
			"name": "image",
			"tooltip": "Image to register as a personal asset."
		},
		"group_id": {
			"name": "group_id",
			"tooltip": "Reuse an existing Seedance asset group ID to skip repeated human verification for the same person. Leave empty to run real-person authentication in the browser and create a new group."
		}
	},
	"outputs": {
		"0": {
...
var ByteDanceCreateVideoAsset = {
	...
	"description": "Create a Seedance 2.0 personal video asset. Uploads the input video and registers it in the given asset group. If group_id is empty, runs a real-person H5 authentication flow to create a new group before adding the asset.",
	"inputs": {
		"video": {
			"name": "video",
			"tooltip": "Video to register as a personal asset."
		},
		"group_id": {
			"name": "group_id",
			"tooltip": "Reuse an existing Seedance asset group ID to skip repeated human verification for the same person. Leave empty to run real-person authentication in the browser and create a new group."
...
var ByteDanceSeedNode = {
	...
	"description": "Generate text responses with ByteDance's Seed 2.0 models. Provide a text prompt and optionally one or more images or videos for multimodal context.",
	"inputs": {
		"prompt": {
			"name": "prompt",
...
var ByteDanceSeedreamNodeV2 = {
	...
	"inputs": {
		...
		"thinking": {
			"name": "thinking",
			"tooltip": "Enable the model's prompt-optimization reasoning ('thinking') for better adherence. Can substantially increase generation time — notably on Seedream 5.0 Pro. Can only be disabled for text-to-image (not when reference images are provided)."
		},
		"control_after_generate": { "name": "control after generate" },
...
var CFGNorm = {
	...
	"inputs": {
		...
		"pre_cfg": {
			"name": "pre_cfg",
			"tooltip": "If true, rescale the combined noise BEFORE the sampler's CFG combine, without clamping (can amplify). Matches the norm-scaled CFG used by models like Lens. Default false keeps the original post-CFG x0-space attenuate-only behavior."
		}
	},
	"outputs": { "0": {
		"name": "patched_model",
...
var CheckpointLoaderSimple = {
	...
	"outputs": {
		"0": { "tooltip": "The model used for denoising latents." },
		"1": { "tooltip": "The CLIP model used for encoding text prompts." },
		"2": { "tooltip": "The VAE model used for encoding and decoding images to and from latent space." }
	}
};
var CheckpointSave = {
...
var ClaudeNode = {
	...
	"description": "Generate text responses with Anthropic's Claude models. Provide a text prompt and optionally one or more images for multimodal context.",
	"inputs": {
		"prompt": {
			"name": "prompt",
			"tooltip": "Text input to the model."
		...
		"images": {
			"name": "images",
			"tooltip": "Optional image(s) to use as context for the model. Up to 20 images."
		},
		"system_prompt": {
			"name": "system_prompt",
			"tooltip": "Foundational instructions that dictate the model's behavior."
...
var CLIPTextEncode = {
	...
	"outputs": { "0": { "tooltip": "A conditioning containing the embedded text used to guide the diffusion model." } }
};
...
var ConditioningSetAreaPercentageVideo = {
	"display_name": "Conditioning (Set Area with Percentage for Video)",
	"inputs": {
		"conditioning": { "name": "conditioning" },
		"width": { "name": "width" },
		"height": { "name": "height" },
...
var ConditioningSetAreaStrength = {
	"display_name": "Conditioning (Set Area Strength)",
	"inputs": {
		"conditioning": { "name": "conditioning" },
		"strength": { "name": "strength" }
	}
...
var ContextWindowsManual = {
	...
	"outputs": { "0": { "tooltip": "The model with context windows applied during sampling." } }
};
var ControlNetApply = {
...
var CropByBBoxes = {
	...
	"outputs": { "0": { "tooltip": "All crops stacked into a single image batch." } }
};
var CropMask = {
...
var DiffusersLoader = {
	"display_name": "Load Diffusers Model (DEPRECATED)",
...
var DualModelGuider = {
	...
	"inputs": {
		"model": {
			"name": "model",
			"tooltip": "Model used for the positive (conditional) pass."
		},
		"positive": { "name": "positive" },
		"cfg": { "name": "cfg" },
		"model_negative": {
...
var EmptyLatentImage = {
	...
	"outputs": { "0": { "tooltip": "The empty latent image batch." } }
};
...
var FluxKVCache = {
	...
	"outputs": { "0": { "tooltip": "The patched model with KV Cache enabled." } }
};
...
var GeminiVideoOmni = {
	...
	"description": "Generate a video with audio from a text prompt using Google's Gemini Omni Flash model. Optionally provide reference images and/or videos to guide or edit the result. Describe the desired length (3-10s) and aspect ratio (16:9 or 9:16) directly in the prompt.",
	"inputs": {
		"model": {
			"name": "model",
...
var GetICLoRAParameters = {
	...
	"description": "Extracts IC-LoRA parameters from the safetensors metadata of a LoRA-loaded model and outputs them for LTXVAddGuide (eg. reference_downscale_factor).",
	"inputs": { "iclora_model": {
		"name": "iclora_model",
		"tooltip": "Direct output from a LoRA Loader for the specific IC-LoRA from which to extract the metadata."
	} },
	"outputs": { "0": {
		"name": "iclora_parameters",
		"tooltip": "IC-LoRA parameters extracted from the LoRA metadata (eg. reference_downscale_factor). Connect to LTXVAddGuide if the LoRA requires special handling of the guides."
	} }
};
var GetImageSize = {
...
var GLIGENTextBoxApply = {
	"display_name": "Apply GLIGEN Text Box",
	"inputs": {
		"conditioning_to": { "name": "conditioning_to" },
		"clip": { "name": "clip" },
...
var HeyGenAvatarVideoNode = {
	...
	"description": "Generate a talking-presenter video from a HeyGen avatar. Includes HeyGen's most popular public avatars; any look ID can be supplied as an override.",
	"inputs": {
		"engine": {
			"name": "engine",
			"tooltip": "Rendering engine; each choice lists only the avatars that support it. 'auto' offers every avatar and picks its best engine (Avatar IV preferred). Avatar V is highest fidelity, Avatar III is the most affordable."
		},
		"speech": {
			"name": "speech",
		...
		"custom_avatar_id": {
			...
			"tooltip": "Optional HeyGen avatar look ID. When set, overrides the avatar selected above. Any of HeyGen's 3000+ public looks (or your private avatars) can be used."
		},
		"resolution": {
			"name": "resolution",
		...
		"background_color": {
			"name": "background_color",
			"tooltip": "Optional solid background color as a hex code (e.g. '#00ff00'). Leave empty for the avatar's own background."
		},
		"seed": {
			"name": "seed",
...
var HeyGenCreateAvatarNode = {
	...
	"description": "Create your own reusable HeyGen avatar from a photo of a person or from a text prompt (a generated character). Feed the resulting avatar_id into HeyGen Avatar Video's custom_avatar_id — and save the ID somewhere to reuse the avatar in future workflows.",
	"inputs": {
		"source": {
			"name": "source",
			"tooltip": "Generate a new character from a text prompt, or create the avatar from a connected photo of a person."
		},
	...
	"outputs": {
		"0": {
			"name": "avatar_id",
			"tooltip": "Avatar look ID. Pass it to HeyGen Avatar Video's custom_avatar_id; save it to reuse the avatar later."
		},
		"1": {
			"name": "preview",
			"tooltip": null
...
var HeyGenTalkingPhotoNode = {
	...
	"description": "Animate any image of a person into a lip-synced talking video (HeyGen Avatar IV). Drive it with a text script or your own audio.",
	"inputs": {
		"image": {
			"name": "image",
...
var HeyGenTextToSpeechNode = {
	...
	"description": "Generate speech audio from text using HeyGen's Starfish TTS engine. Includes HeyGen's most popular voices across 17 languages.",
	"inputs": {
		"text": {
			"name": "text",
			"tooltip": "Text to synthesize (up to 5000 characters). The generated speech must be at least 1 second long."
		},
		"voice": {
			"name": "voice",
		...
		"custom_voice_id": {
			...
			"tooltip": "Optional HeyGen voice ID. When set, overrides the voice selected above. The voice must support the Starfish engine."
		},
		"speed": {
			"name": "speed",
...
var HeyGenVideoTranslateNode = {
	...
	"description": "Translate a spoken video into another language. Clones the original speaker's voice and re-animates the mouth to match the translated speech.",
	"inputs": {
		"video": {
			"name": "video",
...
var Krea2ImageNode = {
	...
	"description": "Generate images via Krea 2 — pick Medium (expressive illustrations) or Large (expressive photorealism). Supports an optional moodboard and up to 10 chained image style references.",
	"inputs": {
		"prompt": {
			"name": "prompt",
		...
		"model": {
			"name": "model",
			"tooltip": "Krea 2 Medium is best for expressive illustrations; Krea 2 Large is best for expressive photorealism."
		},
		"seed": {
			"name": "seed",
...
var Krea2StyleReferenceNode = {
	...
	"description": "Add an image style reference to a Krea 2 generation. Chain multiple Krea 2 Style Reference nodes (max 10) and feed the final `style_reference` output into Krea 2 Image. Each image is uploaded to ComfyAPI storage and passed as URL.",
	"inputs": {
		"image": {
			"name": "image",
...
var KSampler = {
	...
	"outputs": { "0": { "tooltip": "The denoised latent." } }
};
...
var LatentFromBatch = {
	"display_name": "Get Latent From Batch",
	"inputs": {
		"samples": { "name": "samples" },
		"batch_index": { "name": "batch_index" },
		"length": { "name": "length" }
...
var LazyCache = {
	...
	"outputs": { "0": { "tooltip": "The model with LazyCache." } }
};
var Load3D = {
...
var LoadAudio = {
	...
	"inputs": {
		...
		"upload": { "name": "choose file to upload" }
	},
	"outputs": { "0": { "tooltip": null } }
};
...
var LoraLoader = {
	...
	"outputs": {
		"0": { "tooltip": "The modified diffusion model." },
		"1": { "tooltip": "The modified CLIP model." }
	}
};
...
var LoraLoaderBypassModelOnly = {
	...
	"outputs": { "0": { "tooltip": "The modified diffusion model." } }
};
...
var LTXVAddGuide = {
	...
	"inputs": {
		...
		"attention_mask": {
			"name": "attention_mask",
			"tooltip": "Optional pixel-space spatial mask. Controls per-region conditioning influence via self-attention, multiplied by strength."
		},
		"iclora_parameters": {
			"name": "iclora_parameters",
			"tooltip": "Optional IC-LoRA parameters from a Get IC-LoRA Parameters node. Used for adjusting guide processing as required by certain IC-LoRAs (eg. those with a reference_downscale_factor > 1). When chained, each LTXVAddGuide uses only the parameters connected to it."
		}
	},
	"outputs": {
		"0": {
...
var MagnificImageUpscalerCreativeNode = {
	...
	"description": "Prompt‑guided enhancement, stylization, and 2x/4x/8x/16x upscaling. Maximum output: 25.3 megapixels.",
	"inputs": {
		"image": { "name": "image" },
		"prompt": { "name": "prompt" },
...
var MoGePanoramaInference = {
	...
	"inputs": {
		...
		"image": {
			"name": "image",
			"tooltip": "Equirectangular panorama (any aspect)."
		},
		"resolution_level": {
			"name": "resolution_level",
...
var MoGePointMapToMesh = {
	...
	"inputs": {
		...
		"batch_index": {
			"name": "batch_index",
			"tooltip": "Which image of a batched MoGe geometry to mesh. Per-image vertex counts differ, so batches can't be stacked into a single MESH."
		},
		"decimation": {
			"name": "decimation",
...
var MultiGPU_WorkUnits = {
	...
	"description": "Prepares model to have sampling accelerated via splitting work units.\n\nShould be placed after nodes that modify the model object itself, such as compile or attention-switch nodes.\n\nOther than those exceptions, this node can be placed in any order.",
	"inputs": {
		"model": { "name": "model" },
...
var NAGuidance = {
	...
	"outputs": { "0": { "tooltip": "The patched model with NAG enabled." } }
};
var NormalizeImages = {
...
var OpenRouterLLMNode = {
	...
	"description": "Generate text responses through OpenRouter. Routes to a curated set of popular models from Anthropic (Claude), OpenAI (GPT), Google (Gemini), xAI (Grok), DeepSeek, Qwen, Mistral, Z.AI (GLM), Moonshot (Kimi), and Perplexity Sonar.",
	"inputs": {
		"prompt": {
			"name": "prompt",
			"tooltip": "Text input to the model."
...
var PiDConditioning = {
	...
	"inputs": {
		...
		"latent": {
			"name": "latent",
			"tooltip": "latent (from VAEEncode or a KSampler)."
		},
		"latent_format": {
			"name": "latent_format",
...
var SAM3_Detect = {
	...
	"inputs": {
		...
		"individual_masks": {
			...
			"tooltip": "Output per-object masks instead of union"
		},
		"conditioning": {
			"name": "conditioning",
			"tooltip": "Text conditioning from CLIPTextEncode"
		},
		"bboxes": {
			"name": "bboxes",
			"tooltip": "Bounding boxes to segment within"
		},
		"positive_coords": {
			"name": "positive_coords",
			"tooltip": "Positive point prompts as JSON [{\"x\": int, \"y\": int}, ...] (pixel coords)"
		},
		"negative_coords": {
			"name": "negative_coords",
			"tooltip": "Negative point prompts as JSON [{\"x\": int, \"y\": int}, ...] (pixel coords)"
		}
	},
	"outputs": {
		"0": {
...
var SAM3_VideoTrack = {
	...
	"inputs": {
		"images": {
			"name": "images",
			"tooltip": "Video frames as batched images"
		},
		"model": { "name": "model" },
		"detection_threshold": {
		...
		"initial_mask": {
			"name": "initial_mask",
			"tooltip": "Mask(s) for the first frame to track (one per object)"
		},
		"conditioning": {
			"name": "conditioning",
...
var SamplerARVideo = {
	...
	"inputs": { "num_frame_per_block": {
		...
		"tooltip": "Frames per autoregressive block. 1 = framewise, 3 = chunkwise. Must match the checkpoint's training mode."
	} },
	"outputs": { "0": { "tooltip": null } }
};
var SamplerCustom = {
...
var SaveWEBM = {
	...
	"inputs": {
		"images": {
			"name": "images",
			"tooltip": "RGBA images are saved with their alpha channel as transparency (vp9 codec only)."
		},
		"filename_prefix": { "name": "filename_prefix" },
		"codec": { "name": "codec" },
		"fps": { "name": "fps" },
...
var SeedVR2Conditioning = {
	...
	"outputs": {
		"0": {
			"name": "positive",
			"tooltip": "The positive conditioning for sampling."
		},
		"1": {
			"name": "negative",
			"tooltip": "The negative conditioning for sampling."
		}
	}
};
var SeedVR2PostProcessing = {
	...
	"inputs": {
		"images": {
			"name": "images",
			"tooltip": "The generated image to process."
		},
		"original_resized_images": {
			"name": "original_resized_images",
			"tooltip": "The original resized image before pre-processing, used as reference."
		},
	...
	"outputs": { "0": {
		"name": "images",
		"tooltip": "The aligned, color-corrected image."
	} }
};
var SeedVR2Preprocess = {
	...
	"inputs": { "resized_images": {
		"name": "resized_images",
		"tooltip": "The resized image to process."
	} },
	"outputs": { "0": {
		"name": "images",
		"tooltip": "The padded image for VAE encoding."
	} }
};
...
var SeedVR2TemporalMerge = {
	...
	"inputs": {
		"latents": {
			"name": "latents",
			"tooltip": "The sampled temporal chunks in sequence order."
		},
		"temporal_overlap": {
			"name": "temporal_overlap",
	...
	"outputs": { "0": {
		"name": "latent",
		"tooltip": "The recombined full-length latent."
	} }
};
...
var ShuffleVideoDataset = {
	...
	"inputs": {
		"videos": {
			"name": "videos",
			"tooltip": "List of videos to shuffle."
		},
		"seed": {
			"name": "seed",
			"tooltip": "Random seed."
...
var TextEncodeMageFlowEdit = {
	...
	"inputs": {
		...
		"width": {
			"name": "width",
			"tooltip": "Output width. 0 = use the first reference image's size."
		},
		"height": {
			"name": "height",
			"tooltip": "Output height. 0 = use the first reference image's size."
		},
		"batch_size": { "name": "batch_size" },
		"vae": { "name": "vae" }
	},
...
var TextOverlay = {
	...
	"inputs": {
		...
		"font_size": {
			"name": "font_size",
			"tooltip": "Font size as a percentage of the image height."
		},
		"color": {
			"name": "color",
		...
		"outline": {
			"name": "outline",
			"tooltip": "Draw a black outline around the text."
		}
	},
	"outputs": { "0": {
		"name": "images",
...
var TripoP1MultiviewToModelNode = {
	...
	"description": "Tripo P1 multiview-to-3D from 2-4 reference images in [front, left, back, right] order. Front is required; any combination of the other three may be omitted.",
	"inputs": {
		"image": {
			"name": "image",
			"tooltip": "Front view (0°). Required."
		},
		"output_mode": {
			"name": "output_mode",
			"tooltip": "\"Geometry only\" returns an untextured mesh. \"Textured\" adds color/PBR maps."
...
var USOStyleReference = {
	"display_name": "Apply USO Style Reference",
	"inputs": {
		"model": { "name": "model" },
		"model_patch": { "name": "model_patch" },
		"clip_vision_output": { "name": "clip_vision_output" }
...
var VAEDecode = {
	...
	"outputs": { "0": { "tooltip": "The decoded image." } }
};
var VAEDecodeAudio = {
...
var WanSCAILToVideo = {
	...
	"inputs": {
		...
		"reference_image": {
			"name": "reference_image",
			"tooltip": "Reference image. The first image is the primary reference (composite all identities onto it). SCAIL-2: extra batch images are used as additional views (back view, close-up, occluded background), each needing a matching reference_image_mask in that identity's color."
		},
		"reference_image_mask": {
			"name": "reference_image_mask",
			"tooltip": "SCAIL-2 only. Colored reference mask, batch matching reference_image (first = primary reference mask, rest = identity masks for the additional reference_image)."
		},
		"clip_vision_output": {
			"name": "clip_vision_output",
...
var WanUni3CControlnetApply = {
	"display_name": "Apply Wan Uni3C ControlNet",
	"inputs": {
		"model": { "name": "model" },
		"model_patch": { "name": "model_patch" },
		"vae": { "name": "vae" },
		"render_video": {
			"name": "render_video",
			"tooltip": "The guidance video rendered from the camera trajectory, most commonly warped point cloud renders of the input image."
		},
		"strength": { "name": "strength" },
		"start_percent": { "name": "start_percent" },
		"end_percent": { "name": "end_percent" }
...
//#endregion
export { CLIPMergeSimple as $, LTXVEmptyLatentAudio as $a, PerturbedAttentionGuidance as $c, SkipLayerGuidanceSD3 as $d, TomePatchModel as $f, ImageScaleToMaxDimension as $i, RegexMatch as $l, WanUni3CControlnetApply as $m, EmptyImage as $n, LoraModelLoader as $o, VideoLinearCFGGuidance as $p, GrokVideoEditNode as $r, ModelMergeAuraflow as $s, ControlNetApplyAdvanced as $t, SamplerER_SDE as $u, BriaVideoReplaceBackground as A, KlingOmniProFirstLastFrameNode as Aa, MultiGPU_WorkUnits as Ac, SaveWEBM as Ad, TCFG as Af, ImageBatch as Ai, RebatchLatents as Al, WanDancerPadKeyframes as Am, DualCFGGuider as An, LatentUpscaleModelLoader as Ao, UpscaleModelLoader as Ap, FreSca as Ar, MaskToImage as As, ComfyMathExpression as At, RunwayImageToVideoNodeGen4 as Au, ByteDanceImageToVideoNode as B, KlingVideoNode as Ba, OpenAIInputFiles as Bc, SelectVAEDevice as Bd, TextEncodeBooguEdit as Bf, ImageFromBatch as Bi, RecraftReplaceBackgroundNode as Bl, WanInfiniteTalkToVideo as Bm, ElevenLabsTextToSpeech as Bn, LoadImageOutput as Bo, VAEEncodeTiled as Bp, GeminiNanoBanana2 as Br, MeshyRefineNode as Bs, ConditioningSetAreaPercentage as Bt, SDTurboScheduler as Bu, BerniniConditioning as C, KlingImage2VideoNode as Ca, ModelSamplingDiscrete as Cc, SaveLatent as Cd, StringTrim as Cf, HunyuanVideo15SuperResolution as Ci, QuiverImageToSVGNode as Cl, Wan2VideoContinuationApi as Cm, DA3Inference as Cn, LatentMultiply as Co, TripoTextureNode as Cp, FluxKontextProImageNode as Cr, MagnificImageUpscalerCreativeNode as Cs, ClaudeNode as Ct, Rodin3D_Sketch as Cu, BriaRemoveVideoBackground as D, KlingLipSyncTextToVideoNode as Da, ModelSamplingStableCascade as Dc, SaveText as Dd, SyncLipSyncNode as Df, IdeogramV3 as Di, RandomCropImages as Dl, WanCameraImageToVideo as Dm, DiffusersLoader as Dn, LatentSubtract as Do, UNetSelfAttentionMultiply as Dp, FluxVTONode as Dr, ManualSigmas as Ds, CombineHooks4 as Dt, RunwayAleph2VideoToVideoNode as Du, BriaRemoveImageBackground as E, KlingLipSyncAudioToVideoNode as Ea, ModelSamplingSD3 as Ec, SaveSVGNode as Ed, StyleModelLoader as Ef, Ideogram4Scheduler as Ei, RTDETR_detect as El, WanCameraEmbedding as Em, DifferentialDiffusion as En, LatentRotate as Eo, UNetCrossAttentionMultiply as Ep, FluxProUltraImageNode as Er, MakeTrainingDataset as Es, CombineHooks2 as Et, RunwayAleph2PromptImageNode as Eu, ByteDanceCreateImageAsset as F, KlingSingleImageVideoEffectNode as Fa, OpenAIChatNode as Fc, SeedVR2Preprocess as Fd, TencentModelTo3DUVNode as Ff, ImageCompositeMasked as Fi, RecraftCreativeUpscaleNode as Fl, WanFunInpaintToVideo as Fm, ElevenLabsInstantVoiceClone as Fn, LoadBackgroundRemovalModel as Fo, VAEDecodeTiled as Fp, GLIGENTextBoxApply as Fr, MergeSplat as Fs, ConditioningAverage as Ft, SAM3_VideoTrack as Fu, ByteDanceTextToVideoNode as G, LTXVAddGuide as Ga, Painter as Gc, SetLatentNoiseMask as Gd, TextEncodeQwenImageEditPlus as Gf, ImageOnlyCheckpointLoader as Gi, RecraftTextToImageNode as Gl, WanPhantomSubjectToVideo as Gm, EmptyAudio as Gn, LoadTrainingDataset as Go, VOIDSampler as Gp, GenerateTracks as Gr, MinimaxImageToVideoNode as Gs, ConditioningSetProperties as Gt, SamplerARVideo as Gu, ByteDanceSeedNode as H, Krea2ImageNode as Ha, OpenRouterLLMNode as Hc, SetClipHooks as Hd, TextEncodeJoyImageEdit as Hf, ImageHistogram as Hi, RecraftStyleV3InfiniteStyleLibrary as Hl, WanMoveTrackToVideo as Hm, EmptyARVideoLatent as Hn, LoadLatent as Ho, VAESave as Hp, GeminiNode as Hr, MeshyTextToModelNode as Hs, ConditioningSetAreaStrength as Ht, SUPIRApply as Hu, ByteDanceCreateVideoAsset as I, KlingStartEndFrameNode as Ia, OpenAIDalle2 as Ic, SeedVR2TemporalChunk as Id, TencentSmartTopologyNode as If, ImageCrop as Ii, RecraftCrispUpscaleNode as Il, WanHuMoImageToVideo as Im, ElevenLabsSpeechToSpeech as In, LoadDA3Model as Io, VAEDecodeTripoSplat as Ip, GLSLShader as Ir, MergeTextLists as Is, ConditioningCombine as It, SCAIL2ColoredMask as Iu, CFGOverride as J, LTXVAudioVAELoader as Ja, PairConditioningSetProperties as Jc, ShuffleImageTextDataset as Jd, TextGenerateLTX2Prompt as Jf, ImageQuantize as Ji, RecraftV4TextToVectorNode as Jl, WanSoundImageToVideo as Jm, EmptyFlux2LatentImage as Jn, LoadVideoTextDataSetFromFolder as Jo, VPScheduler as Jp, GetSplatCount as Jr, MoGePanoramaInference as Js, ConditioningStableAudio as Jt, SamplerDPMAdaptative as Ju, CFGGuider as K, LTXVAudioVAEDecode as Ka, PairConditioningCombine as Kc, SetUnionControlNetType as Kd, TextEncodeZImageOmni as Kf, ImageOnlyCheckpointSave as Ki, RecraftTextToVectorNode as Kl, WanReferenceVideoApi as Km, EmptyChromaRadianceLatentImage as Kn, LoadVideo as Ko, VOIDWarpedNoise as Kp, GetICLoRAParameters as Kr, MinimaxTextToVideoNode as Ks, ConditioningSetPropertiesAndCombine as Kt, SamplerCustom as Ku, ByteDanceFirstLastFrameNode as L, KlingTextToVideoNode as La, OpenAIDalle3 as Lc, SeedVR2TemporalMerge as Ld, TencentTextToModelNode as Lf, ImageCropV2 as Li, RecraftImageInpaintingNode as Ll, WanImageToImageApi as Lm, ElevenLabsSpeechToText as Ln, LoadImage as Lo, VAEEncode as Lp, GeminiImage2Node as Lr, MeshyAnimateModelNode as Ls, ConditioningConcat as Lt, SDPoseDrawKeypoints as Lu, ByteDance2FirstLastFrameNode as M, KlingOmniProImageToVideoNode as Ma, NormalizeImages as Mc, SeedNode as Md, Tencent3DPartNode as Mf, ImageBlur as Mi, RecraftColorRGB as Ml, WanDancerVideo as Mm, DualModelGuider as Mn, Load3D as Mo, VAEDecodeAudio as Mp, FreeU_V2 as Mr, MediaPipeFaceMask as Ms, ComfyNumberConvert as Mt, SAM3_Detect as Mu, ByteDance2ReferenceNode as N, KlingOmniProTextToVideoNode as Na, NormalizeVideoLatentStart as Nc, SeedVR2Conditioning as Nd, Tencent3DTextureEditNode as Nf, ImageColorToMask as Ni, RecraftControls as Nl, WanFirstLastFrameToVideo as Nm, EasyCache as Nn, Load3DAdvanced as No, VAEDecodeAudioTiled as Np, GITSScheduler as Nr, MediaPipeFaceMeshVisualize as Ns, ComfyOrNode as Nt, SAM3_TrackPreview as Nu, BriaTransparentVideoBackground as O, KlingMotionControl as Oa, ModelSave as Oc, SaveTrainingDataset as Od, SyncTalkingImageNode as Of, IdeogramV4 as Oi, RandomNoise as Ol, WanContextWindowsManual as Om, DisableNoise as On, LatentUpscale as Oo, UNetTemporalAttentionMultiply as Op, FrameInterpolate as Or, MaskComposite as Os, CombineHooks8 as Ot, RunwayFirstLastFrameNode as Ou, ByteDance2TextToVideoNode as P, KlingOmniProVideoToVideoNode as Pa, OpenAIChatConfig as Pc, SeedVR2PostProcessing as Pd, TencentImageToModelNode as Pf, ImageCompare as Pi, RecraftCreateStyleNode as Pl, WanFunControlToVideo as Pm, ElevenLabsAudioIsolation as Pn, LoadAudio as Po, VAEDecodeHunyuan3D as Pp, GLIGENLoader as Pr, MergeImageLists as Ps, ComfySwitchNode as Pt, SAM3_TrackToMask as Pu, CLIPMergeAdd as Q, LTXVCropGuides as Qa, PerpNegGuider as Qc, SkipLayerGuidanceDiTSimple as Qd, ThresholdMask as Qf, ImageScaleBy as Qi, RegexExtract as Ql, WanTrackToVideo as Qm, EmptyHunyuanVideo15Latent as Qn, LoraLoaderModelOnly as Qo, VideoFrameSample as Qp, GrokImageNode as Qr, ModelMergeAdd as Qs, ControlNetApply as Qt, SamplerDPMPP_SDE as Qu, ByteDanceImageNode as R, KlingTextToVideoWithAudio as Ra, OpenAIGPTImage1 as Rc, SelectCLIPDevice as Rd, TextEncodeAceStepAudio as Rf, ImageDeduplication as Ri, RecraftImageToImageNode as Rl, WanImageToVideo as Rm, ElevenLabsTextToDialogue as Rn, LoadImageDataSetFromFolder as Ro, VAEEncodeAudio as Rp, GeminiImageNode as Rr, MeshyImageToModelNode as Rs, ConditioningMultiply as Rt, SDPoseFaceBBoxes as Ru, BeebleSwitchXVideoEdit as S, KlingFirstLastFrameNode as Sa, ModelSamplingContinuousV as Sc, SaveImageWebsocket as Sd, StringSubstring as Sf, HunyuanVideo15LatentUpscaleWithModel as Si, QuadrupleCLIPLoader as Sl, Wan2TextToVideoApi as Sm, DA3GeometryToMesh as Sn, LatentInterpolate as So, TripoTextToModelNode as Sp, FluxKontextMultiReferenceLatentMethod as Sr, MagnificImageStyleTransferNode as Ss, ChromaRadianceOptions as St, Rodin3D_Regular as Su, BriaImageEditNode as T, KlingImageToVideoWithAudio as Ta, ModelSamplingLTXV as Tc, SavePointCloud as Td, StyleModelApply as Tf, HypernetworkLoader as Ti, QwenImageDiffsynthControlnet as Tl, WanAnimateToVideo as Tm, DiffControlNetLoader as Tn, LatentOperationTonemapReinhard as To, UNETLoader as Tp, FluxProFillNode as Tr, Mahiro as Ts, ColorTransfer as Tt, RunwayAleph2KeyframeNode as Tu, ByteDanceSeedreamNode as U, Krea2StyleReferenceNode as Ua, OpticalFlowLoader as Uc, SetFirstSigma as Ud, TextEncodeMageFlowEdit as Uf, ImageInvert as Ui, RecraftStyleV3LogoRaster as Ul, WanMoveTracksFromCoords as Um, EmptyAceStep1_5LatentAudio as Un, LoadMediaPipeFaceLandmarker as Uo, VOIDInpaintConditioning as Up, GeminiNodeV2 as Ur, MeshyTextureNode as Us, ConditioningSetDefaultCombine as Ut, SV3D_Conditioning as Uu, ByteDanceSeedAudio as V, KlingVirtualTryOnNode as Va, OpenAIVideoSora2 as Vc, SelfAttentionGuidance as Vd, TextEncodeHunyuanVideo_ImageToVideo as Vf, ImageGrid as Vi, RecraftStyleV3DigitalIllustration as Vl, WanMoveConcatTrack as Vm, ElevenLabsVoiceSelector as Vn, LoadImageTextDataSetFromFolder as Vo, VAELoader as Vp, GeminiNanoBanana2V2 as Vr, MeshyRigModelNode as Vs, ConditioningSetAreaPercentageVideo as Vt, SD_4XUpscale_Conditioning as Vu, ByteDanceSeedreamNodeV2 as W, LTXAVTextEncoderLoader as Wa, OptimalStepsScheduler as Wc, SetHookKeyframes as Wd, TextEncodeQwenImageEdit as Wf, ImageMergeTileList as Wi, RecraftStyleV3RealisticImage as Wl, WanMoveVisualizeTracks as Wm, EmptyAceStepLatentAudio as Wn, LoadMoGeModel as Wo, VOIDQuadmaskPreprocess as Wp, GeminiVideoOmni as Wr, MinimaxHailuoVideoNode as Ws, ConditioningSetMask as Wt, SVD_img2vid_Conditioning as Wu, CLIPAttentionMultiply as X, LTXVConditioning as Xa, PatchModelAddDownscale as Xc, ShuffleVideoTextDataset as Xd, TextToLowercase as Xf, ImageRotate as Xi, ReferenceLatent as Xl, WanTextToImageApi as Xm, EmptyHunyuanImageLatent as Xn, LoraLoaderBypass as Xo, Veo3VideoGenerationNode as Xp, GrokImageEditNode as Xr, MoGeRender as Xs, ConditioningZeroOut as Xt, SamplerDPMPP_2S_Ancestral as Xu, CFGZeroStar as Y, LTXVConcatAVLatent as Ya, PairConditioningSetPropertiesAndCombine as Yc, ShuffleVideoDataset as Yd, TextOverlay as Yf, ImageRGBToYUV as Yi, RecraftVectorizeImageNode as Yl, WanSoundImageToVideoExtend as Ym, EmptyHiDreamO1LatentImage as Yn, LoraLoader as Yo, Veo3FirstLastFrameNode as Yp, GetVideoComponents as Yr, MoGePointMapToMesh as Ys, ConditioningTimestepsRange as Yt, SamplerDPMPP_2M_SDE as Yu, CLIPLoader as Z, LTXVContextWindows as Za, PerpNeg as Zc, SkipLayerGuidanceDiT as Zd, TextToUppercase as Zf, ImageScale as Zi, ReferenceTimbreAudio as Zl, WanTextToVideoApi as Zm, EmptyHunyuanLatentVideo as Zn, LoraLoaderBypassModelOnly as Zo, VeoVideoGenerationNode as Zp, GrokImageEditNodeV2 as Zr, ModelComputeDtype as Zs, ContextWindowsManual as Zt, SamplerDPMPP_3M_SDE as Zu, BasicScheduler as _, KlingAvatarNode as _a, ModelMergeWAN2_1 as _c, SaveGaussianSplat as _d, StringConcatenate as _f, Hunyuan3Dv2Conditioning as _i, PrimitiveBoundingBox as _l, VoxelToMeshBasic as _m, CreateVideo as _n, LatentCrop as _o, TripoRetargetNode as _p, FluxEraseNode as _r, LumaRay32VideoReframeNode as _s, CaseConverter as _t, ReveImageRemixNode as _u, AddTextSuffix as a, ImageYUVToRGB as aa, ModelMergeFlux1 as ac, SamplerLMS as ad, SplitAudioChannels as af, nodeDefs_default as ah, HappyHorseReferenceVideoApi as ai, PixverseTextToVideoNode as al, Vidu2StartEndToVideoNode as am, CosmosImageToVideoLatent as an, LTXVScheduler as ao, TransformSplat as ap, EmptyQwenImageLayeredLatentImage as ar, LumaConceptsNode as as, CLIPTextEncodeFlux as at, RepeatLatentBatch as au, BatchMasksNode as b, KlingCameraControls as ba, ModelSamplingAuraFlow as bc, SaveImageDataSetToFolder as bd, StringLength as bf, HunyuanRefinerLatent as bi, PrimitiveString as bl, Wan2ImageToVideoApi as bm, CurveEditor as bn, LatentFlip as bo, TripoSplatPreprocessImage as bp, FluxKontextImageScale as br, MagnificImageRelightNode as bs, CheckpointLoaderSimple as bt, Rodin3D_Gen25_Image as bu, AlignYourStepsScheduler as c, InvertMask as ca, ModelMergeMochiPreview as cc, SamplingPercentToSigma as cd, SplitSigmas as cf, wanBlockSwap as ch, HeyGenAvatarVideoNode as ci, PorterDuffImageComposite as cl, Vidu3StartEndToVideoNode as cm, CreateCameraInfo as cn, LatentAdd as co, TripleCLIPLoader as cp, ExtendIntermediateSigmas as cr, LumaImageNode as cs, CLIPTextEncodeKandinsky5 as ct, RescaleCFG as cu, AudioConcat as d, JsonExtractString as da, ModelMergeSD2 as dc, SaveAnimatedWEBP as dd, StableCascade_StageB_Conditioning as df, HeyGenTextToSpeechNode as di, PreviewAny as dl, ViduImageToVideoNode as dm, CreateHookKeyframesInterpolated as dn, LatentBatch as do, TripoImportModelNode as dp, FlipSigmas as dr, LumaRay32ExtendVideoNode as ds, CLIPTextEncodeSD3 as dt, ResizeImagesByLongerEdge as du, ImageScaleToTotalPixels as ea, ModelMergeBlocks as ec, SamplerEulerAncestral as ed, SolidMask as ef, WanVaceToVideo as eh, GrokVideoExtendNode as ei, PhotoMakerEncode as el, VideoRandomTemporalCrop as em, ControlNetApplySD3 as en, LTXVImgToVideo as eo, TopazImageEnhance as ep, EmptyLTXVLatentVideo as er, LoraSave as es, CLIPMergeSubtract as et, RegexReplace as eu, AudioEncoderEncode as f, KSampler as fa, ModelMergeSD35_Large as fc, SaveAudio as fd, StableCascade_StageC_VAEEncode as ff, HeyGenVideoTranslateNode as fi, PreviewAudio as fl, ViduMultiFrameVideoNode as fm, CreateHookLora as fn, LatentBatchSeedBehavior as fo, TripoMultiviewToModelNode as fp, Flux2ImageNode as fr, LumaRay32ImageToVideoNode as fs, CLIPTextEncodeSDXL as ft, ResizeImagesByShorterEdge as fu, BasicGuider as g, KarrasScheduler as ga, ModelMergeSubtract as gc, SaveGLB as gd, StringCompare as gf, HitPawVideoEnhance as gi, PrimitiveBoolean as gl, VoxelToMesh as gm, CreateList as gn, LatentConcat as go, TripoRefineNode as gp, FluxDisableGuidance as gr, LumaRay32VideoEditNode as gs, Canny as gt, ReveImageEditNode as gu, AudioMerge as h, Kandinsky5ImageToVideo as ha, ModelMergeSimple as hc, SaveAudioOpus as hd, StableZero123_Conditioning_Batched as hf, HitPawGeneralImageEnhance as hi, PreviewPointCloud as hl, ViduTextToVideoNode as hm, CreateHookModelAsLoraModelOnly as hn, LatentCompositeMasked as ho, TripoP1TextToModelNode as hp, Flux2Scheduler as hr, LumaRay32TextToVideoNode as hs, CLIPVisionLoader as ht, ReveImageCreateNode as hu, AddTextPrefix as i, ImageUpscaleWithModel as ia, ModelMergeCosmosPredict2_2B as ic, SamplerLCMUpscale as id, SplatToMesh as if, ZImageFunControlnet as ih, HappyHorseImageToVideoApi as ii, PixverseTemplateNode as il, Vidu2ReferenceVideoNode as im, ConvertDictionaryToString as in, LTXVReferenceAudio as io, TrainLoraNode as ip, EmptyMochiLatentVideo as ir, LtxvApiTextToVideo as is, CLIPTextEncodeControlnet as it, RepeatImageBatch as iu, BuildJsonPromptIdeogram as j, KlingOmniProImageNode as ja, NAGuidance as jc, ScaleROPE as jd, TemporalScoreRescaling as jf, ImageBlend as ji, RecordAudio as jl, WanDancerPadKeyframesList as jm, DualCLIPLoader as jn, LazyCache as jo, VAEDecode as jp, FreeU as jr, MediaPipeFaceLandmarker as js, ComfyNotNode as jt, RunwayTextToImageNode as ju, BriaVideoGreenScreen as k, KlingOmniProEditVideoNode as ka, Morphology as kc, SaveVideo as kd, T5TokenizerOptions as kf, ImageAddNoise as ki, RebatchImages as kl, WanDancerEncodeAudio as km, DrawBBoxes as kn, LatentUpscaleBy as ko, USOStyleReference as kp, FrameInterpolationModelLoader as kr, MaskPreview as ks, ComfyAndNode as kt, RunwayImageToVideoNodeGen3a as ku, AnimaLLLiteApply as l, JoinAudioChannels as la, ModelMergeQwenImage as lc, Save3DAdvanced as ld, SplitSigmasDenoise as lf, HeyGenCreateAvatarNode as li, Preview3D as ll, Vidu3TextToVideoNode as lm, CreateHookKeyframe as ln, LatentApplyOperation as lo, TripoConversionNode as lp, FeatherMask as lr, LumaImageNode2 as ls, CLIPTextEncodeLumina2 as lt, ResizeAndPadImage as lu, AudioEqualizer3Band as m, KSamplerSelect as ma, ModelMergeSDXL as mc, SaveAudioMP3 as md, StableZero123_Conditioning as mf, HiDreamO1ReferenceImages as mi, PreviewImage as ml, ViduStartEndToVideoNode as mm, CreateHookModelAsLora as mn, LatentComposite as mo, TripoP1MultiviewToModelNode as mp, Flux2ProImageNode as mr, LumaRay32KeyframesToVideoNode as ms, CLIPVisionEncode as mt, ResolutionSelector as mu, ARVideoI2V as n, ImageStitch as na, ModelMergeCosmos7B as nc, SamplerEulerCFGpp as nd, SoniloVideoToMusic as nf, WavespeedImageUpscaleNode as nh, GrokVideoReferenceNode as ni, PiDConditioning as nl, VideoTriangleCFGGuidance as nm, ControlNetLoader as nn, LTXVLatentUpsampler as no, TopazVideoEnhanceV2 as np, EmptyLatentHunyuan3Dv2 as nr, LotusConditioning as ns, CLIPSetLastLayer as nt, RenderSplat as nu, AdjustBrightness as o, InpaintModelConditioning as oa, ModelMergeKrea2 as oc, SamplerSASolver as od, SplitImageToTileList as of, unCLIPCheckpointLoader as oh, HappyHorseTextToVideoApi as oi, PixverseTransitionVideoNode as ol, Vidu2TextToVideoNode as om, CosmosPredict2ImageToVideoLatent as on, LTXVSeparateAVLatent as oo, TrimAudioDuration as op, EmptySD3LatentImage as or, LumaImageEditNode2 as os, CLIPTextEncodeHiDream as ot, ReplaceText as ou, AudioEncoderLoader as p, KSamplerAdvanced as pa, ModelMergeSD3_2B as pc, SaveAudioAdvanced as pd, StableCascade_SuperResolutionControlnet as pf, HiDreamO1PatchSeamSmoothing as pi, PreviewGaussianSplat as pl, ViduReferenceVideoNode as pm, CreateHookLoraModelOnly as pn, LatentBlend as po, TripoP1ImageToModelNode as pp, Flux2MaxImageNode as pr, LumaRay32KeyframeNode as ps, CLIPTextEncodeSDXLRefiner as pt, ResolutionBucket as pu, CFGNorm as q, LTXVAudioVAEEncode as qa, PairConditioningSetDefaultCombine as qc, ShuffleDataset as qd, TextGenerate as qf, ImagePadForOutpaint as qi, RecraftV4TextToImageNode as ql, WanSCAILToVideo as qm, EmptyCosmosLatentVideo as qn, LoadVideoDataSetFromFolder as qo, VOIDWarpedNoiseSource as qp, GetImageSize as qr, MoGeInference as qs, ConditioningSetTimestepRange as qt, SamplerCustomAdvanced as qu, AddNoise as r, ImageToMask as ra, ModelMergeCosmosPredict2_14B as rc, SamplerLCM as rd, SplatToFile3D as rf, WebcamCapture as rh, GrowMask as ri, PixverseImageToVideoNode as rl, Vidu2ImageToVideoNode as rm, ConvertArrayToString as rn, LTXVPreprocess as ro, TorchCompileModel as rp, EmptyLatentImage as rr, LtxvApiImageToVideo as rs, CLIPTextEncode as rt, RenormCFG as ru, AdjustContrast as s, InstructPixToPixConditioning as sa, ModelMergeLTXV as sc, SamplerSEEDS2 as sd, SplitImageWithAlpha as sf, unCLIPConditioning as sh, HappyHorseVideoEditApi as si, PolyexponentialScheduler as sl, Vidu3ImageToVideoNode as sm, CreateBoundingBoxes as sn, LaplaceScheduler as so, TrimVideoLatent as sp, ExponentialScheduler as sr, LumaImageModifyNode as ss, CLIPTextEncodeHunyuanDiT as st, ReplaceVideoLatentFrames as su, APG as t, ImageSharpen as ta, ModelMergeCosmos14B as tc, SamplerEulerAncestralCFGPP as td, SoniloTextToMusic as tf, WavespeedFlashVSRNode as th, GrokVideoNode as ti, PhotoMakerLoader as tl, VideoTemporalCrop as tm, ControlNetInpaintingAliMamaApply as tn, LTXVImgToVideoInplace as to, TopazVideoEnhance as tp, EmptyLatentAudio as tr, LossGraphNode as ts, CLIPSave as tt, RemoveBackground as tu, AudioAdjustVolume as u, JoinImageWithAlpha as ua, ModelMergeSD1 as uc, SaveAnimatedPNG as ud, StableCascade_EmptyLatentImage as uf, HeyGenTalkingPhotoNode as ui, Preview3DAdvanced as ul, ViduExtendVideoNode as um, CreateHookKeyframesFromFloats as un, LatentApplyOperationCFG as uo, TripoImageToModelNode as up, File3DToSplat as ur, LumaImageToVideoNode as us, CLIPTextEncodePixArtAlpha as ut, ResizeImageMaskNode as uu, BatchImagesNode as v, KlingCameraControlI2VNode as va, ModelNoiseScale as vc, SaveImage as vd, StringContains as vf, Hunyuan3Dv2ConditioningMultiView as vi, PrimitiveFloat as vl, Wan22FunControlToVideo as vm, CropByBBoxes as vn, LatentCut as vo, TripoRigNode as vp, FluxGuidance as vr, LumaReferenceNode as vs, CenterCropImages as vt, Rodin3D_Detail as vu, BetaSamplingScheduler as w, KlingImageGenerationNode as wa, ModelSamplingFlux as wc, SaveLoRA as wd, StripWhitespace as wf, HyperTile as wi, QuiverTextToSVGNode as wl, Wan2VideoEditApi as wm, DA3Render as wn, LatentOperationSharpen as wo, TruncateText as wp, FluxProExpandNode as wr, MagnificImageUpscalerPreciseV2Node as ws, ColorToRGBInt as wt, Rodin3D_Smooth as wu, BeebleSwitchXImageEdit as x, KlingDualCharacterVideoEffectNode as xa, ModelSamplingContinuousEDM as xc, SaveImageTextDataSetToFolder as xd, StringReplace as xf, HunyuanVideo15ImageToVideo as xi, PrimitiveStringMultiline as xl, Wan2ReferenceVideoApi as xm, CustomCombo as xn, LatentFromBatch as xo, TripoSplatSamplingPreview as xp, FluxKontextMaxImageNode as xr, MagnificImageSkinEnhancerNode as xs, CheckpointSave as xt, Rodin3D_Gen25_Text as xu, BatchLatentsNode as y, KlingCameraControlT2VNode as ya, ModelPatchLoader as yc, SaveImageAdvanced as yd, StringFormat as yf, HunyuanImageToVideo as yi, PrimitiveInt as yl, Wan22ImageToVideoLatent as ym, CropMask as yn, LatentCutToBatch as yo, TripoSplatConditioning as yp, FluxKVCache as yr, LumaVideoNode as ys, CheckpointLoader as yt, Rodin3D_Gen2 as yu, ByteDanceImageReferenceNode as z, KlingVideoExtendNode as za, OpenAIGPTImageNodeV2 as zc, SelectModelDevice as zd, TextEncodeAceStepAudio1_5 as zf, ImageFlip as zi, RecraftRemoveBackgroundNode as zl, WanImageToVideoApi as zm, ElevenLabsTextToSoundEffects as zn, LoadImageMask as zo, VAEEncodeForInpaint as zp, GeminiInputFiles as zr, MeshyMultiImageToModelNode as zs, ConditioningSetArea as zt, SDPoseKeypointExtractor as zu };

//# sourceMappingURL=nodeDefs-ODHPSASO.js.map