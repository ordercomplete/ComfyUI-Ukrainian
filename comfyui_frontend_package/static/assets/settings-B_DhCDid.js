//#region src/locales/ko/settings.json
var Comfy_Appearance_DisableAnimations = {
	"name": "애니메이션 비활성화",
	"tooltip": "대부분의 CSS 애니메이션과 전환 효과를 끕니다. 디스플레이 GPU가 생성에도 사용될 때 추론 속도가 빨라집니다."
};
...
var Comfy_ConfirmClear = { "name": "워크플로 비우기 시 확인 요구" };
var Comfy_DOMClippingEnabled = { "name": "DOM 요소 클리핑 활성화 (활성화 시 성능 저하 가능)" };
var Comfy_DevMode = { "name": "개발자 모드 옵션 활성화 (API 저장 등)" };
...
var Comfy_DisableSliders = { "name": "노드 위젯 슬라이더 비활성화" };
var Comfy_EditAttention_Delta = { "name": "Ctrl+위/아래 정밀도" };
var Comfy_EnableTooltips = { "name": "툴팁 활성화" };
var Comfy_EnableWorkflowViewRestore = { "name": "워크플로에서 캔버스 위치 및 확대/축소 수준 저장 및 복원" };
...
var Comfy_Graph_AutoPanSpeed = {
	"name": "자동 팬 속도",
	"tooltip": "캔버스 가장자리로 드래그할 때 자동 팬의 최대 속도입니다. 0으로 설정하면 자동 팬이 비활성화됩니다."
};
var Comfy_Graph_CanvasInfo = { "name": "왼쪽 하단 모서리에 캔버스 정보 표시 (fps 등)" };
var Comfy_Graph_CanvasMenu = { "name": "그래프 캔버스 메뉴 표시" };
var Comfy_Graph_CtrlShiftZoom = { "name": "빠른 확대/축소 단축키 활성화 (Ctrl + Shift + 드래그)" };
...
var Comfy_Graph_ZoomSpeed = { "name": "캔버스 확대/축소 속도" };
var Comfy_GroupSelectedNodes_Padding = { "name": "선택된 노드 패딩" };
var Comfy_Group_DoubleClickTitleToEdit = { "name": "그룹 제목을 두 번 클릭하여 편집" };
...
var Comfy_Load3D_3DViewerEnable = {
	"name": "3D 뷰어 활성화 (베타)",
	"tooltip": "선택한 노드에 대해 3D 뷰어(베타)를 활성화합니다. 이 기능을 통해 전체 크기의 3D 뷰어 내에서 3D 모델을 시각화하고 직접 상호작용할 수 있습니다."
};
var Comfy_Load3D_BackgroundColor = {
	"name": "초기 배경색",
	"tooltip": "3D 장면의 기본 배경색을 설정합니다. 이 설정은 새 3D 위젯이 생성될 때의 배경 모양을 결정하지만, 생성 후 각 위젯별로 개별 조정이 가능합니다."
};
var Comfy_Load3D_CameraType = {
	...
	"tooltip": "새 3D 위젯이 생성될 때 카메라가 기본적으로 원근(perspective) 또는 직교(orthographic)인지 제어합니다. 이 기본값은 생성 후 각 위젯별로 개별 전환이 가능합니다."
};
var Comfy_Load3D_LightAdjustmentIncrement = {
	"name": "조명 조정 단위",
	"tooltip": "3D 장면에서 조명 강도를 조정할 때의 증분 크기를 제어합니다. 더 작은 값은 조명 조정에 더 세밀한 제어를 제공하며, 더 큰 값은 한 번의 조정마다 더 뚜렷한 변화를 줍니다."
};
var Comfy_Load3D_LightIntensity = {
	"name": "초기 조명 강도",
	"tooltip": "3D 장면의 기본 조명 밝기 수준을 설정합니다. 이 값은 새 3D 위젯이 생성될 때 조명이 오브젝트를 얼마나 밝게 비추는지 결정하지만, 생성 후 각 위젯별로 개별 조정이 가능합니다."
};
var Comfy_Load3D_LightIntensityMaximum = {
	"name": "최대 조명 강도",
	"tooltip": "3D 장면에서 허용되는 최대 조명 강도 값을 설정합니다. 이 값은 모든 3D 위젯에서 조명 조정 시 설정할 수 있는 밝기의 상한선을 정의합니다."
};
var Comfy_Load3D_LightIntensityMinimum = {
	"name": "최소 조명 강도",
	"tooltip": "3D 장면에서 허용되는 최소 조명 강도 값을 설정합니다. 이 값은 모든 3D 위젯에서 조명 조정 시 설정할 수 있는 밝기의 하한선을 정의합니다."
};
...
var Comfy_Load3D_ShowGrid = {
	"name": "초기 그리드 표시",
	"tooltip": "새 3D 위젯이 생성될 때 그리드가 기본적으로 표시되는지 제어합니다. 이 기본값은 생성 후 각 위젯별로 개별 전환이 가능합니다."
};
var Comfy_Locale = { "name": "언어" };
...
var Comfy_NodeBadge_ShowApiPricing = { "name": "API 노드 가격 배지 표시" };
...
var Comfy_NodeSearchBoxImpl_FollowCursor = {
	"name": "추가된 노드가 커서를 따라감",
	"tooltip": "활성화하면, 검색 상자에서 추가된 노드가 클릭하여 배치할 때까지 커서를 따라갑니다. 기본 구현에만 적용됩니다."
};
...
var Comfy_Node_AllowImageSizeDraw = { "name": "이미지 미리보기 아래에 너비 × 높이 표시" };
var Comfy_Node_AlwaysShowAdvancedWidgets = {
	"name": "모든 노드에서 고급 위젯 항상 표시",
	"tooltip": "이 옵션을 활성화하면, 모든 노드에서 고급 위젯이 개별적으로 확장하지 않아도 항상 표시됩니다."
};
...
var Comfy_Node_DoubleClickTitleToEdit = { "name": "노드 제목을 두 번 클릭하여 편집" };
var Comfy_Node_MiddleClickRerouteNode = { "name": "중간 클릭으로 새 경유점 생성" };
var Comfy_Node_Opacity = { "name": "노드 불투명도" };
...
var Comfy_PromptFilename = { "name": "워크플로 저장 시 파일 이름 프롬프트" };
var Comfy_QueueButton_BatchCountLimit = {
	"name": "배치 수 제한",
	"tooltip": "한 번의 버튼 클릭으로 실행 큐에 추가되는 최대 작업 수입니다."
};
...
var Comfy_Sidebar_UnifiedWidth = { "name": "통합 사이드바 너비" };
var Comfy_SnapToGrid_GridSize = {
	"name": "그리드 크기에 스냅",
	"tooltip": "Shift 키를 누른 상태에서 노드를 드래그하거나 크기를 조절하면 그리드에 맞춰 정렬됩니다. 이 설정은 그리드의 크기를 제어합니다."
};
var Comfy_TextareaWidget_FontSize = { "name": "텍스트 영역 위젯 글꼴 크기" };
var Comfy_TextareaWidget_Spellcheck = { "name": "텍스트 영역 위젯 맞춤법 검사" };
var Comfy_TreeExplorer_ItemPadding = { "name": "트리 탐색기 항목 패딩" };
...
var Comfy_Validation_Workflows = { "name": "워크플로 유효성 검사" };
var Comfy_VueNodes_AutoScaleLayout = {
	"name": "자동 스케일 레이아웃 (Vue 노드)",
	"tooltip": "Vue 렌더링으로 전환 시 노드 위치를 자동으로 조정하여 겹침 방지"
};
var Comfy_VueNodes_Enabled = {
	"name": "모던 노드 디자인 (Vue 노드)",
	"tooltip": "모던: 향상된 상호작용, 기본 브라우저 기능, 업데이트된 시각적 디자인을 갖춘 DOM 기반 렌더링. 클래식: 전통적인 캔버스 렌더링."
};
...
var Comfy_Window_UnloadConfirmation = { "name": "창 닫을 때 확인 표시" };
...
var Comfy_Workflow_ConfirmDelete = { "name": "워크플로 삭제 시 확인 표시" };
var Comfy_Workflow_Persist = { "name": "페이지 (재)로드시 워크플로우 상태를 유지하고 복원" };
var Comfy_Workflow_ShowMissingModelsWarning = { "name": "누락된 모델 경고 표시" };
var Comfy_Workflow_SortNodeIdOnSave = { "name": "워크플로 저장 시 노드 ID 정렬" };
var Comfy_Workflow_WarnBlueprintOverwrite = { "name": "기존 서브그래프 블루프린트 덮어쓰기 전 확인 요청" };
...
//# sourceMappingURL=settings-B_DhCDid.js.map