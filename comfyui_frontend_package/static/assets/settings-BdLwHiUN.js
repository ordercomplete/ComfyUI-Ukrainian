//#region src/locales/zh/settings.json
var Comfy_Appearance_DisableAnimations = {
	"name": "禁用动画",
	"tooltip": "关闭大多数 CSS 动画和过渡效果。当显示 GPU 也用于生成时，可加快推理速度。"
};
...
var Comfy_ConfirmClear = { "name": "清除工作流时需要确认" };
var Comfy_DOMClippingEnabled = { "name": "启用DOM元素裁剪（启用可能会降低性能）" };
var Comfy_DevMode = { "name": "启用开发模式选项（API保存等）" };
var Comfy_DisableFloatRounding = {
	"name": "禁用默认浮点组件四舍五入。",
	"tooltip": "(需要重新加载页面) 当节点自身设置了四舍五入时，无法禁用该功能。"
};
var Comfy_DisableSliders = { "name": "禁用节点组件滑块" };
var Comfy_EditAttention_Delta = { "name": "Ctrl+上/下 精度" };
var Comfy_EnableTooltips = { "name": "启用工具提示" };
var Comfy_EnableWorkflowViewRestore = { "name": "在工作流中保存和恢复视图位置及缩放" };
...
var Comfy_Graph_AutoPanSpeed = {
	"name": "自动平移速度",
	"tooltip": "拖动到画布边缘时自动平移的最大速度。设置为 0 可禁用自动平移。"
};
var Comfy_Graph_CanvasInfo = { "name": "在左下角显示画布信息（fps等）" };
var Comfy_Graph_CanvasMenu = { "name": "显示图形画布菜单" };
var Comfy_Graph_CtrlShiftZoom = { "name": "启用快速缩放快捷键（Ctrl + Shift + 拖动）" };
...
var Comfy_Graph_ZoomSpeed = { "name": "画布缩放速度" };
var Comfy_GroupSelectedNodes_Padding = { "name": "选定节点的组内边距" };
var Comfy_Group_DoubleClickTitleToEdit = { "name": "双击组标题以编辑" };
...
var Comfy_Load3D_BackgroundColor = {
	"name": "初始背景颜色",
	"tooltip": "控制3D场景的默认背景颜色。此设置决定新建3D组件时的背景外观，但每个组件创建后可单独调整。"
};
var Comfy_Load3D_CameraType = {
	...
	"tooltip": "控制新建3D组件时相机默认是透视还是正交。此默认值可在每个组件创建后单独切换。"
};
var Comfy_Load3D_LightAdjustmentIncrement = {
	"name": "光照调整步长",
	"tooltip": "控制调整3D场景光照强度时的步长。较小的步长值可实现更精细的光照调整，较大的值则每次调整变化更明显。"
};
var Comfy_Load3D_LightIntensity = {
	"name": "初始光照强度",
	"tooltip": "设置3D场景光照的默认亮度级别。此值决定新建3D组件时灯光照亮物体的强度，但每个组件创建后可单独调整。"
};
var Comfy_Load3D_LightIntensityMaximum = {
	"name": "光照强度上限",
	"tooltip": "设置3D场景允许的最大光照强度值。此值定义在任何3D组件中调整光照时可设置的最大亮度。"
};
...
var Comfy_Load3D_ShowGrid = {
	"name": "初始网格可见性",
	"tooltip": "控制新建3D组件时网格是否默认可见。此默认值可在每个组件创建后单独切换。"
};
var Comfy_Locale = { "name": "语言" };
...
var Comfy_NodeBadge_ShowApiPricing = { "name": "显示 API 节点定价徽章" };
...
var Comfy_NodeSearchBoxImpl_FollowCursor = {
	"name": "新增节点跟随光标",
	"tooltip": "启用后，从搜索框添加的节点会跟随光标，直到点击以放置。仅适用于默认实现。"
};
...
var Comfy_Node_AllowImageSizeDraw = { "name": "在图像预览下方显示宽度×高度" };
var Comfy_Node_AlwaysShowAdvancedWidgets = {
	"name": "始终在所有节点上显示高级控件",
	"tooltip": "启用后，所有节点的高级控件将始终可见，无需单独展开。"
};
...
var Comfy_Node_DoubleClickTitleToEdit = { "name": "双击节点标题以编辑" };
var Comfy_Node_MiddleClickRerouteNode = { "name": "中键单击创建新的转接点" };
var Comfy_Node_Opacity = { "name": "节点不透明度" };
...
var Comfy_PromptFilename = { "name": "保存工作流时提示文件名" };
...
var Comfy_Queue_QPOV2 = {
	"name": "在资源侧边栏中使用统一作业队列",
	"tooltip": "将浮动作业队列面板替换为嵌入资源侧边栏的等效作业队列。您可以禁用此选项以恢复为浮动面板布局。"
};
var Comfy_RightSidePanel_ShowErrorsTab = {
	"name": "在侧边面板显示错误标签页",
	"tooltip": "启用后，右侧面板将显示一个错误标签页，可一目了然地查看工作流执行错误。"
};
...
var Comfy_Sidebar_UnifiedWidth = { "name": "统一侧边栏宽度" };
var Comfy_SnapToGrid_GridSize = {
	"name": "吸附网格大小",
	"tooltip": "在按住shift拖动和调整节点大小时，节点将对齐到网格，该选项控制网格。"
};
var Comfy_TextareaWidget_FontSize = { "name": "文本框组件字体大小" };
var Comfy_TextareaWidget_Spellcheck = { "name": "文本框组件拼写检查" };
var Comfy_TreeExplorer_ItemPadding = { "name": "树形浏览器项目内边距" };
...
var Comfy_Validation_Workflows = { "name": "校验工作流" };
...
var Comfy_Window_UnloadConfirmation = { "name": "关闭窗口时显示确认" };
...
var Comfy_Workflow_ConfirmDelete = { "name": "删除工作流时显示确认" };
var Comfy_Workflow_Persist = { "name": "持久化工作流状态并在页面（重新）加载时恢复" };
var Comfy_Workflow_ShowMissingModelsWarning = { "name": "显示缺失模型警告" };
var Comfy_Workflow_SortNodeIdOnSave = { "name": "保存节点ID到工作流" };
var Comfy_Workflow_WarnBlueprintOverwrite = { "name": "覆盖现有子图蓝图时需要确认" };
...
var LiteGraph_Canvas_MinFontSizeForLOD = {
	"name": "缩放节点细节级别 - 字体大小阈值",
	"tooltip": "控制节点何时切换为低质量LOD渲染。使用字体像素大小来决定何时切换。设置为0可禁用。1-24的数值设置LOD的最小字体大小阈值——数值越高（24px），缩小时越早切换为简化渲染，数值越低（1px），则在缩小时更长时间保持完整节点质量。"
};
var LiteGraph_ContextMenu_Scaling = { "name": "放大时缩放节点组合部件菜单（列表）" };
var LiteGraph_Group_SelectChildrenOnClick = {
	"name": "点击时选择组内所有子项",
	"tooltip": "启用后，点击分组会选择其中的所有节点和项目"
};
var LiteGraph_Node_DefaultPadding = {
	"name": "始终收缩新节点",
	"tooltip": "创建节点时将其缩小到最小可能尺寸。禁用后，新添加的节点会略微加宽以显示控件数值。"
};
var LiteGraph_Node_TooltipDelay = { "name": "工具提示延迟" };
...
//# sourceMappingURL=settings-BdLwHiUN.js.map